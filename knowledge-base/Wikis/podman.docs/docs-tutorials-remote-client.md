...
```
Your public key by default should be in your home directory under `$HOME/.ssh/id_ed25519.pub`. You then need to copy the contents of `id_ed25519.pub` and append it into `$HOME/.ssh/authorized_keys` on the Linux server. You can automate this using `ssh-copy-id`:
```
ssh-copy-id -i $HOME/.ssh/id_ed25519.pub 192.168.122.1
```
...
```
podman-remote system connection add myuser --identity $HOME/.ssh/id_ed25519 ssh://myuser@192.168.122.1/run/user/1000/podman/podman.sock
```

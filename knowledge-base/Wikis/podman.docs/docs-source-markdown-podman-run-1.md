...
```
$ podman run -d -e MARIADB_ROOT_PASSWORD=root --user mysql --userns=keep-id -v $HOME/data:/var/lib/mysql:Z,U mariadb
```
...
```
$ podman run -d -e MARIADB_ROOT_PASSWORD=root --user mysql --userns=keep-id:uid=999,gid=999 -v $HOME/data:/var/lib/mysql:Z mariadb
```
...
$ podman inspect healthcheck-example | jq '.[0].State.Health'
```

[Translate this page](//wiki.manjaro.org/index.php?title=Special:Translate&group=page-Manjaro+Packaging+Standards&language=en&action=page&filter= "Special:Translate")

Other languages:

[English](//wiki.manjaro.org/index.php?title=Manjaro_Packaging_Standards "Manjaro Packaging Standards (100% translated)") • ‎[русский](//wiki.manjaro.org/index.php?title=Manjaro_Packaging_Standards/ru "Стандарты пакетов Manjaro (100% translated)")
...
-   [[7] [Licenses]](#Licenses)
-   [[8] [Submitting packages to the AUR]](#Submitting_packages_to_the_AUR)
...
When building packages for Manjaro Linux, **adhere to the package guidelines** below, especially if the intention is to **contribute** a new package to Manjaro Linux. You should also see the [PKGBUILD](https://archlinux.org/pacman/PKGBUILD.5.html) and [makepkg](https://archlinux.org/pacman/makepkg.8.html) manpages.
...
-   **Avoid** using `/usr/libexec/` for anything. Use `/usr/lib/$/` instead.
...
-   **Configuration files** should be placed in the `/etc` directory. If there is more than one configuration file, it is customary to **use a subdirectory** in order to keep the `/etc` area as clean as possible. Use `/etc//` where `` is the name of the package (or a suitable alternative, eg, apache uses `/etc/httpd/`).
...
-   Package files should follow these **general directory guidelines**:
  ...
  `/usr/lib/`         Modules, plugins, etc.
  `/usr/share/doc/`   Application documentation
  `/usr/share/info`        GNU Info system files
  `/usr/share/man`         Manpages
  `/usr/share/`       Application data
  `/var/lib/`         Persistent application storage
  `/etc/`             Configuration files for ``
  `/opt/`             Large self-contained packages such as Java, etc.
  ----------------------------------------------------------------------------- --------------------------------------------------------------------------------------
...
When [makepkg](//wiki.manjaro.org/index.php?title=Makepkg "Makepkg") is used to build a package, it does the following automatically:
...
The [license](//wiki.manjaro.org/index.php?title=Licenses&action=edit&redlink=1 "Licenses (page does not exist)") array is being implemented in the official repos, and it **should** be used in your packages as well. Use it as follows:

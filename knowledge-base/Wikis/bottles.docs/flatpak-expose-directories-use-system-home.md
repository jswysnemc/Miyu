...
..
    |_ users/
        |_ mirko
            |_ AppData
            |_ Contacts
            |_ Desktop -> $HOME/Desktop
            |_ Documents -> $HOME/Documents
            |_ Downloads -> $HOME/Downloads
            |_ Favorites
            |_ Links
            |_ Music -> $HOME/Music
            |_ Pictures -> $HOME/Pictures
            |_ Saved Games
            |_ Searches
            |_ Temp
            |_ Videos -> $HOME/Videos
```
...
First we need to expose the homedir to Bottles, giving it the right permission ([read here](/flatpak/expose-directories.md) how to achieve this). Once the homedir is exposed, get into a terminal and move to the Bottles flatpak path:

```
cd $HOME/.var/app/com.usebottles.bottles/data/bottles/bottles
```

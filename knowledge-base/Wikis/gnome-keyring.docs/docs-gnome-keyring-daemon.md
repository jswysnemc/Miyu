...
`-r`, `--replace`
Try to replace a running keyring daemon, and assume its environment variables. A successful replacement depends on the `GNOME_KEYRING_CONTROL` environment variable being set by an earlier daemon.
...
Please send bug reports to either the distribution bug tracker or the upstream bug tracker at <https://gitlab.gnome.org/GNOME/gnome-keyring/-/issues/>
...
Further details available in the gnome-keyring online documentation at <https://wiki.gnome.org/GnomeKeyring> and in the secret-service online documentation at <http://standards.freedesktop.org/secret-service/>

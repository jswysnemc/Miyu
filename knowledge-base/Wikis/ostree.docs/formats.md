...
1.  [On the topic of “smart servers”](#on-the-topic-of-smart-servers)
2.  [The archive format](#the-archive-format)
3.  [archive efficiency](#archive-efficiency)
4.  [Aside: bare formats](#aside-bare-formats)
5.  [Static deltas](#static-deltas)
6.  [Static delta repository layout](#static-delta-repository-layout)
7.  [Static delta internal structure](#static-delta-internal-structure)
    1.  [The delta superblock](#the-delta-superblock)
8.  [A delta part](#a-delta-part)
9.  [Fallback objects](#fallback-objects)
...
In the [repo](repo.md) section, the concept of objects was introduced, where file/content objects are checksummed and managed individually. (Unlike a package system, which operates on compressed aggregates).
...
The biggest disadvantage of this format is that for a client to perform an update, one HTTP request per changed file is required. In some scenarios, this actually isn’t bad at all, particularly with techniques to reduce HTTP overhead, such as [HTTP/2](https://en.wikipedia.org/wiki/HTTP/2).
...
This “updates as code” model allows for multiple content generation strategies. The design of this was inspired by that of Chromium: [ChromiumOS Autoupdate](http://dev.chromium.org/chromium-os/chromiumos-design-docs/filesystem-autoupdate).
...
So then, what’s more interesting is that OSTree static deltas support a per-file delta algorithm called [bsdiff](https://github.com/mendsley/bsdiff) that most notably works well on executable code.

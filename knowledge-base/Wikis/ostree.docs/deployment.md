...
1.  [Overview](#overview)
    1.  [“stateroot” (AKA “osname”): Group of deployments that share /var](#stateroot-aka-osname-group-of-deployments-that-share-var)
    2.  [Contents of a deployment](#contents-of-a-deployment)
    3.  [Staged deployments](#staged-deployments)
    4.  [The system /boot](#the-system-boot)
...
While OSTree parallel installs deployments cleanly inside the `/ostree` directory, ultimately it has to control the system’s `/boot` directory. The way this works is via the [Boot Loader Specification](https://uapi-group.org/specifications/specs/boot_loader_specification/), which is a standard for bootloader-independent drop-in configuration files.

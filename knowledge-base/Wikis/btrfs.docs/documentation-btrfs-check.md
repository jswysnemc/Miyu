...
The filesystem checker is used to verify structural integrity of a filesystem and attempt to repair it if requested. It is recommended to unmount the filesystem prior to running the check, but it is possible to start checking a mounted filesystem (see `--force<man-check-option-force>`).
...
> [!NOTE]
> Even though the filesystem checker requires a device argument, it scans for all devices belonging to the same filesystem, thus it should not cause a difference using different devices of the same filesystem. Furthermore `btrfs-check` will automatically choose the good mirror, thus as long as there is a good copy for metadata, it will not report such case as an error.
...
that will most likely lead to a corruption unless the filesystem is in a quiescent state which may not be possible to guarantee.
...
`mkfs.btrfs`, `btrfs-scrub`, `btrfs-rescue`

...
- keep size (FALLOCFL_KEEP_SIZE)
- hole punching (FALLOCFL_PUNCH_HOLE)
- zero range (FALLOCFL_ZERO_RANGE)

FIEMAP
an ioctl that enumerates file extents, related tool is `filefrag`

filesystem label
another filesystem identification, could be used for mount or for better recognition, can be set or read by an ioctl or by command `btrfs filesystem label<man-filesystem-label>`

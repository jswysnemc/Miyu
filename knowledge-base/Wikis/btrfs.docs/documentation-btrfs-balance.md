...
-d\[\<filters\>\]
act on data block groups, see section `FILTERS<man-balance-filters>` for details about *filters*

-m\[\<filters\>\]
act on metadata chunks, see `FILTERS<man-balance-filters>` for details about *filters*

-s\[\<filters\>\]
act on system chunks (requires *-f*), see `FILTERS<man-balance-filters>` for details about *filters*.
...
A more comprehensive example when going from one to multiple devices, and back, can be found in section *TYPICAL USECASES* of `btrfs-device`.

...
The `pam_set_item(3)` and `pam_get_item(3)` functions allow applications and PAM service modules to set and retrieve PAM information.

The `pam_get_user(3)` function is the preferred method to obtain the username.

The `pam_set_data(3)` and `pam_get_data(3)` function allows PAM service modules to set and retrieve free-form data from one invocation to another.

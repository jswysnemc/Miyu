...
### Getting and setting *PAM_ITEM*s and *data*
...
Typically, the author may want to link against some standard system libraries. As when one compiles a normal program, this can be done for modules too: you simply append the `-l`*XXX* arguments for the desired libraries when you create the shared module object. To make sure a module is linked to the `libwhatever.so` library when it is `dlopen()`ed, try:

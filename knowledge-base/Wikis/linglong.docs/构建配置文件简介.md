...
### 软件包元信息配置 (`package`) [​](#软件包元信息配置-package)
...
```
  ...
  channel        构建产物的通道 (例如：`main`, `dev`)                 否

### 启动命令 (`command`) [​](#启动命令-command)
...
### 导出裁剪规则 (`exclude`/`include`) [​](#导出裁剪规则-exclude-include)
...
```
build: |
  qmake -makefile PREFIX=$ LIB_INSTALL_DIR=$/lib/$
  make
  make install
```
...
可选字段，用于将安装到 `$` 目录下的文件拆分成不同的模块。这对于按需下载或提供可选功能很有用。
...
```
  ...
  files     一个字符串列表，列出了属于该模块的文件或目录。路径是相对于 `$` 的路径。   是 (在每个模块项内)

**注意:** 所有安装到 `$` 下的文件都会被分配到一个模块中。未定义 `modules` 时，构建系统会自动生成默认的 `binary` 和 `develop` 模块。

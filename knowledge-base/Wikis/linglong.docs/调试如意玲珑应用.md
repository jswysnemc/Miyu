...
## 在终端中使用 gdb 进行调试 [​](#在终端中使用-gdb-进行调试)

### 在调试环境运行应用 [​](#在调试环境运行应用)

在 [运行编译后的应用](./../reference/commands/ll-builder/run.html) 一节中，我们已经知道使用`ll-builder run --exec /bin/bash`可以运行编译后的应用并进入容器的终端。只需要再 run 后面加上 `--debug` 参数即可以调试环境运行容器并进入容器。调试环境和非调试环境的区别主要有以下几点：
...
*如果运行环境没有 gdbserver 命令，请检查应用是否使用 org.deepin.base 作为 base，尝试升级到最新版本的 org.deepin.base。*

## 在 vscode 中使用 gdb 进行调试 [​](#在-vscode-中使用-gdb-进行调试)
...
```
/linglong/output/binary/files/bin/demo",
      "args": [],
      "stopAtEntry": true,
      "cwd": "$",
      "MIMode": "gdb",
      ...
      "setupCommands": [
        ...
        /linglong/output/develop/files/lib/debug"
        }
      ]
    }
  ]
...
-   \"program\": \"\$/linglong/output/binary/files/bin/demo\",
...
-   \"text\": \"set substitute-path /project \$\"
    ...
    这是设置源码路径的替换，`$` vscode 会自动替换成当前工作目录，如果需要更改为实际路径可以修改。

-   \"text\": \"set debug-file-directory \$/linglong/output/develop/files/lib/debug\"

    这里是设置调试文件的目录，如果调试符号没有保存到`develop`模块，需要修改为实际位置。

## 在 Qt Create 中使用 gdb 进行调试 [​](#在-qt-create-中使用-gdb-进行调试)
...
![qt-creator](/assets/qt-creator.BBad23wb.png)
...
## 从debian仓库下载调试符号 [​](#从debian仓库下载调试符号)
...
3.  在宿主机浏览器中打开仓库地址，定位依赖库的deb包所在目录:
    ...
    -   使用命令 `apt-cache show  | grep Filename` 查看deb包在仓库中的路径
...
5.  下载并解压调试符号包:
    ...
    [bash]
    ```
    dpkg-deb -R -dbgsym.deb /tmp/
    ```
    :::

6.  配置调试器查找调试符号: 在上面的场景设置debug-file-directory时，追加解压的目录，用冒号分隔:

    ::: language-
    []
    ```
    $/linglong/output/develop/files/lib/debug:/tmp//usr/lib/debug
    ```
    :::

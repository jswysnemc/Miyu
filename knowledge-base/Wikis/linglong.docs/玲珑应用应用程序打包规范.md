# 玲珑应用应用程序打包规范 [​](#玲珑应用应用程序打包规范)

本文中的关键词**必须**、**禁止**、**必要的**、**应当**、**不应**、**推荐的**、**允许**以及**可选的**\[\^rfc2119-keywords\]的解释见于[RFC 2119](https://datatracker.ietf.org/doc/html/rfc2119)中的描述。
...
-   对于GitHub上开发的第三方应用而言，如果该应用程序所在的组织有额外的域名，则应当优先使用，否则应当采用`io.github.<GITHUB_ID>.`作为前缀。
    ...
    特别的，如果该应用的组织名和应用名称一致，例如[https://github.com/neovim/neovim](https://github.com/neovim/neovim)，打包者**不应当**省略重复的应用名称和组织名，这个应用的ID**应当**为`io.github.neovim.neovim`。
...
以上规范来自[Desktop Entry Specification](https://specifications.freedesktop.org/desktop-entry-spec/latest)。

拓展阅读：[https://docs.flatpak.org/en/latest/conventions.html#application-ids](https://docs.flatpak.org/en/latest/conventions.html#application-ids)

### `prefix`和`$DESTDIR` [​](#prefix和-destdir)
...
当开发者没有指定安装位置时，默认值**应当**为`/usr/local`， 通过包管理系统打包时，包管理系统会配置其值。当使用dpkg相关工具进行打包时，其值会被配置为`/usr`。但编写构建/安装过程时，开发者应当考虑`prefix`被配置成任意值的情况。
...
本节主要参考[https://www.gnu.org/prep/standards/standards.html](https://www.gnu.org/prep/standards/standards.html)以及[https://wiki.debian.org/Multiarch/Implementation](https://wiki.debian.org/Multiarch/Implementation)中的相关内容编写。
...
-   desktop文件对应图标
    ...
    参见：[https://specifications.freedesktop.org/icon-theme-spec/latest/#install_icons](https://specifications.freedesktop.org/icon-theme-spec/latest/#install_icons)
...
本节主要参考[https://cmake.org/cmake/help/v3.30/module/GNUInstallDirs.html#module:GNUInstallDirs](https://cmake.org/cmake/help/v3.30/module/GNUInstallDirs.html#module:GNUInstallDirs)和[https://wiki.debian.org/Multiarch/Implementation](https://wiki.debian.org/Multiarch/Implementation)中的相关内容编写。
...
-   可执行文件
    ...
    参见：[https://cmake.org/cmake/help/v3.30/command/install.html#programs](https://cmake.org/cmake/help/v3.30/command/install.html#programs)

    ::: language-cmake
    [cmake]
    ```
    install(PROGRAMS $/path/to/your/executable TYPE BIN)
    ```
    :::
...
-   内部可执行文件
    ...
    [cmake]
    ```
    install(PROGRAMS $/path/to/your/executable DESTINATION $)
    ```
    :::
...
-   systemd系统级单元
    ...
    if(NOT SYSTEMD_SYSTEM_UNIT_PATH MATCHES ".*:$:.*")
        message(WARNING SYSTEMD_SYSTEM_UNIT_DIR="$" is not
...
-   systemd用户级单元
    ...
    if(NOT SYSTEMD_USER_UNIT_PATH MATCHES ".*:$:.*")
        message(WARNING SYSTEMD_USER_UNIT_DIR="$" is not
...
-   desktop文件对应图标
    ...
    install(FILES path/to/your.png
            DESTINATION $/icons/hicolor/48x48/apps)
    # Add more size of .png icons here...

    install(FILES path/to/your.svg
            DESTINATION $/icons/hicolor/scalable/apps)
    ```
    :::
...
-   **推荐**填写[`TryExec`字段](https://specifications.freedesktop.org/desktop-entry-spec/latest/recognized-keys.html#key-tryexec)，以确保应用已经卸载后，desktop文件不再有效
-   **推荐**填写[`WMClass`字段](https://specifications.freedesktop.org/desktop-entry-spec/latest/recognized-keys.html#key-startupwmclass)，以确保任务栏等基于窗口和应用程序匹配的桌面环境基础功能能正常工作
-   **推荐**填写[`Exec`字段](https://specifications.freedesktop.org/desktop-entry-spec/latest/recognized-keys.html#key-exec)时仅使用可执行文件名称而不是绝对路径
-   **不推荐**填写[`Icon`字段](https://specifications.freedesktop.org/desktop-entry-spec/latest/recognized-keys.html#key-icon)时使用绝对路径
...
这里以desktop文件为例，给出几个常见构建系统下生成最终配置文件的例子。 假设最终产物`org.deepin.demo.desktop`内容如下：
...
-   如果使用CMake作为构建系统。 1、编写desktop模板文件。
...
玲珑的环境最多有三部分组成，以在`x86_64`架构下编译`org.deepin.demo`为例，其头文件以及库文件默认搜索路径包含以下部分：
  ...
  **组成**          **包名**                 **头文件**                                **库文件**
  ----------------- ------------------------ ----------------------------------------- ------------------------------------------------------
  base              org.deepin.base          /usr/include                              /usr/lib\
                                                                                       /usr/lib/x86_64-linux-gnu

  runtime (可选)    org.deepin.runtime.dtk   /runtime/include                          /runtime/lib\
                                                                                       /runtime/lib/x86_64-linux-gnu

  app               org.deepin.demo          /opt/apps/org.deepin.demo/files/include   /opt/apps/org.deepin.demo/files/lib\
                                                                                       /opt/apps/org.deepin.demo/files/lib/x86_64-linux-gnu
  -------------------------------------------------------------------------------------------------------------------------------------------
...
**不推荐**直接在项目构建配置文件中硬编码上述默认搜索路径。 **不推荐**覆盖CFLAGS/CXXFLAGS等环境量，有额外参数**应当**向这些变量后追加。
...
##### 使用vscode linglong扩展 [​](#使用vscode-linglong扩展)
...
注意： deb的编译产物,安装前缀是`/usr`，`install_dep`脚本会自动处理其中的`.pc`文件，将`/usr`替换成`$PREFIX`。`xxx.cmake`类型文件无法批量处理，如果有硬编码路径行为，可能无法正常工作。
...
在构建以及安装过程中过程中，玲珑的构建工具会设置`$PREFIX`环境变量。该环境变量的值**应当**为`/opt/apps/$/files`，但并不建议在编写构建以及安装过程中直接使用其常见值，**推荐**总是读取`$PREFIX`环境变量。
...
玲珑的构建工具在应用构建成功后，会将产物提交到本地的缓存中。在进行离线分发时，应该导出成`.layer` 或`.uab`类型文件。
...
##### dbus services 目录 [​](#dbus-services-目录)
...
存放应用程序需要的各种文件，对于该目录放置文件并无限制，但是建议将可执行程序放置到 bin 子目录。应用程序或者插件依赖的第三方库推荐放置在/opt/apps/\$/files/lib 目录。

Manifest is a file containing checksums for files in the [ebuild](https://wiki.gentoo.org/wiki/Ebuild "Ebuild") directory.

A [Manifest](https://devmanual.gentoo.org/general-concepts/manifest/) can be generated by running the following [[ebuild](https://devmanual.gentoo.org/ebuild-writing/functions/)] command:

`user `[`$`]`ebuild `*`<ebuild>`*` manifest`

Or by using the following [[pkgdev](https://wiki.gentoo.org/wiki/Pkgdev "Pkgdev")] command:

`user `[`$`][`pkgdev`](https://wiki.gentoo.org/wiki/Pkgdev "Pkgdev")` manifest`

## Contents

-   [[1] [File format]](#File_format)
-   [[2] [Hash types]](#Hash_types)
-   [[3] [Thin manifest]](#Thin_manifest)
-   [[4] [Manifest signing]](#Manifest_signing)
-   [[5] [Recursive signed manifests (rsync)]](#Recursive_signed_manifests_.28rsync.29)
...
[FILE] **`Manifest`an example Manifest file (fragment of app-arch/gzip)**
...
According to this plan, BLAKE2B will be enabled on 2017-11-21. This means that starting at this time, all new and updated DIST entries in the Gentoo ebuild repository will use BLAKE2B+SHA512. Old DIST entries will still use the old (SHA256+SHA512+WHIRLPOOL) hash set until updated.^[\[1\]](#cite_note-1)^
...
Thin manifests are enabled in a repository through [thin-manifests](https://wiki.gentoo.org/wiki/Repository_format/metadata/layout.conf#thin-manifests "Repository format/metadata/layout.conf") entry in the [layout.conf] file.
...
[FILE] **`Manifest`an example signed Manifest**
...
The Manifest signing is enabled by default if portage has a GPG key set. It can be disabled explicitly for a repository through [sign-manifests](https://wiki.gentoo.org/wiki/Repository_format/metadata/layout.conf#sign-manifests "Repository format/metadata/layout.conf") entry of [layout.conf].

## [][Recursive signed manifests (rsync)]

See [Project:Portage/Repository_Verification#Recursive_signed_Manifests\_.28rsync.29](https://wiki.gentoo.org/wiki/Project:Portage/Repository_Verification#Recursive_signed_Manifests_.28rsync.29 "Project:Portage/Repository Verification")

## [References]

1.  [[[↑](#cite_ref-1)] [[gentoo-dev-announce: New Manifest hashes](https://archives.gentoo.org/gentoo-dev/message/ba2e5d9666ebd7e1bff1143485a37856)]]
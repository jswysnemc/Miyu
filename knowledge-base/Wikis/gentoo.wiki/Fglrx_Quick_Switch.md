...
These instructions are outdated because ati-drivers and old versions of xorg-server are no longer available.
...
-   [[3] [Procedure]](#Procedure)
    -   [[3.1] [If you have an older device\...]](#If_you_have_an_older_device...)
    -   [[3.2] [Everyone can continue from here\...]](#Everyone_can_continue_from_here...)

## [Rationale]

Unlike other distributions, Gentoo can install the [fglrx](https://wiki.gentoo.org/wiki/Fglrx "Fglrx") and open [Radeon](https://wiki.gentoo.org/wiki/Radeon "Radeon") drivers alongside each other, thanks to **eselect opengl**, but obviously you can only use one at a time. To make matters worse, fglrx can\'t be used while the open kernel modules are loaded, and older devices only support older versions of xorg-server.
...
I experimented with disabling the [framebuffer](https://wiki.gentoo.org/wiki/Framebuffer "Framebuffer") under the running kernel, unloading the modules, resetting the adapter\'s state with vbetool, and also faster reboots with kexec. While all of these worked by themselves, starting X with fglrx always caused a freeze or crash of some kind. Sadly it seems that a proper reboot is still necessary.
...
[CODE]
    ...
    @@ -240,11 +240,13 @@
       ...
       linux_entry "$" "$" advanced \
    -              "$ $"
    +              "$ $ modprobe.blacklist=fglrx"
       if [ "x$" != "xtrue" ]; then
         linux_entry "$" "$" recovery \
    -                "single $"
    +                "single $ modprobe.blacklist=fglrx"
       fi
    +  linux_entry "$" "$ (fglrx)" advanced \
    +              "$ $ modprobe.blacklist=drm,radeon"

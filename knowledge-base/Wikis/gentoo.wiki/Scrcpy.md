...
[[]][GitHub](https://github.com/Genymobile/scrcpy.git)

[[]][Wikipedia](https://en.wikipedia.org/wiki/Scrcpy "wikipedia:Scrcpy")

**scr**een**c**o**py** mirrors Android devices (video and audio) connected via USB or TCP/IP and allows control using the computer\'s keyboard and mouse.
...
`root `[`#`]`emerge --ask app-mobilephone/scrcpy`

## [Configuration]

Complete the device handshake to mirror video. The Android device must have **USB debugging** enabled in its **Settings**, and be connected through USB. On the computer, run [scrcpy]. A window will appear on the Android where **Allow** should be answered to **Allow USB debugging?**
...
`user `[`$`]`scrcpy --video-bit-rate=100M --max-fps=144 --capture-orientation 90 -m 820 --no-audio`

    scrcpy 3.2 <https://github.com/Genymobile/scrcpy>
    INFO: ADB device found:
    INFO:     -->   (usb)  <...>                device <...>
    /usr/share/scrcpy/scrcpy-server: 1 file pushed, 0 skipped. 33.2 MB/s (90888 bytes in 0.003s)
    [server] INFO: Device: [FIH] <...> (Android 11)
    INFO: Renderer: opengl
    INFO: OpenGL version: 4.6 (Compatibility Profile) Mesa 25.0.7
    INFO: Trilinear filtering enabled
    INFO: Texture: 360x816

Adding **\--video-bit-rate=100M** and **\--max-fps=144** streams higher quality video.

The rotation in degrees is set by **\--capture-orientation**.

The decreased horizontal resolution in pixels is set by **-m**.

Using **\--no-audio** forces Android to keep playing sound while connected through USB.

## [[] External resources]

-   [https://github.com/Genymobile/scrcpy/blob/master/doc/audio.md](https://github.com/Genymobile/scrcpy/blob/master/doc/audio.md) - for more option descriptions.
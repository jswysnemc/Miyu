...
Calculate the directory hash of *filename*. The *-c* option will casefold the filename before calculating the hash. The *-v* option will make the **dx_hash** command more verbose and print the hash algorithm and hash seed to calculate the hash. If a file system is open, use the hash_seed and default hash_algorithm used by the file system, although these can be overridden by the *-h* and *-s* options. The hash algorithm specified with *-h* may be **legacy**,** half_md4**, or **tea**. The hash seed specified with *-s* must be in UUID format.

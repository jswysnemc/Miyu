...
@JDEV@.TP @JDEV@.BI device= external-journal @JDEV@Attach the file system to the journal block device located on @JDEV@*external-journal*. The external @JDEV@journal must have been already created using the command @JDEV@.IP @JDEV@.B mke2fs -O journal_dev @JDEV@.I external-journal @JDEV@.IP @JDEV@Note that *external-journal* must be formatted with the same block @JDEV@size as file systems which will be using it. @JDEV@In addition, while there is support for attaching @JDEV@multiple file systems to a single external journal, @JDEV@the Linux kernel and **e2fsck**(8) @JDEV@do not currently support shared external journals yet. @JDEV@.IP @JDEV@Instead of specifying a device name directly, *external-journal* @JDEV@can also be specified by either **LABEL=***label* or **UUID=***UUID* @JDEV@to locate the external journal by either the volume label or UUID @JDEV@stored in the ext2 superblock at the start of the journal. Use @JDEV@**dumpe2fs**(8) @JDEV@to display a journal device's volume label and UUID. See also the @JDEV@**-L** option of **tune2fs**(8).
...
**orphan_file**
Store the list of orphan inodes in multiple blocks instead of a single linked-list to avoid potential scalability bottlenecks for workloads that perform a large number of file truncations or extensions in parallel.
...
**uninit_bg**
Allow the kernel to initialize bitmaps and inode tables lazily, and to keep a high watermark for the unused inodes in a file system, to reduce **e2fsck**(8) time. The first **e2fsck**(8) run after enabling this feature will take the full time, but subsequent **e2fsck**(8) runs will take only a fraction of the original time, depending on how full the file system is.
...
**-Q*** quota-options*
Sets the **quota** feature on the superblock and works on the quota files for the given quota type. Quota options could be one or more of the following:

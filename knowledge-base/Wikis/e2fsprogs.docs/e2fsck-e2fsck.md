...
**e2fsck** \[ **-pacnyrdfkvtDFV** \] \[ **-b** *superblock* \] \[ **-B** *blocksize* \] \[ **-l**\|**-L** *bad_blocks_file* \] \[ **-C** *fd* \] @JDEV@\[ @JDEV@.B -j @JDEV@.I external-journal @JDEV@\] \[ **-E** *extended_options* \] \[ **-z** *undo_file* \] *device*
...
**readahead_kb**
Use this many KiB of memory to pre-fetch metadata in the hopes of reducing e2fsck runtime. By default, this is set to the size of two block groups' inode tables (typically 4 MiB on a regular ext4 file system); if this amount is more than 1/50th of total physical memory, readahead is disabled. Set this to zero to disable readahead entirely.
...
**-F**
Flush the file system device's buffer caches before beginning. Only really useful for doing **e2fsck** time trials. @JDEV@.TP @JDEV@.BI -j " external-journal" @JDEV@Set the pathname where the external-journal for this file system can be @JDEV@found.
...
**-n**
Open the file system read-only, and assume an answer of \`no' to all questions. Allows **e2fsck** to be used non-interactively. This option may not be specified at the same time as the **-p** or **-y** options.

...
- /etc/dbus-1/system-services: this location is specified by the D-Bus Specification, and is suitable for software installed locally by the system administrator, or by an asset manager deploying services that are not part of the OS (perhaps while /usr/ is read-only)

- /run/dbus-1/system-services: this location is specified by the D-Bus Specification, and is suitable for ephemeral services that will disappear after a reboot
...
You can't include both send\_ and receive\_ attributes on the same rule, since "whether the message can be sent" and "whether it can be received" are evaluated separately.

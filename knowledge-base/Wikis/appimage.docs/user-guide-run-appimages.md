...
> Information on how to run AppImages was moved into our `ref-quickstart` page.
>
> Please see `ref-how-to-run-appimage` for more information.
...
An alternative to mounting the AppImages is to extract their contents. This allows for modifying the contents. The resulting directory is a valid `AppDir`, and users can create AppImages from them again using `ref-appimagetool`.

Analog to mounting AppImages, there is a simple commandline switch to extract the contents of type 2 AppImages without external tools. Just call the AppImage with the parameter `--appimage-extract`. This will cause the `ref-runtime` to create a new directory called `squashfs-root`, containing the contents of the AppImage's `AppDir`.

Type 1 AppImages require the deprecated tool [AppImageExtract](https://github.com/AppImage/AppImageKit/releases/6) to extract the contents of an AppImage. It's very limited functionality wise, and requires a GUI to run. It creates a new directory in the user's desktop directory.
...
[appimaged](https://github.com/probonopd/go-appimage/releases) is a daemon that monitors the system and integrates AppImages. It monitors a predefined set of directories on the user's system searching for AppImages, and integrates them into the system using `libappimage`.
...
[AppImageLauncher](https://github.com/TheAssassin/AppImageLauncher) is a helper application for Linux distributions serving as a kind of "entry point" for running and integrating AppImages. It makes a user's system AppImage-ready™.
...
See also:
More information about AppImageLauncher can be found in `ref-appimagelauncher`.

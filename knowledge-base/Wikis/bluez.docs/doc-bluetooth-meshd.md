...
`-c <file>, --config <file>`
Specifies an explicit config file path instead of relying on the default path(*@CONFIGDIR@/mesh-main.conf*) for the config file.

`-s <dir_path>, --storage <dir path>`
Specifies an explicit storage directory path instead of the default path(*@MESH_STORAGEDIR@*) for storing mesh node(s) configuration.

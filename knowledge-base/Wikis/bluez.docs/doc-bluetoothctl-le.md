...
This submenu manages LE (Low Energy) bearer connections using the **org.bluez.Device(5)** interface.
...
\[dev\]
Bluetooth device address (optional, shows all LE bearers if omitted)
...
An active scan report is required before the connection can be established. If no advertising report is received before the timeout, a le-connection-abort-by-local error will be issued.

Usage
**\> connect \<dev\>**

Uses
**org.bluez.Device(5)** method **Connect**
...
By default this command disconnects all profiles/services associated with the le connection, and then terminates the le link.

Usage
**\> disconnect \<dev\>**

Uses
**org.bluez.Device(5)** method **Disconnect**

\<dev\>
Bluetooth device address to disconnect from

...
Object path
\[variable prefix\]/{hci0,hci1,...}/[dev](){BDADDR}/service#/char#
...
As a client if indication procedure is used the confirmation is generated automatically once received, for a server if the file descriptor is writable (POLLOUT) then upon receiving a confirmation from the client one byte (0x01) is written to the file descriptor.
...
For client properties is omitted in case 'write-without-response' flag is not set.

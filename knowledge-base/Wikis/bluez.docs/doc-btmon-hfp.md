...
1.  `AT+BRSF=<features>` / `+BRSF:<features>` -- Supported features bitmask exchange
2.  `AT+BAC=1,2` -- Available codecs (if codec negotiation supported). Codec IDs: 1 = CVSD, 2 = mSBC, 3 = LC3-SWB
...
3.  Find RFCOMM SABM/UA on DLCI 0 then target DLCI -- multiplexer and data channel
4.  Find AT+BRSF in hex dumps -- feature exchange, check codec negotiation bit
...
9.  Find Synchronous Connection Complete -- check Status, Link type, Air mode

...
This submenu manages BR/EDR (Classic Bluetooth) bearer connections using the **org.bluez.Device(5)** interface.
...
\[dev\]
Bluetooth device address or object path (optional, uses current device if omitted)
...
By default, it establishes the bredr connection and then connects all profiles that marked as auto-connectable.
...
By default this command disconnects all profiles associated with the bredr connection, and then terminates the bredr link.

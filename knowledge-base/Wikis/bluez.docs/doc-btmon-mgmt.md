...
| 0x14 | Permission Denied | Process lacks required privileges (CAPNET_ADMIN) |
...
1.  Find `MGMT Open` -- confirm `bluetoothd` connected and note socket ID
2.  Check `Read Controller Information` -- verify address, version, and current settings
3.  Find `Set Powered` -- confirm the adapter was powered on successfully
4.  Search for non-Success status codes -- these indicate failures
5.  For pairing issues, find `Pair Device` and trace forward to its `Command Complete` response
6.  For connection issues, find `Device Connected`/`Device Disconnected` and check the disconnect reason

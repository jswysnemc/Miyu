# GameMode Configuration Reference

The daemon is configured with a `gamemode.ini` file. Configuration files are loaded and merged from `$PWD`,`$XDG_CONFIG_HOME` or `$HOME/.config/`,`/etc/`, and `/usr/share/gamemode/`, from highest to lowest priority.

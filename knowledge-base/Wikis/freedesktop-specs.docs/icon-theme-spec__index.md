...
Publication Date: July 2 2013, Version: Version 0.13

[1 Overview](#overview)

[2 Definitions](#definitions)

[3 Directory Layout](#directory_layout)

[4 File Formats](#file_formats)

[5 Icon Lookup](#icon_lookup)

[6 Example](#example)

[7 Installing Application Icons](#install_icons)

[8 Implementation Notes](#implementation_notes)

[9 Background](#background)

[A Change history](#changes)
...
In order to have a place for third party applications to install their icons there should always exist a theme called "hicolor" [^(\[1\])](#ftn.id-1.4.3.1). The data for the hicolor theme is available for download at: https://icon-theme.freedesktop.org/releases/. Implementations are required to look in the "hicolor" theme if an icon was not found in the current theme.
...
| Context | The context the icon is normally used in. This is in detail discussed in [Section 4.1, “Context”](#context "4.1. Context"). | string | NO |
...
| MaxSize | Specifies the maximum (unscaled) size that the icons in this directory can be scaled to. Defaults to the value of Size if not present. | integer | NO |
| MinSize | Specifies the minimum (unscaled) size that the icons in this directory can be scaled to. Defaults to the value of Size if not present. | integer | NO |
| Threshold | The icons in this directory can be used if the size differ at most this much from the desired (unscaled) size. Defaults to 2 if not present. | integer | NO |
...
## 7 Installing Application Icons
...
[^(\[1\])](#id-1.4.3.1) This name is chosen for backwards compatibility with the old KDE default theme

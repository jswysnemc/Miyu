...
Public Identifier for 1.1  
`PUBLIC "-//freedesktop//DTD Menu 1.0//EN"`

System Identifier for 1.1  
...
Desktop entries in the pool of available entries are identified by their *desktop-file id* (see [Desktop-File Id](go01.md#term-desktop-file-id "Desktop-File Id")). The desktop-file id of a desktop entry is equal to its filename, with any path components removed. So given a \<AppDir\> `/foo/bar` and desktop entry `/foo/bar/Hello.desktop` the desktop entry would get a desktop-file id of `Hello.desktop`
...
Duplicate \<AppDir\> elements (that specify the same directory) should be ignored, but the *last* duplicate in the file should be used when establishing the order in which to scan the directories. This is important when merging (see [Section 5, “Merging”](merge-algorithm.html "5. Merging")). The order of \<AppDir\> elements with respect to \<Include\> and \<Exclude\> elements is not relevant, also to facilitate merging.
...
Directory entries in the pool of available entries are identified by their *relative path* (see [Relative path](go01.md#term-relative-path "Relative path")).
...
\<Include\> elements must appear immediately under \<Menu\> elements. The desktop entries they match are included in the menu. \<Include\> and \<Exclude\> elements for a given \<Menu\> are processed in order, with queries earlier in the file handled first. This has implications for merging, see [Section 5, “Merging”](merge-algorithm.html "5. Merging"). See [Section 6, “Generating the menus”](query-algorithm.html "6. Generating the menus") for full details on how to process \<Include\> and \<Exclude\> elements.
...
\<Filename\>  
The \<Filename\> element is the most basic matching rule. It matches a desktop entry if the desktop entry has the given desktop-file id. See [Desktop-File Id](go01.md#term-desktop-file-id "Desktop-File Id").
...
Any number of \<MergeFile\> elements may be listed below a \<Menu\> element, giving the name of another menu file to be merged into this one. [Section 5, “Merging”](merge-algorithm.html "5. Merging") specifies how merging is done. The root \<Menu\> of the merged file will be merged into the immediate parent of the \<MergeFile\> element. The \<Name\> element of the root \<Menu\> of the merged file are ignored.
...
\<Move\>  
This element may only appear below \<Menu\>. The \<Move\> element contains pairs of \<Old\>/\<New\> elements indicating how to rename a descendant of the current \<Menu\>. If the destination path already exists, the moved menu is merged with the destination menu (see [Section 5, “Merging”](merge-algorithm.html "5. Merging") for details).
...
\<Move\> is also useful for implementing menu editing, see [Section D.1, “Menu editing”](implementation-notes.md#menu-editing "D.1. Menu editing").

\<Old\>  
This element may only appear below \<Move\>, and must be followed by a \<New\> element. The content of both \<Old\> and \<New\> should be a menu path (slash-separated concatenation of \<Name\> fields, see [Menu path](go01.md#term-menu-path "Menu path")). Paths are interpreted relative to the menu containing the \<Move\> element.

...
If **lvm** is invoked with no arguments it presents a \#DEFAULT_LIBLINE# prompt (assuming it was compiled with \#DEFAULT_LIBLINE# support). LVM commands may be entered interactively at this prompt with \#DEFAULT_LIBLINE# facilities including history and command name and option completion. Refer to **\#DEFAULT_LIBLINE#**(3) for details.
...
For performance metrics, use **dmstats**(8) or to manipulate the kernel device-mapper driver used by LVM directly, use **dmsetup**(8).
...
**HOME**
Directory containing *.lvm_history* if the internal \#DEFAULT_LIBLINE# shell is invoked.
...
**LVM_RUN_BY_DMEVENTD**
This variable is normally set by dmeventd plugin to inform lvm command it is running from dmeventd plugin so lvm takes some extra action to avoid communication and deadlocks with dmeventd.

**LVM_SYSTEM_DIR**
Directory containing **lvm.conf**(5) and other LVM system files. Defaults to "*\#DEFAULT_SYS_DIR#*".
...
**LVM_LOG_FILE_EPOCH**
A string of up to 32 letters appended to the log filename and followed by the process ID and a startup timestamp using this format string "\_%s\_%d\_%llu". When set, each process logs to a separate file.
...
*\#DEFAULT_SYS_DIR#/lvm.conf*
*\#DEFAULT_SYS_DIR#/lvmlocal.conf*
*\$HOME/.lvm_history*
...
**dmsetup**(8), **dmstats**(8), **\#DEFAULT_LIBLINE#**(3)

...
- A read and write hot-spot cache, using the dm-cache kernel module. This cache tracks access patterns and adjusts its content deliberately so that commonly used parts of the main LV are likely to be found on the fast storage. LVM refers to this using the LV type **cache**.

- A write cache, using the dm-writecache kernel module. This cache can be used with SSD or PMEM devices to speed up all writes to the main LV. Data read from the main LV is not stored in the cache, only newly written data. LVM refers to this using the LV type **writecache**.
...
Once the fast LV has been attached to the main LV, lvm reports the main LV type as either **cache** or **writecache** depending on the type used. While attached, the fast LV is hidden, and renamed with a \_cvol or \_cpool suffix. It is displayed by lvs -a. The \_corig or \_wcorig LV represents the original LV without the cache.
...
The main LV is created with the specified Name and Size from the slow_hdd. A hidden fast LV is created on the fast_ssd and is then attached to the new main LV. If the fast_ssd is unused, the entire disk will be used as the cache unless the --cachesize option is used to specify a size for the fast LV. The --cachedevice option can be repeated to use multiple disks for the fast LV.
...
When a file system like xfs already exists on the main LV prior to caching, and the file system is using a block size of 512, then the writecache block size should be set to 512. (The file system will likely fail to mount if a writecache block size of 4096 is used in this case.)
...
Pause writeback if there was some write I/O redirected to the origin volume in the last number of milliseconds.

## dm-writecache using metadata profiles

In addition to specifying writecache settings on the command line, they can also be set in **lvm.conf**(5), or in a profile file, using the allocation/cache_settings/writecache config structure shown below.

It's possible to prepare a number of different profile files in the *\#DEFAULT_SYS_DIR#/profile* directory and specify the file name of the profile when starting writecache.
...
The size of data blocks managed by dm-cache can be specified with the --chunksize option when caching is started. The default unit is KiB. The value must be a multiple of 32 KiB between 32 KiB and 1 GiB. Cache chunks bigger than 512KiB shall be only used when necessary.
...
To unset/drop cache setting and restore its default kernel value use special keyword 'default' as option parameter:
...
User can set migration threshold via cache policy settings as "migration_threshold=\<#sectors\>" to set the maximum number of sectors being migrated, the default being 2048 sectors (1 MiB) or 8 cache chunks whichever of those two values is larger.
...
**lvm.conf**(5), **lvchange**(8), **lvcreate**(8), **lvdisplay**(8), **lvextend**(8), **lvremove**(8), **lvrename**(8), **lvresize**(8), **lvs**(8), **lvmraid**(7), **lvmthin**(7),

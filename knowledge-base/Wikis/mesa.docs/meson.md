...
> [!NOTE]
> If your distribution doesn't have something recent enough in its repositories, you can use the methods suggested at <https://mesonbuild.com/Getting-meson.html> to install the current version of Meson.
...
You'll also need [Ninja](https://ninja-build.org/). If it's not
...
directory](https://mesonbuild.com/Using-multiple-build-directories.html)
...
[meson.options](https://gitlab.freedesktop.org/mesa/mesa/-/blob/main/meson.options)
...
complicated](https://mesonbuild.com/Build-options.html#using-build-options),
...
file](https://mesonbuild.com/Native-environments.html), or with the
...
Meson default to installing `libGL.so` in your system's main
`lib/` directory and DRI drivers to a `dri/` subdirectory.
...
[here](https://mesonbuild.com/Reference-tables.html#compiler-ids).
...
`libllvm.so`. There is also a `-Dcmake_module_path` option,
...
file"](https://mesonbuild.com/Native-environments.html), these files
...
file"](https://mesonbuild.com/Cross-compilation.html#defining-the-environment)
...
```
See the Cross Compilation section for more
information.
...
[wrap](https://mesonbuild.com/Wrap-dependency-system-manual.html), in
...
passed to `meson` than to `meson configure`. These options are
...
[Meson supports
cross-compilation](https://mesonbuild.com/Cross-compilation.html) by
...
`$XDG_DATA_HOME/meson/cross` or `$HOME/.local/share/meson/cross`
...
-  [meson-cross-x86-linux-gnu](https://aur.archlinux.org/packages/meson-cross-x86-linux-gnu)
-  [meson-cross-aarch64-linux-gnu](https://github.com/dcbaker/archlinux-meson-cross-aarch64-linux-gnu)

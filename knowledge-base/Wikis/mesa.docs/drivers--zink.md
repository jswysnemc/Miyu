...
The feature-levels implemented by Zink are exposed by [Vulkan Profiles](https://dev.vulkan.org/tools#vulkan-profiles) in the
...
or downloaded from GPUinfo.org
...
* Device extensions:
  ...
  * `VK_EXT_custom_border_color` with `customBorderColorWithoutFormat`
  * `VK_EXT_provoking_vertex`
  * `VK_EXT_line_rasterization`, with the following `VkPhysicalDeviceLineRasterizationFeaturesEXT`:
...
In addition to this, `VK_KHR_external_memory` is required to support the
DRI code-path.
...
Another useful tool for debugging is the [Vulkan Validation Layers](https://github.com/KhronosGroup/Vulkan-ValidationLayers/blob/main/README.md).
...
questions, don't hesitate to visit [#zink on OFTC](irc://irc.oftc.net/zink) and say hi!

...
1. Configure your build with the desired client APIs and enable the
...
`gles*` options after enable OpenGL ES 1.x and 2.x+. The last option enables the listed Gallium drivers.
```
...
`_EGLContext, EGLSurface`, etc. should be considered base classes
...
`haiku`
   This driver supports only the [Haiku](https://www.haiku-os.org/)

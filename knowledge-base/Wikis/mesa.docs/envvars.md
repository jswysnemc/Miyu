...
### `MESA_NO_ERROR`
if set to 1, error checking is disabled as per `GL_KHR_no_error`.
...
behavior to just set `MESA_DEBUG` to `1`:
...
-  GL versions = 3.1, depending on the driver, it may or may not have
   the `GL_ARB_compatibility` extension enabled.
...
cache DBs via `MESA_DISK_CACHE_READ_ONLY_FOZ_DBS` or
`MESA_DISK_CACHE_READ_ONLY_FOZ_DBS_DYNAMIC_LIST`. This
implementation does not support cache size limits via
`MESA_SHADER_CACHE_MAX_SIZE`. If `MESA_SHADER_CACHE_DIR`
...
disk usage.
If `MESA_SHADER_CACHE_DIR` is set, the cache will be stored in
...
if set with `MESA_DISK_CACHE_SINGLE_FILE` enabled, references
...
is shared with `MESA_DISK_CACHE_READ_ONLY_FOZ_DBS_DYNAMIC_LIST`.
...
Like `MESA_DISK_CACHE_SINGLE_FILE`, Mesa-DB reduces overall
disk usage but Mesa-DB supports cache size limits via
`MESA_SHADER_CACHE_MAX_SIZE`. If
`MESA_SHADER_CACHE_DIR` is not set, the cache will be stored
...
unlike `MESA_DISK_CACHE_READ_ONLY_FOZ_DBS`. This variable
...
`MESA_DISK_CACHE_READ_ONLY_FOZ_DBS`. A limit of 8 DBs can be
...
see SPIR-V Shader Capture
### `MESA_SPIRV_READ_PATH`
see SPIR-V Shader Replacement
...
focused (Currently X11 only) or via `MESA_VK_TRACE_FRAME` and
...
the Xlib software driver page for details.
...
> [!NOTE]
> If a device requires using `INTEL_FORCE_PROBE` to force
...
> not require `INTEL_FORCE_PROBE` for the device as soon
> as it is available.
### `INTEL_MEASURE`
...
values will be the same as can be found in the `INTEL_DEBUG`
output, and can be used for `INTEL_SHADER_ASM_READ_PATH` input.

> [!NOTE]
> Unlike the text form of shader dumping, `INTEL_DEBUG`
> does not affect on the list of shaders to dump. All generated shaders
> are always dumped if `INTEL_SHADER_BIN_DUMP_PATH` variable is
> set.
...
`GALLIUM_HUD` to `help` and run e.g. `glxgears` for more info.
### `GALLIUM_HUD_PERIOD`
...
### `RADV_CACHE_COUNTERS_BUFFER_SIZE`
...
These variable are handled by [Khronos' Vulkan loader](https://github.com/KhronosGroup/Vulkan-Loader), *not by Mesa*, but they

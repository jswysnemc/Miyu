...
**原因：** 请提供模板的第一个位置参数以概括原因。 (在[Talk:Simple stateful firewall](<../zh-cn/Talk:Simple_stateful_firewall.html>)讨论)

**翻译状态：**

  * 本文（或部分内容）译自 [Simple_stateful_firewall](<https://wiki.archlinux.org/title/Simple_stateful_firewall> "arch:Simple stateful firewall")，最近一次同步于 2015-10-18，若英文版本有所[更改](<https://wiki.archlinux.org/title/Simple_stateful_firewall?diff=0&oldid=400995>)，则您可以帮助同步与[翻译](<../zh-cn/Help:%E7%BF%BB%E8%AF%91.html> "Help:翻译")更改的内容。
  * 您可以在 [ArchWiki 的对应页面](<https://wiki.archlinux.org/title/Simple_stateful_firewall_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\)?action=history>)找到本文翻译的**原始** 修订历史。
  * 本文可能与英文原文存在出入。
...
**附注：** 部分内容未翻译（在 [Talk:Simple stateful firewall#](<../zh-cn/Talk:Simple_stateful_firewall.html>) 中讨论）

这篇文件解释了如何使用 [iptables](<../zh-cn/Iptables_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\).html> "Iptables \(简体中文\)") 建立状态防火墙。也解释了规则的含义以及为何需要这些规则，为简单起见，文章分成两个主要章节，第一章节讲解在单独一台计算机上使用 iptables，第二章节在第一章节基础上建立一个 NAT 网关。 

**警告：** 规则是按照执行的顺序给与的，如果是远程登录计算机，那么在设置规则时可能会被计算机拒之门外，所以只能在本地登录计算机的时候做下面的设置。 

[example config file](<#Resulting_iptables.rules_file>) 可以用来解决这个问题。 

##  前提条件

**注意：** Linux内核在编译时需要加入 iptables 支持，所有的常用 Arch Linux 内核都有 iptables 支持。

首先，安装用户层工具 [iptables](<https://archlinux.org/packages/?name=iptables>)包 或者确保其已被安装。 
...
否则，参阅 [Iptables (简体中文)#重置规则](<../zh-cn/Iptables_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\).html#%E9%87%8D%E7%BD%AE%E8%A7%84%E5%88%99> "Iptables \(简体中文\)")。 
...
**警告：** 如果使用 SSH 登录，之后的操作将导致丢失 SSH 链接。通过下面的步骤避免出现这个问题：（1）将下面的语句添加到第一 INPUT 链规则中（将保持界面开启）。（2）
...
**注意：** NEW but not SYN is the only invalid TCP flag not covered by the INVALID state. The reason is because they are rarely malicious packets, and they should not just be dropped. Instead, we simply do not accept them, so they are rejected with a TCP RST by the next rule.
...
**注意：** This is where you need to add rules to accept incoming connections, such as SSH, HTTP or other services that you want to access remotely.
...
See [iptables(8)](<https://man.archlinux.org/man/iptables.8>) for more advanced rules, like matching multiple ports. 

#### Port knocking

Port knocking is a method to externally open ports that, by default, the firewall keeps closed. It works by requiring connection attempts to a series of predefined closed ports. When the correct sequence of port "knocks" (connection attempts) is received, the firewall opens certain port(s) to allow a connection. See [Port knocking](</wzh/index.php?title=Port_knocking&action=edit&redlink=1> "Port knocking（页面不存在）") for more information. 

### Protection against spoofing attacks

**注意：**`rp_filter` is currently set to `1` by default in `/usr/lib/sysctl.d/50-default.conf`, so the following step is not necessary.

Blocking reserved local addresses incoming from the internet or local network is normally done through setting `rp_filter` (Reverse Path Filter) in sysctl to 1. To do so, add the following line to your `/etc/sysctl.d/90-firewall.conf` file (see [sysctl](<../zh-cn/Sysctl.html> "Sysctl") for details) to enable source address verification which is built into Linux kernel itself. The verification by the kernel will handle spoofing better than individual iptables rules for each case. 
...
**注意：** There is no reason to enable this in both places. The netfilter method is the modern choice and works with IPv6 too.
...
A 'Ping' request is an ICMP packet sent to the destination address to ensure connectivity between the devices. If your network works well, you can safely block all ping requests. It is important to note that this _does not_ actually hide your computer — any packet sent to you is rejected, so you will still show up in a simple nmap "ping scan" of an IP range. 
...
To block echo requests, add the following line to your `/etc/sysctl.d/90-firewall.conf` file (see [sysctl](<../zh-cn/Sysctl.html> "Sysctl") for details): 
...
**注意：** This opens you up to a form of [DoS](<https://en.wikipedia.org/wiki/Denial-of-service_attack> "wikipedia:Denial-of-service attack"). An attack can send packets with spoofed IPs and get them blocked from connecting to your services.
...
See the [sysctl#TCP/IP stack hardening](<../zh-cn/Sysctl.html#TCP/IP_stack_hardening> "Sysctl") for relevant kernel parameters. 
...
**提示：** For self-testing the rules after setup, the actual blacklist happening can slow the test making it difficult to fine-tune parameters. One can watch the incoming attempts via `cat /proc/net/xt_recent/sshbf`. To unblock the own IP during testing, root is needed `# echo / > /proc/net/xt_recent/sshbf`
...
and make sure your rules are loaded on boot enabling the **iptables** [daemon](<../zh-cn/Systemd.html> "Daemon"). 

Check that the rules load correctly by [starting](</wzh/index.php?title=Starting&action=edit&redlink=1> "Starting（页面不存在）") `iptables.service` and then checking the status of the service. 

### IPv6

If you do not use IPv6 (most ISPs do not support it), you should [disable it](<../zh-cn/Disabling_IPv6.html> "Disabling IPv6"). 
...
Since there is no kernel reverse path filter for IPv6, you may want to enable one in _ip6tables_ with the following: 
    
    # ip6tables -t raw -A PREROUTING -m rpfilter -j ACCEPT
    # ip6tables -t raw -A PREROUTING -j DROP
    
After the configuration is done, [enable](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Enable") the **ip6tables** service, it is meant to run in parallel to _iptables_. 
...
Then edit the relevant line in `/etc/sysctl.d/90-firewall.conf` so it persists through reboot (see [sysctl](<../zh-cn/Sysctl.html> "Sysctl") for details): 
...
and make sure your rules are loaded when you boot enabling the **iptables** [daemon](<../zh-cn/Systemd.html> "Daemon"). 
...
## See Also
  ...
  * [Methods to block SSH attacks](<https://www.webhostingtalk.com/showthread.php?t=456571>)
  * [Using iptables to block brute force attacks](<https://www.ducea.com/2006/06/28/using-iptables-to-block-brute-force-attacks/>)
  * [20 Iptables Examples For New SysAdmins](<https://linuxconfig.org/collection-of-basic-linux-firewall-iptables-rules>)[[失效链接](<https://zh.wikipedia.org/wiki/Wikipedia:%E5%A4%B1%E6%95%88%E9%93%BE%E6%8E%A5> "zhwp:Wikipedia:失效链接") 2021-11-19 ⓘ]

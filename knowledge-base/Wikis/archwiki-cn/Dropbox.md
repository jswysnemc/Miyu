...
**原因：** 请提供模板的第一个位置参数以概括原因。 (在[Talk:Dropbox](<../zh-cn/Talk:Dropbox.html>)讨论)

相关文章

  * [Backup programs](</wzh/index.php?title=Backup_programs&action=edit&redlink=1> "Backup programs（页面不存在）")

**翻译状态：**

  * 本文（或部分内容）译自 [Dropbox](<https://wiki.archlinux.org/title/Dropbox> "arch:Dropbox")，最近一次同步于 2014-12-16，若英文版本有所[更改](<https://wiki.archlinux.org/title/Dropbox?diff=0&oldid=349226>)，则您可以帮助同步与[翻译](<../zh-cn/Help:%E7%BF%BB%E8%AF%91.html> "Help:翻译")更改的内容。
  * 您可以在 [ArchWiki 的对应页面](<https://wiki.archlinux.org/title/Dropbox_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\)?action=history>)找到本文翻译的**原始** 修订历史。
  * 本文可能与英文原文存在出入。
...
**原因：** Last updated in 2014（在 [Talk:Dropbox#](<../zh-cn/Talk:Dropbox.html>) 中讨论）
...
您可以从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "Arch 用户软件仓库") 安装 [dropbox](<https://aur.archlinux.org/packages/dropbox/>)AUR 。 

**注意：** 如果 PKGBUILD 已经过时，你可以在下载后更新它，或者在 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "Arch 用户软件仓库") 页面标记其为过期。
...
**原因：** 应该尽量使用 pacman 来安装和管理软件包，或是从 PKGBUILD 建立软件包。（在 [Talk:Dropbox](<../zh-cn/Talk:Dropbox.html>) 中讨论）
...
###  可选软件包
  ...
  * 纯命令行界面，从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 安装 [dropbox-cli](<https://aur.archlinux.org/packages/dropbox-cli/>)AUR。
  * GNOME 集成，从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 安装 [nautilus-dropbox](<https://aur.archlinux.org/packages/nautilus-dropbox/>)AUR，此插件将会自动启动 Dropbox。
  * Nemo 集成，从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 安装 [nemo-dropbox-git](<https://aur.archlinux.org/packages/nemo-dropbox-git/>)AUR。
  * [Thunar](<../zh-cn/Thunar.html> "Thunar") 集成, 从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 安装 [thunar-dropbox](<https://aur.archlinux.org/packages/thunar-dropbox/>)AUR。
  * 对于 [KDE](<../zh-cn/KDE.html> "KDE") 用户，也可以在 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 中找到 KDE 下的应用：[kfilebox](<https://aur.archlinux.org/packages/kfilebox/>)AUR。

##  启动

###  自动启动 Dropbox

对于 [KDE](<../zh-cn/KDE.html> "KDE") 用户来说，在注销用户或者是重启机器后，之前正在运行的程序将会自动重新启动。[Xfce](<../zh-cn/Xfce.html> "Xfce") 稍有不同，将 `dropbox.desktop` 放置在 {ic|~/.config/autostart}} 下，Dropbox 在下次启动时就会自动启动了。 
...
If you have followed the [systemd/User](<../zh-cn/Systemd/%E7%94%A8%E6%88%B7.html> "Systemd/User") wiki page, you probably want to start dropbox only when you log in or launch your WM/DE. The solution in that case is to create a service in your home directory instead of using the sysadmin account: 
...
另外，您也可以尝试从 [AUR](<../zh-cn/Arch_%E7%94%A8%E6%88%B7%E8%BD%AF%E4%BB%B6%E4%BB%93%E5%BA%93.html> "AUR") 安装 [droxi](<https://aur.archlinux.org/packages/droxi/>)AUR ，此终端工具提供了类似于 GNU `ftp` 客户端的功能。 
...
If you want to store sensitive data in your Dropbox, you should encrypt it before. Syncing to Dropbox is encrypted, but all files are (for the time being) stored on the server unencrypted just as you put them in your Dropbox. 
  ...
  * Dropbox works with [TrueCrypt](</wzh/index.php?title=TrueCrypt&action=edit&redlink=1> "TrueCrypt（页面不存在）"), and after you initially uploaded the TrueCrypt volume to Dropbox, performance is quite okay, because Dropbox has a working binary diff.

  * Another possibility is to use [EncFS](</wzh/index.php?title=EncFS&action=edit&redlink=1> "EncFS（页面不存在）"), which has the advantage that all files are encrypted separately, i.e. you do not have to determine in advance the size of the content you want to encrypt and your encrypted directory grows and shrinks while you add/delete/modify files in it. You can also mount an encrypted volume at startup using the `-S` option of `encfs` to avoid having to input the passphrase, but note that your encrypted files are not secure from someone who has direct access to your computer.

### Setup EncFS with Dropbox

Follow the Wiki instructions to install [EncFS](</wzh/index.php?title=EncFS&action=edit&redlink=1> "EncFS（页面不存在）"). 
...
To mount your EncFS folder on every boot, follow the instructions in the [EncFS](</wzh/index.php?title=EncFS&action=edit&redlink=1> "EncFS（页面不存在）") wiki page. 
...
The basic principle and general how-to are described in the [Dropbox Wiki](<https://www.dropboxwiki.com/Multiple_Instances_On_Unix>). 
...
**注意：** When using any of these methods, you need to prevent Dropbox from doing a standard autostart by unchecking _Dropbox - Preferences - General - Start Dropbox on system startup_. This prevents Dropbox from creating the `~/.config/autostart/dropbox.desktop` file and thus from starting twice.

### Using netctl

For [netctl](<../zh-cn/Netctl.html> "Netctl"), use `ExecUpPost` and `ExecDownPre` respectively in every network profile you use, or for example in `/etc/netctl/interfaces/wlan0` to start Dropbox automatically whenever profile on `wlan0` is active. Add '|| true' to your command to make sure [netctl](<../zh-cn/Netctl.html> "Netctl") will bring up your profile, although Dropbox fails to start. 
...
If you have connectivity problem with [NetworkManager](<../zh-cn/NetworkManager.html> "NetworkManager"), try using a [dispatcher script](<../zh-cn/NetworkManager.html#Network_services_with_NetworkManager_dispatcher> "NetworkManager"): [networkmanager-dispatcher-dropbox](<https://aur.archlinux.org/packages/networkmanager-dispatcher-dropbox/>)AUR[[损坏的链接](<../zh-cn/Help:%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B.html#%E4%BF%AE%E5%A4%8D%E6%8D%9F%E5%9D%8F%E7%9A%84%E5%8C%85%E9%93%BE%E6%8E%A5> "Help:操作流程")：package not found] or [networkmanager-dispatcher-dropbox-systemd](<https://aur.archlinux.org/packages/networkmanager-dispatcher-dropbox-systemd/>)AUR[[损坏的链接](<../zh-cn/Help:%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B.html#%E4%BF%AE%E5%A4%8D%E6%8D%9F%E5%9D%8F%E7%9A%84%E5%8C%85%E9%93%BE%E6%8E%A5> "Help:操作流程")：package not found]. 
...
**注意：** If you use PCManFM as your file manager, Dropbox will use 'xdg-open' calls pcmanfm to open the Dropbox folder.However, without a dbus session, you can not use Trash in PCManFM. You should refer to [Dbus](<../zh-cn/D-Bus.html> "Dbus") and [General troubleshooting#Session permissions](<../zh-cn/General_troubleshooting.html#Session_permissions> "General troubleshooting")to edit your ~/.xinitrc based on /etc/skel/.xinitrc to start a D-Bus session before your launch any other program in ~/.xinitrc. Do use 'dbus-launch dropboxd' instead of just 'dropboxd' in wicd postconnect script. otherwise pcmanfm launched by clicking dropbox icon can not use the Trash.
...
**注意：** Dropbox 仅支持此形式的 `http://your.proxy.here:port` 代理表示方法， 此表示方法不被支持 `your.proxy.here:port`（大部分程序都不支持此方法） 。

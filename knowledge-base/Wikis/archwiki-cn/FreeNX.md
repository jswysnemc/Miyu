...
**原因：** 请提供模板的第一个位置参数以概括原因。 (在[Talk:FreeNX](<../zh-cn/Talk:FreeNX.html>)讨论)
...
**附注：** 大量英文段落（在 [Talk:FreeNX#](<../zh-cn/Talk:FreeNX.html>) 中讨论）

"NX 是一项令人激动的远程控制技术。它提供了在高延迟、低带宽环境中**接近本地速度** 的应用程序响应链接。NX 的核心库由 [NoMachine](<https://www.nomachine.com/>) 在 GPL 授权下提供。**FreeNX** 是一个 NX 服务器和客户端组件的 GPL 实现。"
...
如果你打算用 freenx 来连接无头 PC，记得你得有一个配置好的 X 服务器，这样你应该就能安装任何与 [xorg](<../zh-cn/Xorg.html> "Xorg") 相关联的包了。 
...
如果你的 ssh 守护进程不是运行在默认的 22 端口，你需要取消注释并修改：
...
**注意：** The variable `$VNC_PORT` in the following script defines the X display, which is configured as `display :0` under `$HOME/.x11vncrc`, `590**0**` is the root session, if you want to use `display :1` use the port `590**1**` and so on
...
You are able to connect to your current X session via NX client now. 
    ...
    — [FreeNX to existing display (opensuse.org)](<https://en.opensuse.org/SDB:FreeNX_to_existing_display>)
...
If you do not want to do this, you can start X by [installing a login manager like SLIM or XDM](<../zh-cn/Display_manager.html> "Display manager"). I would recomend using SLiM because of its small size. 
...
This problem can be solved by running a login manager- The problem is that X11 is not started, and it appears that "startx" or similar do not work from the freenx client. Follow these instructions to setup a login manager and load it at startup: [Display manager](<../zh-cn/Display_manager.html> "Display manager")

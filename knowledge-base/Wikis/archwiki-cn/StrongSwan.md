**翻译状态：**
  ...
  * 本文（或部分内容）译自 [StrongSwan](<https://wiki.archlinux.org/title/StrongSwan> "arch:StrongSwan")，最近一次同步于 2016-06-07，若英文版本有所[更改](<https://wiki.archlinux.org/title/StrongSwan?diff=0&oldid=437545>)，则您可以帮助同步与[翻译](<../zh-cn/Help:%E7%BF%BB%E8%AF%91.html> "Help:翻译")更改的内容。
  * 您可以在 [ArchWiki 的对应页面](<https://wiki.archlinux.org/title/StrongSwan_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\)?action=history>)找到本文翻译的**原始** 修订历史。
  * 本文可能与英文原文存在出入。

IPSec 是一个加密和认证标准，可以组建安全虚拟专用网路(VPN). 

Linux kernel 已经原生支持这个标准，但是用户需要配置加密密钥。IPSec VPN 使用 [IKE](<https://en.wikipedia.org/wiki/Internet_Key_Exchange> "wikipedia:Internet Key Exchange") 协议自动协商密钥的交换，交换方式包括认证、提前共享或同时使用。 
...
安装软件包 [strongswan](<https://archlinux.org/packages/?name=strongswan>)包。 
...
客户端需要个人证书才能使用 VPN. 和生成主机证书类似，只是这里用客户端的电子邮件而不是主机名: 
...
**原因：** Need to better explain the differences, advantages and inconvenients, then write a tutorial. (在 [Talk:StrongSwan](<../zh-cn/Talk:StrongSwan.html>) 中讨论)

###  IPSec/L2TP

[L2TP/IPsec VPN client setup](</wzh/index.php?title=L2TP/IPsec_VPN_client_setup&action=edit&redlink=1> "L2TP/IPsec VPN client setup（页面不存在）") 页面包含了如何建立一个连接到 IPSec/L2TP 服务器的客户端。这种 IPSec VPN 和纯 IPSec 相比，可以将隧道用于非 IP 包，缺点是需要运行一个额外的 L2TP 后台进程。 
...
最后，[start](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Start") 并 [enable](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Enable") `strongswan` 服务。 

###  在容器中运行 Strongswan

要在容器中运行 `strongswan`，比如在 [systemd-nspawn](<../zh-cn/Systemd-nspawn.html> "Systemd-nspawn") 中，需要类似下面的服务文件： 
...
**原因：** Need to explain at least ip xfrm and common issues (在 [Talk:StrongSwan](<../zh-cn/Talk:StrongSwan.html>) 中讨论)

**翻译状态：**
  ...
  * 本文（或部分内容）译自 [PCI passthrough via OVMF](<https://wiki.archlinux.org/title/PCI_passthrough_via_OVMF> "arch:PCI passthrough via OVMF")，最近一次同步于 2018-08-03，若英文版本有所[更改](<https://wiki.archlinux.org/title/PCI_passthrough_via_OVMF?diff=0&oldid=532316>)，则您可以帮助同步与[翻译](<../zh-cn/Help:%E7%BF%BB%E8%AF%91.html> "Help:翻译")更改的内容。
  * 您可以在 [ArchWiki 的对应页面](<https://wiki.archlinux.org/title/PCI_passthrough_via_OVMF_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\)?action=history>)找到本文翻译的**原始** 修订历史。
  * 本文可能与英文原文存在出入。

开放虚拟机固件（Open Virtual Machine Firmware, [OVMF](<https://github.com/tianocore/tianocore.github.io/wiki/OVMF>))是一个为虚拟机启用 UEFI 支持的项目。Linux 内核 3.9 之后的版本 和近期版本的 [QEMU](<../zh-cn/QEMU.html> "QEMU") ，支持将显卡直通给虚拟机，让虚拟机在执行图形密集型任务的时候有接近实体机的图形性能。 
...
设置[内核参数](<../zh-cn/%E5%86%85%E6%A0%B8%E5%8F%82%E6%95%B0.html> "内核参数")以启用 IOMMU，注意不同品牌的 CPU 所需的内核参数并不同。 

  * 对于 Intel CPU(VT-d)，使用 `intel_iommu=on`。因为内核参数CONFIG_INTEL_IOMMU_DEFAULT_ON在[linux](<https://archlinux.org/packages/?name=linux>)包中并未启用。
  * 对于 AMD CPU(AMD-Vi)，当内核检测到硬件支持IOMMU时就会自动启用。
...
**注意：**
  ...
  * 如果宿主机和客户机所使用的GPU设备ID相同（例如是同一型号）那么就不能使用供应商-设备ID来指定需要隔离的设备。如果是这种情况，参见 [#宿主机和客户机使用相同的GPU](<#%E5%AE%BF%E4%B8%BB%E6%9C%BA%E5%92%8C%E5%AE%A2%E6%88%B7%E6%9C%BA%E4%BD%BF%E7%94%A8%E7%9B%B8%E5%90%8C%E7%9A%84GPU>)。
  * 音频设备（例如上面的10de:0fbb）不是必须绑定的，Libvrt能够自动将其从`snd_hda_intel`解绑

有两种方法可以实现设备ID与vfio-pci绑定。 

方法一：在内核启动参数中进行绑定。 

通过内核启动参数进行绑定，能够方便地编辑、删除、撤销这类具有破坏性的配置。参考[内核参数](<../zh-cn/%E5%86%85%E6%A0%B8%E5%8F%82%E6%95%B0.html> "内核参数")加入以下命令： 
    
    vfio-pci.ids=10de:13c2,10de:0fbb
    
**注意：**

  * 如果你使用nvidia驱动，并且对其进行了modesetting，则`vfio-pci.ids`必须通第二种方式进行绑定。第一种方法会导致参数生效过晚从而绑定失败。

方法二：通过载入模块进行绑定。 

您需要在`/etc/modprobe.d/`创建`vfio.conf`，并将编辑内容如下： 
    
    /etc/modprobe.d/vfio.conf
    
    options vfio-pci ids=10de:13c2,10de:0fbb

更改过后需要[重新生成initramfs](<../zh-cn/Regenerate_the_initramfs.html> "Regenerate the initramfs")

**注意：**

  * 像上面提到过的一样，[#如果客户机所用显卡插在 CPU 提供的 PCI-E 插槽中](<#%E5%A6%82%E6%9E%9C%E5%AE%A2%E6%88%B7%E6%9C%BA%E6%89%80%E7%94%A8%E6%98%BE%E5%8D%A1%E6%8F%92%E5%9C%A8_CPU_%E6%8F%90%E4%BE%9B%E7%9A%84_PCI-E_%E6%8F%92%E6%A7%BD%E4%B8%AD>)，并且 PCI 根设备是 IOMMU 组的一部分，**不** 要将根设备的 ID 传递给`vfio-pci`，因为他需要连接到宿主机以保持正常运行。但是，该组中的任何其它设备都应该绑定到`vfio-pci`。

###  提前加载vfio-pci

#### mkinitcpio

因为Arch的[linux](<https://archlinux.org/packages/?name=linux>)包内核将vfio-pci作为一个模块，我们需要强迫vfio-pci在显卡驱动抢占硬件之前加载。为了达成这一目的，我们向[mkinitcpio](<../zh-cn/Mkinitcpio.html> "Mkinitcpio")中添加 `vfio_pci`, `vfio`, `vfio_iommu_type1`, `vfio_virqfd`： 

**注意：**

  * 从内核 6.2 版本开始，vfio_virqfd 的功能已经并入 vfio 模块
...
**注意：**
  ...
  * 如果你在其中加载了其他驱动来进行modesetting（例如`nouveau`, `radeon`, `amdgpu`, `i915`等）则上述vfio模组必须在他们之前加载
...
由于新模块已添加到 initramfs 配置中，因此必须[重新生成initramfs](<../zh-cn/Regenerate_the_initramfs.html> "Regenerate the initramfs")。若在`/etc/modprobe.d/vfio.conf`修改了设备ID，您同样需要重新生成它。之后重启系统。 

#### booster

与mkinitcpio类似，你需要自定义模组来提前加载： 
    
    /etc/booster.yaml
    
    modules_force_load: vfio_pci,vfio,vfio_iommu_type1,vfio_virqfd

之后[重新生成initramfs](<../zh-cn/Regenerate_the_initramfs.html> "Regenerate the initramfs")

#### dracut

与上面的思路类似， 在 `/etc/dracut.conf.d`添加以下文件： 
    
    10-vfio.conf
    
    force_drivers+=" vfio_pci vfio vfio_iommu_type1 vfio_virqfd "

注意到我们使用`force_drivers` 而不是常规的 `add_drivers` 选项, 这能保证驱动程序能通过modprobe提前加载 ([Dracut#Early kernel module loading](<../zh-cn/Dracut.html#Early_kernel_module_loading> "Dracut")). 

与mkinitcpio类似的，你需要重新生成initramfs. 详见[dracut](<../zh-cn/Dracut.html> "Dracut")。 

###  验证`vfio-pci`是否已经正确加载并绑定到正确的设备
...
[Libvirt](<../zh-cn/Libvirt.html> "Libvirt") 是许多虚拟化实用程序的包装，可以极大的简化虚拟机的配置和部署过程。对于 KVM 和 QEMU ，它提供的前端让我们避免处理 QEMU 的权限，并使得在现有的虚拟机上添加和删除设备更加容易。但是，作为一个包装，它可能并不总是支持最新的 QEMU 功能，最终可能需要一些提供的脚本为 QEMU 传递一些额外的参数。 

安装[qemu](<https://archlinux.org/packages/?name=qemu>)包[[损坏的链接](<../zh-cn/Help:%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B.html#%E4%BF%AE%E5%A4%8D%E6%8D%9F%E5%9D%8F%E7%9A%84%E5%8C%85%E9%93%BE%E6%8E%A5> "Help:操作流程")：package not found]、[libvirt](<https://archlinux.org/packages/?name=libvirt>)包、[ovmf](<https://archlinux.org/packages/?name=ovmf>)包[[损坏的链接](<../zh-cn/Help:%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B.html#%E4%BF%AE%E5%A4%8D%E6%8D%9F%E5%9D%8F%E7%9A%84%E5%8C%85%E9%93%BE%E6%8E%A5> "Help:操作流程")：replaced by [edk2-ovmf](<https://archlinux.org/packages/?name=edk2-ovmf>)包]和[virt-manager](<https://archlinux.org/packages/?name=virt-manager>)包之后，将您的 OVMF 固件映像和运行时变量模板添加到 libvirt 配置中，以便让 `virt-install`或是`virt-manager` 可以找到它们。 
...
您现在可以[启用](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Enable")并[启动](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Start")`libvirtd.service`和它的日志记录组件`virtlogd.socket`。 

###  安装客户机系统

使用 `virt-manager` 配置虚拟机的大部分过程都无需指导，只要按照屏幕上的提示即可。 

如果使用`virt-manager`，则必须将用户添加到 `libvirt` [组](<../zh-cn/User_group.html> "User group")。 

但是，您仍应该特别注意如下步骤： 

  * 在虚拟机创建向导要求您命名虚拟机时（点击“完成”前的最后一步），勾选“在安装前自定义配置”。
  * 在“概况”屏幕，将“固件”选为["UEFI"](<https://i.imgur.com/73r2ctM.png>)，如果您不能选择它，检查是否在`/etc/libvirt/qemu.conf`正确配置了固件的路径并且[重新启动](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Restart")了`libvirtd.service`。
  ...
  * 如果要最小化IO开销，请点击“添加硬件”，并在“控制器：中选择“SCSI”类型，型号为 "VirtIO SCSI"。你可以将默认的 IDE 磁盘改为 SCSI 磁盘，并绑定到上述的控制器。 
    * Windows 不包含VirtIO驱动程序，所以你需要从[这里](<https://fedorapeople.org/groups/virt/virtio-win/direct-downloads/latest-virtio/>)下载包含驱动程序的 ISO 并且添加一个IDE CDROM（Windows 7之后可以使用SATA）并且连接到刚才的 ISO 。否则在安装过程中 Windows 无法识别 VirtIO 控制器。当 Windows 安装程序要求您选择要安装的磁盘时，加载 CD-ROM 下 visscsi 目录下的驱动程序。
...
To allocate huge pages at boot, one must simply specify the desired amount on their kernel command line with `hugepages=_x_`. For instance, reserving 1024 pages with `hugepages=1024` and the default size of 2048 KiB per huge page creates 2 GiB worth of memory for the virtual machine to use. 
...
**原因：** Need futher testing if this variant as effective as static one（在 [Talk:使用 OVMF 直通 PCI](<../zh-cn/Talk:%E4%BD%BF%E7%94%A8_OVMF_%E7%9B%B4%E9%80%9A_PCI.html>) 中讨论）

Hugepages could be allocated manually via `vm.nr_overcommit_hugepages` [sysctl](<../zh-cn/Sysctl.html> "Sysctl") parameter. 
    
    /etc/sysctl.d/10-kvm.conf
    
    vm.nr_hugepages = 0
    vm.nr_overcommit_hugepages = _num_

Where `_num_` \- is the number of huge pages, which default size if 2 MiB. Pages will be automatically allocated, and freed after VM stops. 

More manual way: 
    
    # echo _num_ > /sys/kernel/mm/hugepages/hugepages-2048kB/nr_hugepages
    # echo _num_ > /sys/kernel/mm/hugepages/hugepages-1048576kB/nr_hugepages
...
根据您[CPU 调速器](<../zh-cn/CPU_frequency_scaling.html> "CPU frequency scaling")的配置方式，虚拟机的线程可能无法达到负载阈值以提升频率。实际上，KVM 自身无法更改 CPU 频率，如果 CPU 频率并不能随着虚拟 CPU 的负载使用而提升，这可能是一个严重的性能问题。检查此问题最简单的方法是在虚拟机上运行 CPU 密集型任务的同时在宿主机上运行`watch lscpu`检查CPU频率是否上升。如果您的 CPU 频率无法正常上升且无法达到所报告的最大值，可能是由于[主机操作系统限制了 CPU 升频](<https://lime-technology.com/forum/index.php?topic=46664.msg447678#msg447678>)。这种情况下，尝试手动将所有CPU内核设为最大频率查看是否能够提升性能。请注意，如果您使用带有默认pstate驱动程序的现代Intel芯片，则cpupower命令[无效](<../zh-cn/CPU_frequency_scaling.html#CPU_frequency_driver> "CPU frequency scaling")[[损坏的链接](<../Project:%E7%A4%BE%E7%BE%A4%E9%A6%96%E9%A1%B5.html#%E6%8D%9F%E5%9D%8F%E7%9A%84%E9%93%BE%E6%8E%A5> "Project:社群首页")：无效的章节]，因此请监视`/proc/cpuinfo`确保您的CPU处于最大频率。 

###  高DPC延迟

[![](../File:Tango-inaccurate.png)](<../File:Tango-inaccurate.png>)**本文或本章节的事实准确性存在争议。**

**原因：** As far as I can tell all virtio modules listed here are for virtual devices used when Linux runs as a guest. Loading them on the host serves no purpose.（在 [Talk:使用 OVMF 直通 PCI](<../zh-cn/Talk:%E4%BD%BF%E7%94%A8_OVMF_%E7%9B%B4%E9%80%9A_PCI.html>) 中讨论）

如果您在虚拟中遇到了高DPC和/或中断延迟，请确保已在主机内核上[加载](<../zh-cn/Kernel_modules.html#Manual_module_handling> "Kernel modules")了所需的VirtIO内核模块，可加载的VirtIO内核模块包括：`virtio-pci`，`virtio-net`，`virtio-blk`，`virtio-balloon`，`virtio-ring`和`virtio`。 
...
有一个[内核补丁](<https://patchwork.kernel.org/patch/10027525/>)可以解决这个问题，它已经在4.14-stable和4.9-stable 版本被接受了。 如果你正在运行官方的[linux](<https://archlinux.org/packages/?name=linux>)包或[linux-lts](<https://archlinux.org/packages/?name=linux-lts>)包内核，那么该补丁被已经应用了（请确保你使用的是最新内核）。如果您正在运行另一个内核，您可能需要手动应用补丁。 
...
**原因：** A number of users have been having issues with this, it should probably be adressed by the article. (在 [Talk:PCI passthrough via OVMF#Additionnal sections](</wzh/index.php?title=Talk:PCI_passthrough_via_OVMF&action=edit&redlink=1> "Talk:PCI passthrough via OVMF（页面不存在）") 中讨论)
...
[重新生成initramfs](<../zh-cn/Regenerate_the_initramfs.html> "Regenerate the initramfs")并重新启动。 

###  直通启动时所用的GPU

[![](../File:Tango-view-fullscreen.png)](<../File:Tango-view-fullscreen.png>)**这篇文章的某些内容需要扩充。**

**原因：** This is related to VBIOS issues and should be moved into a separate section regarding VBIOS compatibility. (在 [Talk:使用 OVMF 直通 PCI#UEFI (OVMF) Compatibility in VBIOS](</wzh/index.php?title=Talk:%E4%BD%BF%E7%94%A8_OVMF_%E7%9B%B4%E9%80%9A_PCI&action=edit&redlink=1> "Talk:使用 OVMF 直通 PCI（页面不存在）") 中讨论)
...
记得为脚本授予[可执行](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%B7%BB%E5%8A%A0%E5%8F%AF%E6%89%A7%E8%A1%8C%E6%9D%83%E9%99%90> "Executable")权限。 

创建一个[systemd](<../zh-cn/Systemd.html> "Systemd")单元文件，以便在开机时创建这个文件。 
...
[启动](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Start")并[启用](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Enable") `looking-glass-init.service`。 
...
Looking glass 客户端可以通过 [looking-glass](<https://aur.archlinux.org/packages/looking-glass/>)AUR或[looking-glass-git](<https://aur.archlinux.org/packages/looking-glass-git/>)AUR 安装。 

**警告：** 如果你在使用[looking-glass-git](<https://aur.archlinux.org/packages/looking-glass-git/>)AUR时出现问题，请选择[looking-glass](<https://aur.archlinux.org/packages/looking-glass/>)AUR。git版本并不受上游支持，可能在任何时候破损。只有带有版本号的发布受到支持，并且保证能与Windows客户机上的主机程序一起使用。
...
现在可以执行这2个脚本，以将USB设备附加/分离到客户机。注意它们可能需要以root身份执行。要从Windows客户机运行脚本，您可以使用[PuTTY](</wzh/index.php?title=PuTTY&action=edit&redlink=1> "PuTTY（页面不存在）") [SSH](<../zh-cn/%E5%AE%89%E5%85%A8%E5%A4%96%E5%A3%B3%E5%8D%8F%E8%AE%AE.html> "SSH")连接到主机，然后执行脚本。在Windows上，PuTTY附带了plink.exe，它可以通过SSH执行单一命令，然后自动关闭，而不是打开SSH终端。 
    
    detach_devices.bat
    
    "C:\Program Files\PuTTY\plink.exe" root@$HOST_IP -pw $ROOTPASSWORD /home/$USER/.VFIOinput/input_detach.sh

用宿主机[IP 地址](<../zh-cn/%E7%BD%91%E7%BB%9C%E9%85%8D%E7%BD%AE.html#IP_addresses> "网络配置")替换`$HOST_IP`，用root密码替换`$ROOTPASSWORD`。 

**警告：** 如果有人可以访问您的虚拟机，则此方法并不安全，因为他们可以打开文件并读取您的ROOT密码。建议使用[SSH keys](<../zh-cn/SSH_keys.html> "SSH keys")！

您可能还希望使用快捷键来执行脚本文件。在Windows上，您可以使用[Autohotkey](<https://autohotkey.com/>)，在宿主机上可以使用[Xbindkeys](</wzh/index.php?title=Xbindkeys&action=edit&redlink=1> "Xbindkeys（页面不存在）")，您的桌面环境也可能提供快捷键设置。 由于需要以root身份运行脚本，您可能还需要使用[Polkit](<../zh-cn/Polkit.html> "Polkit")，从而无需输入密码。 

**提示：**

  * （译注）如果您配置了Polkit无密码授权，那么这个脚本也可以使用您自己的用户来执行。参见[Libvirt#设置授权](<../zh-cn/Libvirt.html#%E8%AE%BE%E7%BD%AE%E6%8E%88%E6%9D%83> "Libvirt")和[Polkit#跳过口令提示](<../zh-cn/Polkit.html#%E8%B7%B3%E8%BF%87%E5%8F%A3%E4%BB%A4%E6%8F%90%E7%A4%BA> "Polkit").
  * （译注）您也可以使用Windows 10 自带的 OpenSSH，使用方法和Linux下相同，参见[OpenSSH](<../zh-cn/OpenSSH.html> "OpenSSH")。
...
您需要一个应用了这个补丁的内核，最简单的办法就是安装[linux-vfio](<https://aur.archlinux.org/packages/linux-vfio/>)AUR包。 
...
安装和配置后，设置[内核参数](<../zh-cn/%E5%86%85%E6%A0%B8%E5%8F%82%E6%95%B0.html> "内核参数")以在启用`pcie_acs_override=`选项的情况下加载新内核。 

##  仅使用QEMU不使用libvirt

如果不想在libvirt的帮助下设置虚拟机，可以使用具有自定义参数的普通QEMU命令来运行要使用PCI直通的虚拟机，这对于脚本等一些用例来说是十分理想的，可以灵活地搭配其他脚本。 

先完成[#设置IOMMU](<#%E8%AE%BE%E7%BD%AEIOMMU>)和[#隔离GPU](<#%E9%9A%94%E7%A6%BBGPU>)，接着配置好[QEMU](<../zh-cn/QEMU.html> "QEMU")虚拟化环境,[启用KVM](<../zh-cn/QEMU.html#Enabling_KVM> "QEMU")并使用`-device vfio-pci,host=07:00.0`选项，将`07:00.0`替换为您先前隔离的设备ID。 

要使用OVMF固件，确保已安装[ovmf](<https://archlinux.org/packages/?name=ovmf>)包[[损坏的链接](<../zh-cn/Help:%E6%93%8D%E4%BD%9C%E6%B5%81%E7%A8%8B.html#%E4%BF%AE%E5%A4%8D%E6%8D%9F%E5%9D%8F%E7%9A%84%E5%8C%85%E9%93%BE%E6%8E%A5> "Help:操作流程")：replaced by [edk2-ovmf](<https://archlinux.org/packages/?name=edk2-ovmf>)包]，将UEFI固件从`/usr/share/ovmf/x64/OVMF_VARS.fd`复制到临时位置，如`/tmp/MY_VARS.fd`。最后指定OVMF路径。使用如下的参数（注意顺序）： 
...
建议通过使用[virtio驱动程序](<../zh-cn/QEMU.html#Installing_virtio_drivers> "QEMU")并进一步配置，以增强性能,参阅[QEMU](<../zh-cn/QEMU.html> "QEMU")。 
...
**注意：** 如果您的USB控制器不支持Reset，不在单独的组中，或者无法直通，您仍然可以通过[udev](<../zh-cn/Udev.html> "Udev")规则达到类似的目的。参见[[1]](<https://github.com/olavmrk/usb-libvirt-hotplug>)，它允许连接到指定USB端口的任何设备自动连接到虚拟机。

###  使用PulseAudio将虚拟机音频传递给宿主机

可以使用libvirt将虚拟机的音频作为应用程序传输到宿主机。 这具有以下优点：多个音频流可传输到一个主机输出，并且与无需考虑音频设备是否支持直通。[PulseAudio](<../zh-cn/PulseAudio.html> "PulseAudio") 是必需的。 
...
你同样需要告诉QEMU使用PulseAudio后端并识别要连接的服务器，首先将QEMU命名空间添加到你的域。 编辑域的`.xml`文件（使用`virsh edit domain`），将`domain type`行改为： 
    ...
    <domain type='kvm' xmlns:qemu='<http://libvirt.org/schemas/domain/qemu/1.0'>[[失效链接](<https://zh.wikipedia.org/wiki/Wikipedia:%E5%A4%B1%E6%95%88%E9%93%BE%E6%8E%A5> "zhwp:Wikipedia:失效链接") 2020-08-04 ⓘ]>
...
[重启](<../zh-cn/Help:%E9%98%85%E8%AF%BB.html#%E6%8E%A7%E5%88%B6_systemd_%E5%8D%95%E5%85%83> "Restart") `libvirtd.service` 和[用户服务](<../zh-cn/Systemd/%E7%94%A8%E6%88%B7.html> "Systemd/用户") `pulseaudio.service`。 

现在，虚拟机音频将作为应用程序传递给宿主机。[pavucontrol](<https://archlinux.org/packages/?name=pavucontrol>)包可用于控制输出设备。请注意，在Windows客户机上，[不使用消息信号中断可能会导致出现噼啪声](<#HDMI%E9%9F%B3%E9%A2%91%E4%B8%8D%E5%90%8C%E6%AD%A5%E6%88%96%E6%98%AF%E6%9C%89%E5%99%BC%E5%95%AA%E5%A3%B0>)。 
...
如果您在配置的时候遇到了麻烦，您可以参考[完整的配置示例](</wzh/index.php?title=PCI_passthrough_via_OVMF/Examples&action=edit&redlink=1> "PCI passthrough via OVMF/Examples（页面不存在）")。一些用户在那里描述了它们的配置步骤，或许您可以从中发现一些技巧。 

##  疑难解答

如果您遇到的问题并没有在下面列出，您也可以参考[QEMU#Troubleshooting](<../zh-cn/QEMU.html#Troubleshooting> "QEMU")。 
...
**原因：** This error is actually related to the boot_vgs issue and should be merged together with everything else concerning GPU ROMs. (在 [Talk:使用 OVMF 直通 PCI#UEFI (OVMF) Compatibility in VBIOS](</wzh/index.php?title=Talk:%E4%BD%BF%E7%94%A8_OVMF_%E7%9B%B4%E9%80%9A_PCI&action=edit&redlink=1> "Talk:使用 OVMF 直通 PCI（页面不存在）") 中讨论)
...
**注意：** 可能需要同时设置[内核参数](<../zh-cn/%E5%86%85%E6%A0%B8%E5%8F%82%E6%95%B0.html> "内核参数") `video=efifb:off`。 [[2]](<https://pve.proxmox.com/wiki/Pci_passthrough#BAR_3:_can.27t_reserve_.5Bmem.5D_error>)
...
One should compare VBIOS versions between host and guest systems using [nvflash](<https://www.techpowerup.com/download/nvidia-nvflash/>) (Linux versions under _Show more versions_) or [GPU-Z](<https://www.techpowerup.com/download/techpowerup-gpu-z/>) (in Windows guest). To check the currently loaded VBIOS: 
...
如果启用 `intel_iommu`后，Intel GPU的HDMI输出设备在宿主机上无法使用，那么设置选项`igfx_off`（即`intel_iommu=on,igfx_off`）可能会恢复音频，有关设置`igfx_off`的详细信息，请阅读[iommu.html](<https://docs.kernel.org/x86/iommu.html#graphics-problems>)。 
...
这可以通过[告诉Chromium使用哪个GPU](<../zh-cn/Chromium/Tips_and_tricks.html#Forcing_specific_GPU> "Chromium/Tips and tricks")来解决。 
...
如果仍然不能工作，确保你的用户已经加入了kvm和libvirt[组](<../zh-cn/User_group.html> "User group")。 

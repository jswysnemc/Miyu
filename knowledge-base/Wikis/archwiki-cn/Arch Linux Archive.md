**翻译状态：**
  ...
  * 本文（或部分内容）译自 [Arch_Linux_Archive](<https://wiki.archlinux.org/title/Arch_Linux_Archive> "arch:Arch Linux Archive")，最近一次同步于 2022-11-24，若英文版本有所[更改](<https://wiki.archlinux.org/title/Arch_Linux_Archive?diff=0&oldid=757648>)，则您可以帮助同步与[翻译](<../zh-cn/Help:%E7%BF%BB%E8%AF%91.html> "Help:翻译")更改的内容。
  * 您可以在 [ArchWiki 的对应页面](<https://wiki.archlinux.org/title/Arch_Linux_Archive_\(%E7%AE%80%E4%BD%93%E4%B8%AD%E6%96%87\)?action=history>)找到本文翻译的**原始** 修订历史。
  * 本文可能与英文原文存在出入。
...
Arch Linux 存档库（**A** rch **L** inux **Archive** ， _简称 ala_ ），以前称为 _Arch Linux 回滚机（Arch Linux Rollback Machine_ ， _简称 ARM_ ），保存了 _官方仓库快照_ 、 _iso 镜像_ 和 _引导程序包_ 的历史版本。 
...
如果报 PGP 签名错误，软件包损坏（corrupted/invalid packages），请先尝试更新 [archlinux-keyring](<https://archlinux.org/packages/?name=archlinux-keyring>)包 和 [ca-certificates](<https://archlinux.org/packages/?name=ca-certificates>)包。或者，您可以决定临时完全[禁用签名检查](<../zh-cn/Pacman/Package_signing.html#Disabling_signature_checking> "Pacman/Package signing")。 
...
通过 [archive.org Python 客户端](<https://github.com/jjjake/internetarchive>)也能进行下载。 
...
##  历史
  ...
  * 最早的 ARM （ _Archlinux 回滚机_ ） 已于 2013-08-18 关闭[[1]](<https://bbs.archlinux.org/viewtopic.php?pid=1313360#p1313360>)。
  ...
  * 2015-12-19 迁移至 [archive.archlinux.org](<https://archive.archlinux.org>)。[[2]](<https://lists.archlinux.org/archives/list/arch-dev-public@lists.archlinux.org/message/WFG4Z3CIHOYSEGRP6PRD2WGW2YJB3TFN/>)
  * 从 2018-06-05 开始 2013-2016 的老软件包上传到了 [archive.org](<https://archive.org/details/archlinuxarchive>)。

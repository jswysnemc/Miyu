...
Specifies whether the file pseudo-device can be used for new printer queues. The URI "file:///dev/null" is always allowed. File devices cannot be used with "raw" print queues - a PPD file is required. The specified file must already exist and is opened for writing in exclusive mode. Creating new files or writing to directories is not supported. Specifying a value of "Yes" allows access to TTY devices of the form "/dev/ttySOMETHING". Specifying a regular expression allows access to TTY devices whose paths match the specified regular expression.

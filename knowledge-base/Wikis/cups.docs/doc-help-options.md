...
CUPS provides both the System V ([lp(1)](man-lp.html)) and Berkeley ([lpr(1)](man-lpr.html)) printing commands for printing files. In addition, it supported a large number of standard and printer-specific options that allow you to control how and where files are printed.
...
Many systems will have more than one printer available to the user. These printers can be attached to the local system via a parallel, serial, or USB port, or available over the network. Use the [lpstat(1)](man-lpstat.html) command to see a list of available printers:
...
If you normally use a particular printer, you can tell CUPS to use it by default using the [lpoptions(1)](man-lpoptions.html) command:
...
The [cancel(1)](man-cancel.html) and [lprm(1)](man-lprm.html) commands cancel a print job:
...
The *job-id* is the number that was reported to you by the **lp** command. You can also get the job ID using the [lpq(1)](man-lpq.html) or [lpstat](man-lpstat.html) commands:
...
The [lpmove(8)](man-lpmove.html) command moves a print job to a new printer or class:

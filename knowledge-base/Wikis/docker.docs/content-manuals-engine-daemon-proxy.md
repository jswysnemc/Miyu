# Content / Manuals / Engine / Daemon / Proxy
...
> `$HOME/.config/systemd/<user>/docker.service.d/`. In addition, `systemctl` must
...
2. Create a file named `$HOME/.config/systemd/user/docker.service.d/http-proxy.conf`
   that adds the `HTTP_PROXY` environment variable:

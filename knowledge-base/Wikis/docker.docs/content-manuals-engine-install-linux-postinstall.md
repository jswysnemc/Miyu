# Content / Manuals / Engine / Install / Linux Postinstall
...
4. Verify that you can run `docker` commands without `sudo`.
   ...
   This error indicates that the permission settings for the `$HOME/.docker/`
   directory are incorrect, due to having used the `sudo` command earlier.

   To fix this problem, either remove the `$HOME/.docker/` directory (it's recreated

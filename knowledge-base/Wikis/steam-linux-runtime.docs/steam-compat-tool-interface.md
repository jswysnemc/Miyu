...
downloaded out-of-band and stored in `$HOME/.steam/root/appcache/appinfo.vdf`,
...
such as `$HOME/.steam/root/compatibilitytools.d`.

Each subdirectory of `compatibilitytools.d` that contains `compatibilitytool.vdf` is assumed to be a compatibility tool. The
...
is a [VDF][] text file with one top-level entry, `manifest`.`manifest` has the following known fields:
...
* `compatmanager_layer_name`:
    ...
    * `container-runtime`:
        ...
        "[Steam Linux Runtime 2.0 (soldier)][soldier]" (app ID 1391110),
        "[Steam Linux Runtime 3.0 (sniper)][sniper]" (app ID 1628350) or
        "[Steam Linux Runtime 4.0][steamrt4]" (app ID 4183110),
        or their arm64 equivalents.
...
`$HOME/.local/share/Steam/steamapps/common/MyCompatTool/run`.
...
In a version 2 manifest, the special token `%verb%` can appear in the `commandline`. It is replaced by either `run` or `waitforexitandrun`.
...
named `Inner` requires a compat tool named `Outer`, and both tools have `/run %verb% --` as their `commandline`, then the final command line will
...
Properties, and they contain the special token `%command%`, then `%command%` is replaced by the entire command-line built up from the
compatibility tools. For example, if a compat tool named `Inner` requires
a compat tool named `Outer`, both tools have `/run %verb% --` as their `commandline`, and the user has set the Launch Options in the game's
properties to
...
example, setting the launch options to `--debug` is equivalent to `%command% --debug`.
...
* `STEAM_COMPAT_CLIENT_INSTALL_PATH`:
    ...
    same as the target of the `$HOME/.steam/root` symbolic link.

    In the legacy [scout `LD_LIBRARY_PATH` runtime][ldlp], this variable
    is not set.
    Falling back to dereferencing `$HOME/.steam/root` can be used as an
    alternative.
...
* `STEAM_COMPAT_EMULATOR`:
    ...
    Absolute or relative path (an absolute path is recommended)
    to a file in the format described by
    [**steam-runtime-emulator.json**(5)][steam-runtime-emulator.json(5)].
    This should be set by Steam when using CPU emulation
    (for example running x86\_64 or i386 games on aarch64)
    in conjunction with a [container runtime][],
    instead of adding the emulator to the exec chain,
    so that the container runtime machinery can use the emulator during setup.

    When not using CPU emulation
    (x86\_64 or i386 games on x86\_64,
    native aarch64 games on aarch64,
    or a compatibility tool on aarch64 whose ELF binaries are all aarch64),
    this variable should remain unset.
...
* `STEAM_COMPAT_FLAGS`:
    ...
    * `search-cwd-first`: Same as **search-cwd**,
        but the `${STEAM_COMPAT_INSTALL_PATH}` is prepended to the `LD_LIBRARY_PATH` instead of appending.
...
* `STEAM_COMPAT_GRAPHICS_PROVIDER`:
    ...
    Absolute path to a file in the format described by
    [**steam-runtime-graphics-provider.json**(5)][steam-runtime-graphics-provider.json(5)].

    The graphics stack provider described in this file will be used
    for each architecture listed in its `architectures` field.
    Additional architectures will be taken from the real root directory
    as usual.
    ...
    if the **ld.so**(8),
    glibc
    and graphics drivers for the emulated architectures are not available
    in the real root directory.
    For example,
    when using an Arch-derived operating system,
    it is not possible to install both x86\_64 and aarch64 glibc into the
    same root directory because they both want to create `/usr/lib/libc.so.6`.
    In this situation,
    the Steam client would typically set `STEAM_COMPAT_GRAPHICS_PROVIDER` to
    reference a directory containing x86\_64 and i386 **ld.so**(8),
    glibc and graphics drivers,
    leaving the real root directory responsible for providing the aarch64
    **ld.so**(8) and libraries.

    This variable could potentially also be used in the future to provide
    an i386 library stack on purely x86\_64 operating systems.
...
* `STEAM_COMPAT_MACHINE_ARCHITECTURE`:
    ...
    The architecture (native ABI) of the machine that Steam is running on,
    represented as a [multiarch tuple][multiarch tuples].
    This should be set by Steam or its compatibility tool stack when using
    CPU emulation,
    for example running x86\_64 or i386 games on aarch64.

    For example,
    when running x86 games via [FEX][] on an aarch64 (64-bit ARM) system,
    either Steam or FEX would set
...
* `STEAM_COMPAT_RUNTIME_SDL2`:
    ...
    May be set to 1 for the same effect as adding `runtime-sdl2` to `$STEAM_COMPAT_FLAGS`.
    ...
    May also be set to `classic` to attempt to use the "classic" SDL2 library,
    if available,
    even if sdl2-compat is the default.
    This will only work if the container runtime actually contains a copy of
    "classic" SDL2,
    and only with newer Steam Linux Runtime releases.
    (For example, a game's Launch Options can be set to
    `STEAM_COMPAT_RUNTIME_SDL2=classic %command%`)

* `STEAM_COMPAT_RUNTIME_SDL3`:

    May be set to 1 for the same effect as adding `runtime-sdl3` to `$STEAM_COMPAT_FLAGS`.
    (For example, a game's Launch Options can be set to
...
are invoked with a `%verb%` in the `commandline`(if any) replaced by `waitforexitandrun`.
...
* Run the install script. For version 1 compat tools, the compat
    ...
    (usually `$HOME/.local/share/Steam` or `$HOME/.steam/debian-installation`)
...
`STEAM_COMPAT_APP_ID` is set to `0`.`SteamAppId` is not set.`STEAM_COMPAT_DATA_PATH` is set to a unique directory per non-Steam game.
...
`STEAM_COMPAT_INSTALL_PATH` is not set.`LD_LIBRARY_PATH` is usually set to the empty string to "escape" from
...
[sniper]: container-runtime.md#sniper
[soldier]: container-runtime.md#soldier
[steamrt4]: container-runtime.md#steamrt4
[steam-runtime-emulator.json(5)]: steam-runtime-emulator.json.5.md
[steam-runtime-graphics-provider.json(5)]: steam-runtime-graphics-provider.json.5.md

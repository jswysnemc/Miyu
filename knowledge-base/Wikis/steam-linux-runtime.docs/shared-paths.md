...
* `/efi` is irrelevant to a user-level application.
    It is never shared.
...
* `/run` is unique to the container: in general we cannot share this,
    because we need it for filesystem APIs like `/run/host` and `/run/pressure-vessel`.
...
* The user's `$HOME/.local/share/fonts` becomes `/run/host/user-fonts`
...
* Every top-level directory not mentioned here
    (unless
    [ignored by `PRESSURE_VESSEL_WORKAROUNDS=limit-shared-dirs`][limiting shared directories]),
    except for:
    * `/app`
    * `/boot`
    * `/efi`
...
with the game's container.
This means that games are free to save files in `$HOME/.local/share/Game` or `$HOME/.game`, for example.
...
`$HOME/.var/app/com.steampowered.App1234` outside the container is mapped
...
The private directory itself, such as `$HOME/.var/app/com.steampowered.App1234`,
...
shared read/write,
unless ignored by `PRESSURE_VESSEL_WORKAROUNDS=limit-shared-dirs`:

* `/home`: Other users' home directories (if accessible by the current user),
    sometimes also used to contain mount points for non-system partitions
...
If the
[`limit-shared-dirs` workaround][limiting shared directories]
is requested by environment variable
`PRESSURE_VESSEL_WORKAROUNDS=limit-shared-dirs`,
then those locations are also not shared,
except for `/var/tmp`.`/var/tmp` is treated specially.
...
directory, such as `$HOME/.var/app/com.steampowered.App1234/.cache/tmp`
(this is consistent with Flatpak's behaviour).

## Only shared for Steam Client

[Only shared for Steam Client]: #only-shared-for-steam-client
[Usually not shared]: #usually-not-shared

All top-level directories that are not otherwise mentioned are generally
not shared with the containers used to run apps/games.
This includes any custom top-level directory such as `/large-disk-drive`.

However,
when the container runtime framework is used to run the Steam Client,
custom top-level directories usually *are* shared.
This is done to make it possible to add custom Steam Library directories
in those locations.
...
the `STEAM_COMPAT_MOUNTS`,`PRESSURE_VESSEL_FILESYSTEMS_RW` and/or `PRESSURE_VESSEL_FILESYSTEMS_RO` environment variables.
These environment variables cannot be used for locations that are
[never shared][].

## Limiting shared directories

[limiting shared directories]: #limiting-shared-directories

If the container runtime framework is run with environment variable
`PRESSURE_VESSEL_WORKAROUNDS=limit-shared-dirs`,
or with `limit-shared-dirs` or one of its aliases added to a list of
workarounds separated by commas and/or spaces,
then several directories are no longer shared.
This is a workaround for automounted network filesystems that might be
unreachable;
a more robust solution for this is to avoid mounting filesystems that
cannot be relied on to work,
and instead use non-filesystem-based network abstractions such as
[gvfs][] or [KIO][] to access potentially unreliable network resources.

Users can override this by setting the `STEAM_COMPAT_MOUNTS`,`PRESSURE_VESSEL_FILESYSTEMS_RW` and/or `PRESSURE_VESSEL_FILESYSTEMS_RO` environment variables
to the subset of these filesystems that do work reliably.

For example,
if `PRESSURE_VESSEL_WORKAROUNDS=limit-shared-dirs` is combined with `STEAM_COMPAT_MOUNTS=/srv:/home:/hdd`,
then `/srv`,`/home` and `/hdd`
will be shared with the container,
but `/mnt` and `/media` will not,
which could be useful if `/mnt/fileserver` is an unreliable NFS mount.
...
* `/home`
    ...
    * The rest of `/home` [follows the home directory][],
        if not [limiting shared directories][]

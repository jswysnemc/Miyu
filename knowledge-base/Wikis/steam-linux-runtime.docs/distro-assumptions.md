...
- The [traditional Steam Runtime][ldlp],
    ...
    `$HOME/.steam/root/ubuntu12_32/steam-runtime` provides
...
- The [container runtime framework][container runtime],
    ...
    `.../steamapps/common/SteamLinuxRuntime_soldier`,`.../steamapps/common/SteamLinuxRuntime_sniper` and/or `$HOME/.steam/root/ubuntu12_64/steam-runtime-sniper`.
...
The container runtime framework assumes that `/sbin/ldconfig` reads
its list of additional library paths from one of these locations:

* `/etc/ld.so.conf` (glibc default, preferred)
* `/usr/share/defaults/etc/ld.so.conf`, as used on Solus
* `/etc/ld-x86_64-pc-linux-gnu.path` or `/etc/ld-i686-pc-linux-gnu.path`, as used on Exherbo
* more paths can be added if necessary,
    [contact the maintainers][Steam Runtime issues]
...
* `/usr/x86_64-pc-linux-gnu/lib/locale` and/or `/usr/i686-pc-linux-gnu/lib/locale`, as used on Exherbo
* more paths can be added if necessary,
    [contact the maintainers][Steam Runtime issues]
...
* Wayland (optional): `$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY` or `$XDG_RUNTIME_DIR/wayland-0`
* D-Bus system bus (recommended): `/var/run/dbus/system_bus_socket` or `/run/dbus/system_bus_socket`
* D-Bus session bus (recommended): as described by `$DBUS_SESSION_BUS_ADDRESS`,
    or `$XDG_RUNTIME_DIR/bus`
* PulseAudio (recommended): as described by `$PULSE_SERVER`, or see `pressure-vessel/flatpak-run.c` in `steam-runtime-tools` source code
    for the full search path
* Pipewire (optional): `$XDG_RUNTIME_DIR/pipewire-0`, or see `pressure-vessel/wrap-pipewire.c` in `steam-runtime-tools` source code
    for the full search path
...
2. (preferred) They can be listed in the `ld.so` cache, with one of the
    ...
    - `/etc/ld-x86_64-pc-linux-gnu.cache` or `/etc/ld-i686-pc-linux-gnu.cache`, as used on Exherbo
    - more paths can be added if necessary,
        [contact the maintainers][Steam Runtime issues]
...
`/etc/ld-x86_64-pc-linux-gnu.cache` and `/etc/ld-i686-pc-linux-gnu.cache` both exist, and `/usr/x86_64-pc-linux-gnu/bin/ldconfig` and `/usr/i686-pc-linux-gnu/bin/ldconfig` both exist as executable files,
then it will run
`/usr/x86_64-pc-linux-gnu/bin/ldconfig -XNv` and `/usr/i686-pc-linux-gnu/bin/ldconfig -XNv` and combine their results.
...
`$HOME/.steam/root/ubuntu12_32/steam-runtime`, should **not** be in the `LD_LIBRARY_PATH`.
...
version can provide its own 32-bit (`i386`) version of `libfontconfig.so.1`, with an equal or newer version.
...
need to be be in the dynamic linker search path (`ld.so` cache or `LD_LIBRARY_PATH`).
...
GBM backends are optional.
If the `libgbm.so.1` library is required, then the GBM backends such
as `dri_gbm.so` are assumed to be available in the search path
defined by `GBM_BACKENDS_PATH`, or in a `gbm` subdirectory of the
directory containing the loader library `libgbm.so.1`.
...
then Steam should be run with the `GBM_BACKENDS_PATH` environment variable
set appropriately.
...
If the operating system needs a non-standard search path to be used,
then Steam should be run with the `__EGL_VENDOR_LIBRARY_DIRS` or `__EGL_VENDOR_LIBRARY_FILENAMES` environment variable set appropriately.
...
directory containing the loader library `libGLX_mesa.so.0`,`libEGL_mesa.so.0` or `libGL.so.1`.
Some special cases for the Flatpak freedesktop.org SDK are also checked
(see steam-runtime-tools source code for details).
If the operating system needs a non-standard search path to be used,
...
a `dri` subdirectory of the directory containing the loader library `libva.so.2`.
Some special cases for the Flatpak freedesktop.org SDK are also checked
(see steam-runtime-tools source code for details).
If the operating system needs a non-standard search path to be used,
...
directory in the same prefix, for example `/usr/lib64/libdrm.so.2` implies `/usr/share`.
...
For example, on Arch Linux and other "merged-`/usr`" operating systems,`/bin` is a symbolic link to `usr/bin`, so the shell is really `/usr/bin/sh`.
...
The glibc locale data conventionally stored in `/usr/lib/locale` and `/usr/share/i18n` is assumed to be available in one of the locations
...
provided by `flatpak run --command=bash com.valvesoftware.Steam`.`steam-runtime-system-info` is primarily designed to be run inside a
...
```
$HOME/.steam/root/ubuntu12_32/steam-runtime/setup.sh
$HOME/.steam/root/ubuntu12_32/steam-runtime/run.sh -- steam-runtime-system-info
```
...
[steam-runtime#791]: https://github.com/ValveSoftware/steam-runtime/issues/791

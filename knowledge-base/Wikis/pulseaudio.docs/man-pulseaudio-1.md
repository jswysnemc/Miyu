...
-->
     ...
     PulseAudio is a networked low-latency sound server for Linux, POSIX and Windows systems.
       ...
       daemon.conf  (see below), parse remaining
      ...
      daemon configuration, in a format that is compatible with
       daemon.conf .
        ...
        List available loadable modules. Combine with
       -v  for a more elaborate listing.
        ...
        Identify stale PulseAudio POSIX shared memory
      segments in  /dev/shm  and remove them if
      ...
      virtual file system mounted to  /dev/shm
      (e.g. Linux).
      ...
      different from starting PulseAudio without  --start
      ...
      returns. Implies  --daemonize .
        ...
        Kill an already running PulseAudio daemon of the
      ...
      server is accessible via a socket). Also disabling PID files with
       --use-pid-file=no  prevents  --check  from
      ...
      there's currently no  --check-connection  option to
      replace  --check , but running "pactl info" could be a
      pretty good substitute.


        --system  [=BOOL]

        Run as system-wide instance instead of
      ...
      use  --daemonize=no  for systemd notification to work.


        --fail  [=BOOL]

        Fail startup when any of the commands specified in
      the startup script  default.pa  (see below)
      fails.


        --high-priority  [=BOOL]

        Try to acquire a high Unix nice level. This will
      ...
      system daemon (see  --system  above). It is recommended
      ...
      below).
        ...
        Try to acquire a real-time scheduling for
      ...
      plug.
        ...
        Unload autoloaded samples from the cache when they
      ...
      corresponding to  error ,  warn ,
       notice ,  info ,  debug . Default
      log level is  notice , i.e. all log messages with lower
      log levels are printed:  error ,  warn ,
       notice .
      ...
      file name to avoid overwriting.
        ...
        When FRAMES is greater than 0, log for each message a
      stack trace up to the number of specified stack frames.
      ...
      (plugins).
       ...
       --dump-resample-methods  above for possible
      values).
        ...
        Do not install CPU load limiter on platforms that
      ...
      debugging PulseAudio with tools like   which slow down execution.
        ...
        PulseAudio clients and the server can exchange audio
      ...
      arguments.
        ...
        Run the specified script on startup. May be
      specified multiple times to specify multiple scripts to be run
      in order. Combine with  -n  to disable loading of the
      ...
      during runtime. Equivalent to
       ...
       default.pa  (see below) on startup. Useful in
    ...
    authentication cookie. Defaults to  $HOME/.config/pulse/cookie .

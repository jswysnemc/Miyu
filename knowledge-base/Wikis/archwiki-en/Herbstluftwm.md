...
## Display manager
 includes  as Xsession which starts the window manager.

## From inside other window manager
If you are already inside a window manager session, you can replace that window manager with herbstluft by running .

## First steps
Read the  and  man pages, they contain a lot of information from an explanation of the binary tree in which the layout is kept to configuration file options and possible values.

## Configuration
Copy  file to . You can edit that file for your needs. Make sure the autostart file is executable, else you will probably end up without keybindings!

The configuration of herbstluftwm can be updated on-the-fly by running  (See #Commands). Autostart is called on each reload, therefore within autostart you typically unmap all existing configuration first.

## Multi-Monitor Support
Herbstluftwm supports multiple monitors as a virtual concept; monitors in herbstluftwm do not have to match the real monitor configuration as reported by xrandr. It brings a lot of flexibility and gives the user more control over their monitor-arrangement. You can use  to automatically adapt to the physical setup. Otherwise, see the manpage on how to add, remove, resize and move monitors. Tags in a multi-monitor set-up are not "owned" by a monitor. This means that when one monitor switches to a tag that is active in another monitor, the two monitors will swap tags.

## Commands
Herbstclient is a very powerful tool, as it provides you with full control over your window manager from the command line.

There is a tab-completion for the parameters for herbstclient. Try  to show all parameters.

Right now there is no error message if you use wrong parameters on a command, but only a non-zero return value. If you do not show the return value of a command anyway (e.g. in your $SHELL-prompt), you might want to  to find the return value of the last command.
...
Herbstluftwm includes a number of example scripts:  or https://github.com/herbstluftwm/herbstluftwm/tree/master/scripts
...
import sys
tag_offset, mode = sys.argvtag_offset = int(tag_offset)
if mode == 'full':
    ch = {'.'}
elif mode == 'empty':
    ch = {':', '!'}
...
while (tag_next != tag_curr) and (tag_list[tag_nextin ch):
...
if files == false  # 0 files
  system "herbstclient #{pad2}"
  system "touch #{file1}"
elsif files == false  # 1 file
...
## Troubleshooting
* After installing,  keybind does not start a terminal: the herbstluftwm autostart configuration file uses the xterm terminal by default. Make sure  is installed or edit the autostart file to use a different terminal.
* To log out of herbstluftwm, use the command  or the default keybind of

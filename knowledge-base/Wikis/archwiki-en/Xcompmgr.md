...
If there is no output, add the  option to the  section of xorg.conf:

Xcompmgr can be installed with the package . For transparency also install the . See Xterm#Automatic transparency for an example.
...
## Window transparency
Although its practical use is limited, due to its slow performance, the  utility can be used to set the transparency of individual windows.
...
This is fixed by installing  and setting the background color by executing  (just type the code of the color you want instead of #000000) before . Alternatively, if  is called prior to  in , you can change  to  which will fork a subshell and allow  to execute after your window manager has already started.

...
## Install
You can follow the standard Installation guide. No problems with UEFI spotted.

## Wi-Fi
When installing from 2014.12 image running on 3.17 kernel Wi-Fi for some reason does not work. It seems it works for some older versions, and for 3.18+ but with 3.17 you would probably need some USB Wi-Fi. Wi-Fi is detected, wpa_supplicant runs on it however it cannot associate in any band. It seems it is caused by way too restrictive regulation settings, so you may also try to adjust AP settings for safe channels (1-11, 36-44) and sit closer to AP to get it working.

Do not forget to install  among other required wireless packages to apply less restrictive regional regulation domain settings.
...
It is really about power management, not power supply for the card (which is managed by rfkill). Or to preserve it
...
## Troubleshooting
## SATA
You may notice during installation or on initial reboot ATA errors, such as  or  - those are result of SATA speed mismatches. The controller is capable of 6Gbps while the internal HDD can only run at 3Gbps. This will add a boot delay until  finds the correct speed by probing and resetting.

It is likely better to enforce 3Gbps by adding  to the kernel command line. Remove this restriction after migrating to an SSD drive.

## Kernel
Even though system is capable of running current stable kernel it maybe worth spending some time compiling newer rc+ kernel until it is packaged.
...
| Wi-Fi
| Realtek 8821AE
| Device Drivers -> Network Device Support -> Realtek rtlwifi family -> Realtek 8821AE
|-
| Touchpad
| Elantech EPS/2 Touchpad
| Device Drivers -> Input device support -> Mice -> PS/2 Mouse -> Elantech PS/2 Protocol extension
|-
| Touchscreen
...
| Bluetooth
| Realtek USB HCI
| Networking Support -> Bluetooth Subsystem Support -> Bluetooth Device Drivers -> HCI USB Driver
|-
| Audio
| Intel HD + Conexant
| Device Drivers -> Sound card support -> Advanced Linux Sound Architecture -> HD Audio -> Conexant and HDMI
|-
| WebCAM
| Chicony HD USB Video
| Device Drivers -> Multimedia Support -> Media USB Adapters -> USB Video Class
|}
...
; Bit 3 is set (0x4): Full software control, only keypress is sent (0x88) and handler terminates.
; Bit 1 is set (0x1): Fn switch toggles both radios (toggling airplane mode), sending corresponding ACPI key event when they are on or off.
; Any other values: Toggles radios in a sequence - both off, WL on/BT off, WL off/BT on, both on.

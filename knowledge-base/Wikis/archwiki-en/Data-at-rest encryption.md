...
:*  (temporary files created by user applications)
:** (potential remedies: avoid such applications; mount  inside a ramdisk)
:*  (log files and databases and such; for example, locate stores an index of all file names in )
...
;How should swap, , etc. be taken care of?
...
For practical implications of the chosen layer of operation, see the #Block device vs stacked filesystem encryption below, as well as the general write up for eCryptfs. See Encryption for the available content of the methods compared below, as well as other tools not included in the table.
...
| (Using 'discard' may reveal file sizes)
|
|-
...
|
| ZFS#Installation
| kernel module: already shipped with default kernel; tool:
...
| begin/end of (decrypted) device (format spec)
| DSL (dataset & snapshot layer; talk/slides)
|  directory at root of each filesystem
...
;Example 1: Simple user data encryption (internal hard drive) using a virtual folder called  in the user's home directory encrypted with EncFS
:* encrypted versions of the files stored on-disk in
...
:*  and  partitions encrypted with Dm-crypt with LUKS, using an automatically generated per-session throwaway key
:* indexing/caching of contents of  by slocate (and similar applications) disabled.

;Example 3: System encryption - whole hard drive except  partition (however,  can be encrypted with GRUB) encrypted with Dm-crypt with LUKS
...
;Example 4: Hidden/plain system encryption - whole hard drive encrypted with plain dm-crypt
:* USB-boot, using dedicated passphrase plus USB stick with keyfile
:* data integrity checked before mounting
:*  partition located on aforementioned USB stick

;Example 5: File container encryption - a pre-allocated file serves as encrypted container for user data
:* A file such as  is encrypted with Dm-crypt/Encrypting a non-root file system#File container
...
When comparing the parameters used by both modes one notes that dm-crypt plain mode has parameters relating to how to locate the keyfile (e.g. , ). The dm-crypt LUKS mode does not need these, because each blockdevice contains a header with the cryptographic metadata at the beginning. The header includes the used cipher, the encrypted master-key itself and parameters required for its derivation for decryption. The latter parameters in turn result from options used during initial encryption of the master-key (e.g. , ).
...
| 128 bits
| 128, 192 or 256 bits
| Approved by the NSA for protecting "TOP SECRET" classified US-government information when used with a key size of 256 bits (CNSA 2.0). 128- and 192-bit keys remain safe against non-quantum adversaries (same for below). Commonly has fast hardware implementation.
|-
| Blowfish
| 64 bits
| 32–448 bits
...
| 128 bits
| 128, 192 or 256 bits
| Considered the most secure of the five AES-competition finalists|}
...
## Stream ciphers
Stream ciphers work in streams and do not need such thing as a "mode of operation". Nevertheless, properties required for disk encryption (e.g. "wide-block") mean that layers need to be added on top of them. As far as disk encryption on Linux is concerned, the main example is XChaCha12 in the [https://lwn.net/Articles/776721/ Adiantum configuration, , intended for low-end systems where the hardware has no AES acceleration. Adiantum is fast on such hardware. It also guarantees authenticity of the encrypted data.
...
## Block device encryption
There are multiple options; you can back up the disk block device where the encryption container resides as an image, e.g. , or you can back up the filesystem inside of the encrypted container, e.g. , or you can back up the files, e.g. with rsync. The following sections list the advantages and disadvantages of each option.

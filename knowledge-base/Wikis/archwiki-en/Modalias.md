...
Let us take it apart piece-by-piece.  First, the file name, :

* pci0000:00 is the id for the first PCI bus.  For most machines this will be the only PCI bus you have, but it is possible this can extend to pci0000:01 or pci0000:02 - the exacts are unimportant, as it is a good guess that you only have one PCI bus (HINT: try  to check)
* 0000:00:1f.1 is the index of the given device on the PCI bus.  Specifically, this is on bus 0000:00 and has index 1f.1
* All this is rather unimportant, unless you want to know where all these numbers come from.  For completeness, if you check the output of  you will see the same information:
...
bc (base class) and sc (sub class) are used to create the "Class" listed by , in order "bcsc".  This is the device class, which is fairly generic.  In this case, the "class" is looked up in the normal  output.  We can see that "Class 0101" maps to "IDE Interface" ( also looks up the vendor and device id's - 8086 maps to "Intel Corp." and 24DB maps to 'Unknown Device', hehe)
...
One thing which people tend to ignore, is all the work  does.  When you run , it builds a series of "map" files in  which tell modprobe how to handle certain things it needs to do.  In this case we can ignore most of them.  The important one is .  This file contains aliases, or secondary names for modules.  Just for a demonstration, let us look at aliases for, say, snd_intel8x0m:

Hey, wait! I recognize that! That's the vendor/device id information from before!

Yes, it is.  It is a rather simple format of "alias  ".  In fact, you can alias just about anything you want.  I can add "alias boogabooga snd_intel8x0m" and then safely "modprobe boogabooga".

The "*" indicates it will match anything, much like filesystem globbing ().  As stated before, most aliases ignore sv, sd, bc, sc, and i by way of the "*" matching.
...
It matches the aliases found by 'grep'ing the alias file.  These aliases exported by each module, are gathered by  and merged into the  file dynamically.  There is no hand-changing of a lookup table, as it is built on-the-fly.  Each module knows exactly what it supports, and therefore  can use that information to help load modules.

...
Following the xterm automatic transparency guide, it is possible to get per application window transparency with just , or any other compositor, and either  or . Just replace xterm with your program name and transset-df with your transset selection:

 xterm & sleep .8s && transset-df -a

For Openbox key/mousebindings, use this inside your :

 sh -c 'xterm & sleep .8s && transset-df -a'
...
## Installation
Firstly, you will need to make sure that , , and  are installed. Once installed, run xcompmgr (if you do not already have it running):
...
First, create an opacity.ds file in  (create that directory if it does not already exist):
...
While this will simply make GVim transparent, you might want to do this with more than one application. Here is an example configuration that makes all GVim, Mirage, and Chromium windows transparent. (Adding more windows should be apparent from this example.)
...
## In the background
In some instances, setting an application (in particular a terminal) as a transparent window in the background is desired.
...
## Starting with X
Simply place the following in your X startup script (i.e., ) to have per application window transparency load:
...
Alternatively,  is a GUI configuration editor for .

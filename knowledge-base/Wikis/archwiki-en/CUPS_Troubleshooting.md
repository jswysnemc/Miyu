...
## Debug log
The best way to get printing working is to enable debug logging:

And then viewing the output from  like this:
...
Print a document and watch  to get a more detailed and correct image of the printing process.
...
If all jobs sent to the printer become "stopped", first check the error log at . If it does not yield any useful hints to what is the source of the issue, delete the printer and add it again.
...
In GNOME Printer settings for example, you could find troubles adding your printer, even though you have installed the drivers. You can try installing  and executing it via terminal, and adding the printer from there (for instance using LPD/LPR queue 'PASSTHRU').
...
## Old CUPS server
As of CUPS version 1.6, the client defaults to IPP 2.0. If the server uses CUPS  Device : ID : Hewlett-Packard DeskJet D1360
}}
...
If receiving any of the following error messages in  while using a HP printer, with jobs appearing to be processed while they all end up not being completed with their status set to 'stopped':
...
* Modify the PPD used on the client side so the job does not goes through the filter client-side. Find the right PPD in  and copy it in your home directory. Edit the copy : replace the line  with . Now, add your printer on the client CUPS, selecting your custom PPD located in your home directory.
...
## CUPS: prints only an empty or a single page and an error-message page on HP LaserJet
There is a bug that causes CUPS to fail when printing images on HP LaserJet (Seen for LasertJet 3380, and for LaserJet 1300 as recently as July 2024). The bug has been reported and fixed by Ubuntu.
...
The issue might have to do with the file permission change that had been made to . To correct the issue, a simple  and  should be sufficient. For further information, please read this link.
...
This can also be caused by printers such as the P1102 that provide a virtual CD-ROM drive for MS Windows drivers. The  appears in  and then disappears. In that case, try the  package, that lets one switch off the "Smart Drive" (udev rules included in said package).

## Networked printers
This can also occur with network attached printers using dynamic hostnames if the avahi-daemon is not running. Another possibility is that hp-setup failed to locate the printer because the IP address of the printer changed due to DHCP. If this is the case, consider adding a DHCP reservation for the printer in the DHCP server's configuration.
If the printer already has a static IP and comes with an own webserver check if SNMP is turned on by calling the printers ip address in a browser: Networking > SNMP.
...
Or, if in interactive (console) mode, you may encounter something similar to this even when providing a correct path to a valid ppd file:
  ...
  Traceback (most recent call last):
    File "/usr/bin/hp-setup", line 536, in
      desc = nickname_pat.search(nickname).group(1)
  TypeError: cannot use a string pattern on a bytes-like object

The solution is to install and start  before running .

## hp-setup: "Qt/PyQt 5 initialization failed"
Install , which is an optdepend of . Alternatively, to run hp-setup with the command line interface, use the  flag.
...
If adding a printer fails silently in the UI or you receive a  traceback from , the  may need to be manually updated.
Check if  is defined the section for your printer, if not add it:

## Broken pipe
If  contains the  error, you probably need to install the  and restarts the cups service.
...
If a remote printer requests authentication CUPS will automatically add an  directive to the printer in . However, some graphical applications (for instance, some versions of LibreOffice have no way to prompt for credentials, so printing fails.
To fix this include the required username and password in the URI.
See [https://bugs.launchpad.net/ubuntu/+source/cups/+bug/283811, === Unknown supported format: application/postscript ===
...
If you get this error, ensure CUPS is running, the environment variable  is unset, and that  is correct.

## "Using invalid Host" error message
Try adding  into .

## Cannot print from LibreOffice
If you can print a test page from the CUPS web interface, but not from LibreOffice, try to install the  package.
...
To automatically have CUPS reactivate the printer, change [https://www.cups.org/doc/man-cupsd.conf.html?TOPIC=Man+Pages#ErrorPolicy ErrorPolicy from the default  to .

## Samsung: URF ERROR - Incomplete Session by time out
This error is usually encountered when printing files over the network through IPP to a Samsung printer, and is solved by using the  package.
...
If a regular user needs to be able to change the printers properties or manage the printer queue, the user may need to be added to the  group.

## Cannot login into web interface
Check if there is more than one  process running. If this is the case then stop , kill all processes named  and start  again.

## Virtual PDF printer stops jobs
If the virtual PDF printer (driver: ) keeps stopping print jobs and the error log shows , reinstalling  may help.

## No suitable destination found
If a printer prefers ipps (ipp-secure), CUPS will store the printer's certificate in . However, these certificates are never removed, even if invalid. There are several ways for a certificate to become invalid, apart from expiring: for example, if you switch from standard, self signed, printer certificates to official SSL-certificates; if your printer software is upgraded and it generates a new certificate; or if a printer is simply changed for a new one with the old name. In any of these situations, CUPS will not renew the cached certificate, but will only complain with a rather generic message "no suitable destination found".

This is, unfortunately, not fixed by removing and re-installing the printer: even then, the invalid certificates are kept on the file system. The only way to fix this is to manually remove the certificates.

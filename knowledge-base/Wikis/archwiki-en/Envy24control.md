...
envy24control is an application included in the  package. It enables controlling the digital mixer, channel gains, and other hardware settings for sound cards based on the VIA Ice1712 chipset (A.K.A. Envy24).
...
## Installation
Install the  package, which includes envy24control.
...
# The default setting, "PCM Out 1/2", is your music software outputs. For example, Ardour's main outs connected to "playback_1/2" via JACK's Connections dialog.
# "Digital Mix" is the output of the card's mixer.This handy option allows you to mix hardware and software audio sources directly in your card's mixer with near-zero latency.
# "S/PDIF In L/R" is a direct connection to the card's S/PDIF inputs.
# "H/W In 1/2" is a direct connection to the card's analog audio inputs.
...
# The "S/PDIF Out 1/2 (L/R)" option connects these outputs to your music software outputs. Again, for example, Ardour's main outs connected via JACK.
...
{| class="wikitable"
!align="center"|Locked !! Reset !! Behavior
|-
|align="center"| - ||align="center"| X || Software applications can set codec sample rate; card returns to selected rate when not in use (default and recommended)
|-
|align="center"| - ||align="center"| - || Software applications can set codec sample rate; card leaves rate at last rate specified when not in use
|-
|align="center"| X ||align="center"| - || Codec sample rate is locked to selected rate; applications may not specify other rates (beware XRUNS!)
|}

## Actual Rate
This displays the current codec sample rate of the card. Depending on your settings, this may vary as it is controlled by applications such as JACK or Ardour and/or by envy24control itself. See #Master Clock and #Rate State for details.
...
## Tips and tricks
* One of the most common solutions when you cannot hear your incoming audio signal is to check the ADC faders for the hardware inputs into which your source is plugged. Until you adjust the ADC faders for the appropriate channels, the signal is effectively muted.
...
* Also on the "Monitor PCMs" page, unmute both the L and R channels of "PCM Out 3" and set the faders to 20. This is where will be routing your incoming audio source's signal for monitoring.

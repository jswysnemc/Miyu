...
connections and manages devices. Clients (including the PipeWire Session Manager) are separate processes that talk to the daemon using the
...
As shown above, the protocol is handled by the Protocol Native. From PipeWire's point-of-view this module is just
another module.

...
example is very similar to Tutorial - Part 4: Playing A Tone.
...
See SPA POD for more information about how to make these
POD objects.
...
```c
	...
	printf("  format: %d (%s)", data->format.info.raw.format,
			spa_debug_type_find_name(spa_type_video_format,
				data->format.info.raw.format));
	printf("  size: %dx%d", data->format.info.raw.size.width,
			data->format.info.raw.size.height);
	printf("  framerate: %d/%d", data->format.info.raw.framerate.num,
...
Tutorial - Part 4: Playing A Tone for another example.

 tutorial5.c on_process

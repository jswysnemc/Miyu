...
Pulseaudio client configuration (requires pipewire-pulse(1) restart to apply).
See pipewire-pulse.conf(5)
```css
...
Set value of a device Param to a JSON value when the device is loaded.
This works similarly as pw-cli(1) `set-param` command.
...
Set value of a node Param to a JSON value when the device is loaded.
This works similarly as pw-cli(1) `set-param` command.
The `PARAM` should be replaced with the name of the Param to set,
...
in pipewire.conf(5) and session
manager settings.

**Note:** Only the properties `pipewire.*` are safe to use for security
...
pw-dump(1) output) is larger than considered
above:

...
The PipeWire core will emit a `check_access` event in the pw_context_events
...
manager, see e.g. PipeWire Session Manager.

## The PipeWire Access Module

The Access hooks into the `check_access` event that is
emitted when a new client is registered. The module checks the permissions of
the client and stores those in the PW_KEY_ACCESS
...
the permissions for a client. See the Access documentation
for details.
...
set the PW_KEY_MEDIA_CATEGORY property in the client object to "Manager".

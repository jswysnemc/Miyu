...
number of the pw_context on the server side.
...
**See:** native-protocol-registry-generation
...
pw_context that increments by one when the server allocates a new
global PW_KEY_OBJECT_ID for an object.
...
1. Look up the pw_global based on the *id*.
...
PW_KEY_OBJECT_SERIAL, which are unique for objects and not
reused, unlike PW_KEY_OBJECT_ID

**See:** PW_KEY_OBJECT_SERIAL

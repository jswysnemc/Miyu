...
that "monotonic system clock" does not refer to the  MONOTONIC_RAW clock in
...
spa_io_clock instance that is assigned to them through the
spa_node_methods::set_io callback. The fields of the struct must be
updated as follows:

- spa_io_clock::nsec : Must be set to the time (according to the monotonic
...
- spa_io_clock::rate : Set to a value that can translate samples to nanoseconds.
- spa_io_clock::position : Current cycle position, in samples. This is the
  ...
  SPA_IO_CLOCK_FLAG_DISCONT flag to inform other nodes about this.
- spa_io_clock::duration : Duration of this new cycle, in samples.
- spa_io_clock::rate_diff : A decimal value that is set to whatever correction
...
- spa_io_clock::next_nsec : Must be set to the time (according to the monotonic
  ...
  SPA_IO_CLOCK_FLAG_DISCONT flag in such a case. Just like with nsec, to
  minimize jitter, it is usually a good idea to increment this by a computed amount
  except for when the driver starts and when discontinuities occur in its clock.

The driver node signals the start of the graph cycle by calling spa_node_call_ready
with the SPA_STATUS_HAVE_DATA and SPA_STATUS_NEED_DATA flags passed
...
the spa_io_clock::next_nsec timestamp. They do not have to use the monotonic
clock itself for scheduling the next cycle. If for example the internal clock
can directly be used with  timerfd , the next cycle can be triggered that way.

As mentioned above, the spa_io_clock::position field is the _ideal_ position
of the graph cycle, in samples. This contrasts with spa_io_clock::nsec, which
...
clock. In that case, the spa_io_clock::nsec timestamps are adjusted to match the
pace of that different clock (explained in the section below). In such a case,
spa_io_clock::position still is incremented by the duration in samples. This
...
clock time. This is accomplished by adding that offset to the spa_io_clock::nsec
and spa_io_clock::next_nsec fields. For example, when the driver uses the realtime
system clock instead of the monotonic system clock, then that realtime clock can still
be used with  timerfd to schedule callback invocations within the data loop. Then,
computing the (monotonic_clock_time - realtime_clock_time) offset is sufficient, as
mentioned, to be able to translate clock's time to monotonic time for spa_io_clock::nsec
and spa_io_clock::next_nsec (which require monotonic clock timestamps).
...
for triggering callbacks (usually via  timerfd) and adjust the time when callbacks are
...
spa_io_clock::rate_diff field is set to 2.0.
...
for scaling durations between  timerfd timeouts. This forms a control loop, since the
correction factor causes the spa_io_clock::next_nsec increments (that is, the
...
notes above about spa_io_clock::position being the ideal position of the graph

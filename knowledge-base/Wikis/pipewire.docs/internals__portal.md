...
the task of the PipeWire Session Manager to make the objects in the graph
visible, depending on the client's `media_roles` (see also PW_KEY_MEDIA_ROLE).

## The PipeWire Portal Module

The PipeWire daemon uses the Portal to find the PID of the
...
Client connections from this PID are tagged as PW_KEY_ACCESS
`"portal"` (see Access). It will also set ALL permissions for
...
PW_KEY_ACCESS property to find portal connections. For client connections

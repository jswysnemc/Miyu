...
**bind** *`NAME`* *`PATH`* \[*`PATH`*\]  
Bind mounts a file or directory from the host into the specified container. The first path argument is the source file or directory on the host, the second path argument is the destination file or directory in the container. When the latter is omitted, the destination path in the container is the same as the source path on the host. When combined with the `--read-only` switch, a read-only bind mount is created. When combined with the `--mkdir` switch, the destination path is first created before the mount is applied. Note that this option is currently only supported for systemd-nspawn(1) containers, and only if user namespacing (`--private-users`) is not used. This command supports bind mounting directories, regular files, device nodes, `AF_UNIX` socket nodes, as well as FIFOs.
...
**edit** *`NAME|FILE`*  
Edit the settings file of the specified machines. For the format of the settings file, refer to systemd.nspawn(5). If an existing settings file of the given machine cannot be found, **edit** automatically create a new settings file from scratch under `/etc/systemd/nspawn/`.
...
`--kill-whom=`  
When used with **kill**, choose which processes to kill. Must be one of `leader`, `supervisor`, or `all` to select whether to kill only the leader process of the machine, the supervisor process of the machine, or all processes of the machine. If omitted, defaults to `all`.

...
<figcaption>A trackpoint</figcaption>
</figure>

libinput always treats the buttons below the space bar as the buttons that belong to the trackpoint even on the few laptops where the buttons are not physically wired to the trackpoint device anyway, see `t440_support`.

## Button scrolling on trackpoints

Trackpoint devices have `button_scrolling` enabled by default. This may interfere with middle-button dragging, if middle-button dragging is required by a user then button scrolling must be disabled.
...
The values provided by a trackpoint are motion deltas, usually corresponding to the pressure applied to the trackstick. For example, pressure towards the screen on a laptop provides negative y deltas. The reporting rate increases as the pressure increases and once events are reported at the maximum rate, the delta values increase. The figure below shows a rough illustration of this concept. As the pressure decreases, the delta decrease first, then the reporting rate until the trackpoint is in a neutral state and no events are reported. Trackpoint data is hard to generalize, see [Observations on trackpoint input data](https://who-t.blogspot.com/2018/06/observations-on-trackpoint-input-data.html) for more details.

<figure class="align-center">
<img src="trackpoint-delta-illustration.svg" alt="trackpoint-delta-illustration.svg" />
<figcaption>Illustration of the relationship between reporting rate and delta values on a trackpoint</figcaption>
</figure>

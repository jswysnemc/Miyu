...
[This introductory blog post from 2015](https://who-t.blogspot.com/2015/06/libinput-and-lack-of-device-types.html) describes some of the motivations.
...
libinput handles all common devices used to interact with a desktop system. This includes mice, keyboards, touchscreens, touchpads and graphics tablets. libinput does not expose the device type to the caller, it solely provides capabilities and the attached features (see [this blog post](https://who-t.blogspot.com/2015/06/libinput-and-lack-of-device-types.html)).

For example, a touchpad in libinput is a device that provides pointer events, gestures and has a number of `config_options` such as `tapping`. A caller may present the device as touchpad to the user, or simply as device with a config knob to enable or disable tapping.

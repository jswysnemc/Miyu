...
To use the rrsync script, edit the user's `$HOME/.ssh/authorized_keys` file and add
...
To use an rsync daemon setup, edit the user's `$HOME/.ssh/authorized_keys` file and
...
[`--config`](rsync.1#dopt) option is omitted, it defaults to `$HOME/rsyncd.conf`.
...
The `$HOME/.ssh/authorized_keys` file might have lines in it like this:

...
>     rsync -aiv host1:dir1 $HOME/host1-files
...
system (which can be forced via the `$HOME/.ssh/authorized_keys` file, if desired).
...
>     rsync -aiv --remove-source-files rhost:/tmp/{file1,file2}.c $HOME/src/
...
0.  `--modify-window=NUM`, `-@`
    ...
    create a `$HOME/.popt` file and put these lines in it:
...
0.  `--perms`, `-p`
    ...
    putting this line in the file `$HOME/.popt` (the following defines the `-Z`

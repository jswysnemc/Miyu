# Docs Xml / Manpages / libsmbclient.7
...
smb://name/
			...
			This command causes libsmbclient to perform a name look-up. If the NAME  or
			NAME  exists (workgroup name), libsmbclient will list all servers in the
			workgroup (or domain). Otherwise, a name look-up for the NAME  (machine name)
	...
	if present, will fully over-ride the system  file. If
	instead libsmbclient finds a file called  $HOME/.smb/smb.conf.append ,
	it will read the system  /etc/samba/smb.conf  and then append the
	contents of the  $HOME/.smb/smb.conf.append  to it.

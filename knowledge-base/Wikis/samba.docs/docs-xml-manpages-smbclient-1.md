...
servicename is the name of the service
		you want to use on the server. A service name takes the form
		 //server/service  where
server
		  is the NetBIOS name of the SMB/CIFS server
		offering the desired service and
service
...
smbclient
.
		For example: smbclient -M FRED  mymessage.txt  to the
		machine FRED.
...
See the
message command  parameter in the   smb.conf
...
F  - File containing a list of files and directories.
			The
F  causes the name following the tarfile to
...
The client will request that the server attempt
		to delete all files and directories matching
mask
 from the current working
...
full
) tar will back up everything
		regardless of the  archive  bit
		setting. When on (using
inc
), tar will only
...
reset
), tar will remove the archive bit on
...
--password
 options for details on ways to

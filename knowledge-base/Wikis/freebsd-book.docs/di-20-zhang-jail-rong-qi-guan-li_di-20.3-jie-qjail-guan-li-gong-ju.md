> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-20-zhang-jail-rong-qi-guan-li/di-20.3-jie-qjail-guan-li-gong-ju.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-20-zhang-jail-rong-qi-guan-li/di-20.3-jie-qjail-guan-li-gong-ju.md)
...
Qjail 管理的 Jail 本质上仍是标准的 FreeBSD Jail，因此 `jls`、`jexec` 等系统命令同样可用于查看和操作这些 Jail。以下为 Qjail 专属管理命令。
...
Qjail 提供 `qjail config` 命令，用于定制每个 Jail 的配置。Jail 运行时核心配置参数无法安全修改，运行该命令前必须先停止目标 Jail。
...
此时 Jail 仍无法连接外部网络，该 Jail 绑定在 `lo1` 网络接口上，`lo1` 无法直接访问外网，需借助 PF 配置网络，其中 `em0` 为外网接口。使用 `ifconfig` 命令可查找系统中的外网接口名称。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.12-jie-nei-he-zhuang-tai-mo-ren-pei-zhi-wen-jian-sysctl-conf.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.12-jie-nei-he-zhuang-tai-mo-ren-pei-zhi-wen-jian-sysctl-conf.md)
...
sysctl 以管理信息库（MIB）风格的 ASCII 名称作为标识。
...
> **技巧**
>
> 不建议直接修改 **/etc/sysctl.conf** 文件，如需自定义配置，应使用 **/etc/sysctl.conf.local** 文件扩展本地配置，避免系统更新时覆盖配置。

## 配置文件

系统进入多用户模式时读取 **/etc/sysctl.conf** 文件，用于设置内核的默认配置。格式与 **/etc/rc.conf** 类似。
...
> **警告**
>
> 虽然 **/etc/sysctl.conf** 文件实质上为空，但这不代表系统默认的 sysctl 参数为空。它们通过不同的宏（如 `SYSCTL_INT`）注入系统。使用命令 `sysctl -a` 可列出当前系统所有默认的参数值。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-3-zhang-freebsd-an-zhuang-ji-chu/di-3.7-jie-zai-windows-zhong-jiang-usb-qi-dong-pan-hui-fu-wei-pu-tong-cun-chu-she-bei.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-3-zhang-freebsd-an-zhuang-ji-chu/di-3.7-jie-zai-windows-zhong-jiang-usb-qi-dong-pan-hui-fu-wei-pu-tong-cun-chu-she-bei.md)

---

# 3.7 在 Windows 中将 USB 启动盘恢复为普通存储设备

USB 存储介质用作系统安装介质后，其分区表结构可能发生变化，导致操作系统无法完全识别其可用存储空间。

以下提供三种 Windows 环境下恢复 USB 存储介质的方法。
...
使用 Rufus、Win32DiskImager 等软件制作 U 盘启动盘用于安装系统后，用户可能会发现 U 盘的可见容量仅有 31.9 MB（即 EFI 分区）。
...
首先介绍使用 DiskGenius 恢复 U 盘的方法。DiskGenius 是一款磁盘管理工具，其官网为 <https://www.diskgenius.cn/>。该软件的免费版本可满足本节所述操作的需要。
...
判断哪个是目标 U 盘，通常可根据 U 盘容量判断。如果不确定 U 盘容量，可查询购买记录或拔下 U 盘查看其外壳上标注的容量。
...
点击 AOMEI Technology. 傲梅分区助手下载页面[EB/OL]. [2026-03-25]. <https://www.disktool.cn/download.html>，该页面提供傲梅分区助手各版本下载链接，推荐下载“绿色版”（免安装，可直接运行）。在解压目录中找到“PartAssist.exe”（可执行文件名称可能略有不同），右键单击并选择“打开”。
...
可通过以下信息判断设备是否为 U 盘（如果界面显示不全，可使用鼠标滚轮向下滚动）：

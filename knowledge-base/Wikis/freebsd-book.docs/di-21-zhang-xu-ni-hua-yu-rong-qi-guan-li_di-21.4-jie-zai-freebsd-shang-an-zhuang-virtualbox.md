> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-21-zhang-xu-ni-hua-yu-rong-qi-guan-li/di-21.4-jie-zai-freebsd-shang-an-zhuang-virtualbox.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-21-zhang-xu-ni-hua-yu-rong-qi-guan-li/di-21.4-jie-zai-freebsd-shang-an-zhuang-virtualbox.md)

---

# 21.4 在 FreeBSD 上安装 VirtualBox
...
自 4.0 起，VirtualBox 只有开源版本 Open Source Edition（OSE），其余组件以闭源扩展包（VirtualBox Extension Pack）的形式单独提供，**尚不支持** FreeBSD：该扩展包提供 USB 2.0/3.0 支持、磁盘镜像加密、VRDP 远程桌面协议等附加功能。
...
为使普通用户能够使用 VirtualBox 的 DVD/CD 功能，必须具有对 **/dev/xpt0**、**/dev/cdN** 和 **/dev/passN** 的访问权限。将以下内容添加到 **/etc/devfs.rules** 文件（如不存在请创建）：
...
虚拟机系统访问宿主机 DVD/CD 驱动器是通过共享物理设备实现的。在 VirtualBox 中，可通过虚拟机设置中的 Storage 窗口进行配置。如有需要，先创建空的 IDE CD/DVD 设备。然后在虚拟 CD/DVD 选择菜单中选择 Host Drive。此时会出现 Passthrough 复选框，勾选该项后虚拟机会直接访问硬件。例如，音频 CD 或刻录功能仅在该选项启用时可用。

因为扩展包在 FreeBSD 上暂不可用，OSE 版本的宿主机控制器仅能模拟 USB 1.1 设备。
...
### 报错：`NS_ERROR_FACTORY_NOT_REGISTERED`

以非 root 用户运行 VirtualBox 时报错 `NS_ERROR_FACTORY_NOT_REGISTERED`，可删除匹配该模式的临时文件：
...
- Oracle. Oracle VM VirtualBox Extension Pack[EB/OL]. [2026-04-17]. <https://docs.oracle.com/en/virtualization/virtualbox/7.0/user/Introduction.html>. 扩展包提供 USB 2.0/3.0、磁盘镜像加密、VRDP 等功能。

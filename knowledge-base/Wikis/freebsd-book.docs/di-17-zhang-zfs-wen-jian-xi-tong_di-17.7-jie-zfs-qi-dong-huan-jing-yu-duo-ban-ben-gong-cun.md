> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.7-jie-zfs-qi-dong-huan-jing-yu-duo-ban-ben-gong-cun.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.7-jie-zfs-qi-dong-huan-jing-yu-duo-ban-ben-gong-cun.md)

---

# 17.7 ZFS 启动环境与多版本共存

借助 ZFS 启动环境，可在同一台机器上保留多个独立的系统版本，升级失败时无需重新安装即可回滚到先前状态，从而实现原子更新、多系统并存与快速回滚。

## 创建启动环境 15.0-RELEASE

ZFS 启动环境（Boot Environment，BE）允许在系统中创建多个独立的系统环境，实现不同版本的共存与安全切换。以下创建启动环境 15.0-RELEASE。
...
操作启动环境前，需先挂载到文件系统中。创建临时目录并执行挂载：
...
> **警告**
>
> 如不按下方设置就强行升级 ZFS 池/特性，将无法访问旧版系统。

需实现多版本共存的读者可直接重启，进入启动环境 `default`（14.3-RELEASE）。

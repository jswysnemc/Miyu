> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-29-zhang-freebsd-nei-he-jia-gou/di-29.6-jie-gou-jian-ding-zhi-nei-he.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-29-zhang-freebsd-nei-he-jia-gou/di-29.6-jie-gou-jian-ding-zhi-nei-he.md)
...
如今，FreeBSD 内核中的大部分功能都包含在可以根据需要动态加载和卸载的模块中。这使得运行时内核能够立即适应新硬件，因此将这种内核称为模块化内核。
...
- 降低内存占用。定制内核通过省略未使用的功能和设备驱动来减少内存占用。内核代码始终驻留在物理内存中，应用程序无法占用，因此定制内核对于内存较小的系统尤为重要。
...
构建定制内核前，应明确其目的。具体而言，如果需特定硬件支持，该支持可能已以模块形式存在。
...
因此，不应直接编辑 GENERIC 文件，而应复制该文件并编辑其副本。配置文件名称必须 **全部使用大写字母**。管理多台硬件不同的 FreeBSD 主机时，建议以主机名命名配置文件。
...
实际上，上述示例出自 16.0-CURRENT 的 GENERIC 内核配置文件，-CURRENT 与 -STABLE、-RELEASE 的重要差异在于 -CURRENT 引入了大量的调试功能。该功能即由 `include "std.debug"` 引入，部分用户因此可能注意到 -CURRENT 系统性能显著下降。
...
> **警告**
>
> 替换内核操作不可逆。如果 `kernel.good` 也有问题，系统将无法启动且无其他可回退的内核。建议在执行前先确认回退内核可正常启动。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.4-jie-zfs-guan-li.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.4-jie-zfs-guan-li.md)
...
通过网络发送流数据是维持远程备份的有效手段，但存在一个缺点：经网络链路发送的数据未加密，任何人都可拦截流数据并将其还原为原始数据，而无需发送方的授权。这在通过互联网向远程主机发送数据时不可接受。为确保数据安全传输，可使用 SSH 加密发送的数据。由于 ZFS 要求将流从标准输出重定向，因此可通过 SSH 轻松管道传输。为确保文件系统的内容在传输过程中及在远程系统上均保持加密，可考虑使用 PEFS。

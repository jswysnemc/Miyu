> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-28-zhang-kai-fa-huan-jing/di-28.9-jie-vim-kai-fa-huan-jing-pei-zhi.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-28-zhang-kai-fa-huan-jing/di-28.9-jie-vim-kai-fa-huan-jing-pei-zhi.md)
...
Vim 常被称为“程序员的编辑器”，它在编程中非常实用，因此许多人认为它是完整的 IDE。然而，它不仅适用于程序员，Vim 也非常适合各种文本编辑，从编写电子邮件到编辑配置文件。
...
Vim 的功能可通过插件扩展，需先安装插件管理器以统一管理。以下采用 Vim 插件管理器 vim-plug。如果使用其他插件管理器，请自行调整：
...
这样即可在 coc 中对 **/usr/local/include** 下的头文件进行补全（指定编译程序搜索头文件的路径为 **/usr/local/include**）。
...
```sh
$ export CC=clang20       # 设置 C 编译程序为 clang20
$ export CXX=clang++20    # 设置 C++ 编译程序为 clang++20
```
...
代码格式化可统一代码风格，提升代码可读性。由于 vim-clang-format 插件不兼容新版 clang-format 的命令行接口变更，且多年未更新，对新版 clang-format 支持存在问题（clang-format15 可正常使用，而 clang-format17 和 clang-format19 可能存在异常），因此推荐使用 vim-codefmt。
...
- 第二条 Glaive 语句设置格式化风格。也可以设为 `"file"` 或 `"file:<format_file_path>"`。参考 ClangFormatStyleOptions[EB/OL]. [2026-03-25]. <https://clang.llvm.org/docs/ClangFormatStyleOptions.html>. 该文档列举了 clang-format 所有可配置的格式化选项及其取值说明。
...
生成程序文件 **/home/j/project/build/src/test**，然后即可正常运行。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-7-zhang-wang-luo-guan-li/di-7.3-jie-wu-xian-wang-luo-wi-fi-pei-zhi.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-7-zhang-wang-luo-guan-li/di-7.3-jie-wu-xian-wang-luo-wi-fi-pei-zhi.md)
...
基本无线网络由多个站点组成，这些站点通过 2.4GHz、5GHz 频段的无线电通信实现互联（6GHz 频段在 FreeBSD 上处于实验性支持阶段，需要特定硬件和驱动支持）。配置无线网络包含三个步骤：
...
上述输出中的 `rtwn0` 是示例网卡（COMFAST CF-912AC）的设备名称，实际输出可能因硬件而异。如果输出中冒号 `:` 后为空，说明无线网卡未识别，此时需检查硬件连接或更换兼容性更好的无线网卡。
...
如果网络连接正常，则可进行永久配置。在 **/etc/rc.conf** 文件中添加或修改相关配置：
...
如果当前系统没有网络连接，可通过 USB 网络共享等方式临时获得网络连接后再执行上述命令；也可手动从 [USTC 镜像站点](https://mirrors.ustc.edu.cn/freebsd-pkg/FreeBSD%3A14%3Aamd64/kmods_latest_3/All/) 下载所需固件包，然后使用以下命令安装：

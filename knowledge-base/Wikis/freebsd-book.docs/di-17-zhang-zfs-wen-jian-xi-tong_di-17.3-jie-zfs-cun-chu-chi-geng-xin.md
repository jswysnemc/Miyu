> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.3-jie-zfs-cun-chu-chi-geng-xin.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-17-zhang-zfs-wen-jian-xi-tong/di-17.3-jie-zfs-cun-chu-chi-geng-xin.md)
...
ZFS 存储池的可用功能由功能标志（feature flags）控制。FreeBSD 大版本升级后，需手动执行 `zpool upgrade` 启用新功能集——需要注意的是，该操作不可逆，升级后旧版系统将无法识别新功能集。
...
zroot  67.5G  2.20G  65.3G        -         -     2%     3%  1.00x    ONLINE  -	# 分区扩容仅调整了分区表，ZFS 池尚未感知到底层设备的空间变化，因此容量仍为 67.5G，尚未扩展，需要单独操作扩展。
```
...
创建 ZFS 存储池时，有些决策不可更改，其中最重要的是物理磁盘按何种 vdev 类型分组。创建池后，镜像（mirror）允许向 vdev 添加新磁盘；条带（stripe）也可以通过将新磁盘附加到 vdev 来升级为镜像。启用 `raidz_expansion` 功能标志后，RAID-Z vdev 也可扩展。尽管添加新的 vdev 可以扩展池，vdev 类型在创建后无法更改。此时只能备份数据、销毁池，然后重新创建。
...
ZFS 在各 vdev 之间条带化数据。例如，使用两个镜像 vdev 实际上构成了 RAID 10，写入会条带化到两个镜像组上。ZFS 分配空间时力求各 vdev 同步达到 100% 占用。vdev 之间的可用空间不均衡会降低性能，因为更多写入会流向剩余空间较多的 vdev。
...
升级 FreeBSD 后，或从运行旧版系统的计算机导入池时，需要手动将池升级到最新的 ZFS 版本以启用新功能标志。升级前应评估旧系统未来是否会再次导入该池，因为升级是单向操作——旧池可以升级，但升级后的池不支持降级。
...
找到类型为 `freebsd-boot` 的分区，此处分区序号为 `1`，对应以下命令中的 `-i` 选项，然后重写 `bootcode`：

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.4-jie-jin-cheng-yu-shou-hu-jin-cheng.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.4-jie-jin-cheng-yu-shou-hu-jin-cheng.md)
...
每个进程都由一个称为进程 ID（PID）的数字唯一标识。与文件类似，每个进程都有一个属主和组，属主和组权限用于确定进程可以打开哪些文件和设备。大多数进程还有一个启动它们的父进程。例如，shell 是一个进程，在 shell 中启动的任何命令都是以 shell 为父进程的进程。例外是特殊进程 init(8)，其始终是系统启动时的首个进程，并且始终具有 PID 1。在 FreeBSD 中，所有进程都是以某个账户的名义启动的。
...
有些程序并非为持续接收用户输入而设计，而是在启动后即与终端断开连接。例如，Web 服务器响应 Web 请求而非用户输入，邮件服务器是此类应用程序的另一个例子。这类程序被称为守护进程（daemon）。术语“daemon”来自希腊神话，代表一种非善非恶的实体，它默默执行有用的任务。因此，BSD 的吉祥物设计为身着运动鞋、手持草叉的快乐守护进程形象。在 Windows 系统中，这类程序被称为“服务”，可在任务管理器中查看。

这些长期在后台运行的服务通常命名为 `xxxd`，例如 `sshd`、`ntpd`，其中的 `d` 表示守护进程（[daemon](https://www.freebsd.org/copyright/daemon/)），这是 UNIX 系统的通用命名约定。例如，BIND 是 Berkeley Internet Name Domain，但实际执行的程序名为 named。Apache Web 服务器程序是 httpd，数据同步 Rsync 进程是 rsyncd。这仅是命名约定，例如数据库应用程序 MySQL 的守护进程是 mysql-server，而非 mysqld。

## 查看进程

用户只能向自己拥有的进程发送信号，root 用户可以向任何进程发送信号。SIGKILL 和 SIGSTOP 信号无法捕获或忽略。特殊 PID：`0` 发送到同组所有进程，`-1` 发送到所有进程（root）或自己的所有进程。

要查看系统上运行的进程，可以使用 ps(1) 或 top(1)。FreeBSD 的 `ps` 基于 4.4BSD，BSD 风格关键字允许省略 `-`，如 `ps auxww`。

要显示当前运行进程的静态列表（包括它们的 PID、内存使用量和启动命令），使用 ps(1)。要以交互方式显示所有运行进程并每隔几秒更新显示以查看系统运行状态，使用 top(1)。

默认情况下，ps(1) 仅显示由当前用户拥有、且具有控制终端的进程：
...
- PID 列显示进程 ID。PID 从 1 开始分配，递增至 99999 后回绕至起始值。若某个 PID 已被占用，则不会再次分配。
...
- `a` 显示所有用户的有控制终端的进程的信息。若系统设置 `security.bsd.see_other_uids=0`，则非 root 用户使用此选项将无法查看其他用户的进程
- `u` 显示进程属主的用户名和内存使用量
- `x` 显示守护进程的信息
...
选项 `ww` 可以使 ps(1) 显示每个进程的完整命令行，而非在超出屏幕宽度时截断。但是，在某些情况下，使用带有 `ww` 选项的 `ps` 时，输出可能仍然被截断，因为内核中存储的命令行长度有限制。对于内核线程，命令字段可能显示为 `[ ]` 包裹的名称。
...
在非常繁忙的系统上，top 可能显示略微滞后的信息，因为采样需要时间。

## 终止进程

与任何运行中的进程或守护进程通信的一种方式是使用 kill(1) 发送信号。有许多不同的信号；有些具有特定含义，而其他信号在应用程序的文档中描述。用户只能向自己拥有的进程发送信号，向其他人的进程发送信号会提示权限不足。例外是 root 用户，其可向任何进程发送信号。
...
两个信号可用于停止进程：SIGTERM 和 SIGKILL。因为进程可以读取信号、关闭可能打开的任何日志文件，并尝试在关闭前完成正在执行的操作，SIGTERM 是终止进程的较为温和的方式。在某些情况下，如果进程正在执行某些无法中断的任务，它可能会忽略 SIGTERM。

进程无法忽略 SIGKILL。向进程发送 SIGKILL 通常会立即停止该进程。

其他常用的信号是 SIGHUP、SIGUSR1 和 SIGUSR2。由于这些是通用信号，不同的应用程序会有不同的响应。例如，在更改 Web 服务器的配置文件后，需要告诉 Web 服务器重新读取其配置。重启 httpd 会导致 Web 服务器短暂停机。相反，应向守护进程发送 SIGHUP 信号来使其重新读取配置。请注意，不同的守护进程会有不同的行为，因此请参阅守护进程的文档以确定 SIGHUP 是否能达到预期的结果。

随意终止系统中的进程是危险行为。尤其 init(8)，PID 1 是最特殊的。
...
可通过界面或配置文件持久化 htop 的配置。需注意：默认配置下，使用 F10 保存后无法通过 `Ctrl + C` 退出，必须选择 `quit` 选项才能确保配置保存；也可直接通过编辑配置文件设置。
...
1. 使用 `ps aux` 和 `top` 查看系统运行的进程，按用户进程与系统守护进程进行分类，分析两类进程在父进程关系和会话组上的特征差异。
2. 使用 `nice` 和 `renice` 修改进程优先级，记录不同 nice 值对 CPU 密集型任务执行时间的影响，分析 FreeBSD 调度程序对优先级的处理机制。
3. 编写一个简单的守护进程脚本，配置为 rc.d 服务，使用 `service` 命令启动、停止和查询其状态，记录服务生命周期管理的完整流程。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/di-12.9-jie-wei-xin-linux-ban.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/di-12.9-jie-wei-xin-linux-ban.md)

---

# 12.9 微信（Linux 版）

微信尚无原生 FreeBSD 版本，需通过 Linux 兼容层安装运行。本节基于 Rocky Linux 兼容层，通过 RPM 包管理器安装并使用。

## 基于 Rocky Linux 兼容层（FreeBSD Port）

请先参照本书其他章节，安装 Rocky Linux 兼容层（FreeBSD Port）。
...
> **技巧**
>
> `libbz2.so.1` 这个库本身已存在，但文件名不同。如果找不到，可输入 `ls /compat/linux/lib64/libbz2`，然后按 **Tab** 键补全，查看实际文件名。
...
> **技巧**
>
> 找不到 Rocky Linux 的某个库时，可到 <https://rockylinux.pkgs.org/> 搜索。FreeBSD Ports 已经打包了一部分，可参照 pkg 章节使用 `pkg-provides` 搜索。

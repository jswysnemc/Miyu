> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/di-12.8-jie-linux-jail.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-12-zhang-linux-er-jin-zhi-jian-rong-ceng/di-12.8-jie-linux-jail.md)
...
Linux 兼容层（Linuxulator）与 Jail 技术的结合，使 FreeBSD 能够运行 Linux 二进制程序。Linuxulator 作为内核兼容层模块，通过系统调用转换机制使 Linux 二进制程序在 FreeBSD 内核上直接运行。

以下介绍在 FreeBSD Jail 中部署 Debian 用户空间的步骤，包括 debootstrap 的使用与初始化配置。
...
pf 防火墙的表（table）是一种命名结构，可保存地址和网络的集合，表中的地址可通过 NAT 访问网络。

即使没有相关规则引用某表，`persist` 标志也使防火墙始终保留该表，从而确保表在防火墙规则重新加载时不会被自动清空。
...
> **技巧**
>
> 上述示例中的 `192.168.5.1`、`em0` 为占位符，须替换为实际的值。
...
Debian 使用 systemd 作为初始化系统，而 Jail 因缺少必要的 cgroup 挂载和系统权限，无法使用 systemd，因此无法直接运行相应命令（但 service 命令仍可使用）。此处使用 **/bin/true** 安全返回 `true`（成功状态），不执行任何操作。
...
antiX Linux 是基于 Debian 的轻量级发行版，采用 SysVinit 初始化系统而非 systemd。SysVinit 是传统的 UNIX 初始化系统，依赖运行级别（runlevels）和 init 脚本进行服务管理，在 Jail 环境中兼容性更好。
...
antiX 不使用 systemd，而是使用 rc 进行初始化。此处 **/etc/init.d/rc 3** 指定 antiX Jail 在运行级别 3 启动，因此 Debian Jail 中因 systemd 受限而导致的服务启动问题不会在此出现，可直接在 Jail 启动时运行服务（如 sshd）。
...
root@antix:/# mandb       # apt upgrade 时如果出现 mandb 权限错误，多次执行 mandb 命令即可完成索引更新。
```
...
创建 **/etc/fstab.alpine** 文件。**/tmp** 挂载注释掉是为了避免将整个宿主机 **/tmp** 目录暴露给 Jail，提高安全性：
...
FreeBSD # jail -r alpine    # 先关闭 Alpine Jail，以便在 FreeBSD 宿主机中进行 OpenRC 配置
```
...
在这 4 个 Jail 中分别安装 xclock、Firefox、Chrome、jwm 这几款软件，其中 jwm 用于界面管理。Alpine 中 VNC 方法未成功，Firefox 和 Chrome 也未成功运行，但 xclock、xterm 等可正常运行。
...
在 Windows 10 桌面上显示了 4 个独立的 xclock，均可正常运行。
...
该写法将 FreeBSD 的 **/tmp** 目录暴露于 Jail 并且可读写，破坏隔离性，因此注释掉。而仅挂载 **/tmp/.X11-unix** 可显著提高安全性。
...
宿主机 X Server TCP 监听加 xhost 连接管理的方式安全性较差，这是 TCP 监听默认关闭的原因之一。

共享宿主机 Socket 的方式，需注意仅挂载 **/tmp/.X11-unix** 而非整个 **/tmp**，并使用 `xhost +指定IP` 而非 `xhost +`；满足这两点即可保障安全性。

带 X Server 的终端、共享主机 Socket、VNC 这三种方式值得推荐。需要指出的是，无论采用哪种方式，Linux Jail 中的兼容性均存在一定限制，不应期望每个 X 应用都能运行；相比之下，FreeBSD Jail 的兼容性则更好。

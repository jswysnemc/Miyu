> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/fu-lu1/cun-chu-ka.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/fu-lu1/cun-chu-ka.md)
...
SD 卡的标准较为复杂，与 USB-IF 协会制定的 USB 标准相比，两者的复杂程度相当。

SD 卡标准之所以复杂，是因为：随着技术发展，SD 卡协会既不会弃用旧标准（如放弃英制单位、改用公制单位），也很少升级原有标准（如提升版本号），而是另行制定更高等级的新标准。**SD 卡存在多种并行的度量衡体系，量程各有重叠和差异。**
...
microSD 通常也称 TF 卡（TF 为 SanDisk 最初的商品名 TransFlash，后被 SD 协会采纳并更名为 microSD，二者系同一物理产品），也常被称为手机存储卡。

这块闪迪的 microSD 卡标注了以下信息：

- `SanDisk Ultra`：`SanDisk` 是品牌名“闪迪”的英文名称；`Ultra` 是闪迪的产品型号系列，通常译为“至尊高速”。
- `128 GB`：存储卡的容量是 128 GB。
- `C10`（`10` 被圆形“C”符号环绕）：该参数在当前产品中的参考价值有限，目前市售存储卡几乎全部标注为 C10，低于该等级的产品已较为少见。
- `U1`（`1` 被圆形“U”符号环绕）：`U1` 的最低持续写入速度为 10 MB/s，`U3` 为 30 MB/s。**不存在 U2 等级**。目前仅有 U1 和 U3，通常只有较老产品或低速产品会标注 U1。
- `microSDXC`：表示该卡容量位于超过 32 GB 至 2 TB 范围内（市场上常见产品从 64 GB 起步）；4 GB–32 GB 的规格称为 microSDHC。其余规格目前已极为少见，包括容量不超过 2 GB（microSD）或容量大于 2 TB（microSDUC）的规格。由于存储卡本身已明确标注容量，该参数在实际选购中参考价值有限。SD 协会（SD Association）定义：SDHC 标准为超过 2 GB 至 32 GB，SDXC 标准为超过 32 GB 至 2 TB。
...
- `667x`、`1066x`：雷克沙通常标注 667x 或 1066x。这种标识方法主要源自光驱倍速体系，目前仅在光盘设备等领域沿用，属于较为古老的标注方式（起源于 20 世纪 80 年代）。

① 667x = 150 KB/s × 667 ≈ 100 MB/s；

② 1066x = 150 KB/s × 1066 ≈ 156 MB/s。

- `V30`：新产品通常将 `U3` 标为 `V30`。无需刻意选择 V60，该等级产品价格较高（普通 A2 存储卡约 1 元/GB，V60 约 3 元/GB，且 V60 与 A2 规格通常不共存）。V90 等级的 microSD 卡目前较为少见；低于 V30 的亦较为少见，通常标注为 `U1`。

速率标准对应关系：***C10 = U1 = V10*** 这三个等级含义相同，但通常会同时标注。

该闪迪 microSD 存储卡上标注了 7 项参数，其中有 4 项在实际使用中的参考价值较低。
...
早期，廉价的大容量存储卡往往是扩容卡。但由于树莓派启动盘制作工具内置了镜像校验程序，此类卡无法通过其验证。如果标称容量和实际容量不符，则无法完成镜像写入。因此，对于树莓派无须考虑此问题。

当前存储颗粒成本已大幅降低，通常不会再采用扩容手段。若标称容量并非明显异常（如超过 128 GB 却价格极低），通常不会出现此问题。
...
移速（MOVE SPEED）该卡的测试速度甚至高于部分三星产品。~~这是用空间换时间了吗？~~ 某些 A2 存储卡实测 4K 读写不足 1.5 MB/s，已超出参数虚标范围。
...
由于最终下载链接仍会跳转至上述页面，不建议直接访问其官方网站。
...
在选购固态硬盘时，不应仅关注读写速度，更需关注固态硬盘的主控、NVMe 协议版本及支持状态。

多数小众品牌固态硬盘不支持 ASPM（Active State Power Management，活动状态电源管理）技术。此技术能在保证固态硬盘运行效率的同时为其降温，实测可使硬盘温度降低约 20℃。

部分小众品牌固态硬盘因无法良好适配，开启 ASPM 后即出现掉盘，因此在固件中主动关闭该技术。另有部分小众品牌固态硬盘仍在使用 NVMe 1.4 协议版本。甚至存在多块硬盘使用相同序列号的情况。硬盘序列号的重要性不亚于网卡的 MAC 地址，原则上不应重复，若重复则系统可能无法正确识别多块硬盘。
...
其原因有二：第一，未使用超频读卡器（即雷克沙配套的读卡器，用于支持其自定义协议）；第二，UHS-I 协议的理论速度上限为 104 MB/s（SDR104），任何存储卡理论上无法超越该速度，除非使用 UHS-II（两排金手指）。然而，microSD 极少采用 UHS-II，UHS-II 主要用于标准大小的 SD 卡（相机使用）。市场上虽有少量 UHS-II microSD 产品，如至誉 Catalyst 系列和 Sabrent Rocket 系列，但价格较高且选择有限。
...
- RC丸钢. 移速（MOVE SPEED）64 GB TF（MicroSD）存储卡测试[EB/OL]. [2026-03-25]. <https://www.bilibili.com/read/mobile?id=21681916>. 对移速品牌 TF 卡的实际读写速度测试与评价。
- SilentNocturne. 移速这个卡虚标了，速度只有标注的二分之一[EB/OL]. [2026-03-25]. <https://post.smzdm.com/talk/p/az6o8zkr/>. 指出移速存储卡实测速度与标称值存在较大差距。
- 远航的加菲猫. Mvespeed 移速 400G 内存卡简单测评[EB/OL]. [2026-03-25]. <https://post.smzdm.com/p/arq759g7/>. 对大容量移速存储卡的容量与性能实测。

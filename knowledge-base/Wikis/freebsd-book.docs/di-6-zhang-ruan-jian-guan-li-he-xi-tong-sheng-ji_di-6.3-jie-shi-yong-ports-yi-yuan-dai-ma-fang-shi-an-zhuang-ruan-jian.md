> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-6-zhang-ruan-jian-guan-li-he-xi-tong-sheng-ji/di-6.3-jie-shi-yong-ports-yi-yuan-dai-ma-fang-shi-an-zhuang-ruan-jian.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-6-zhang-ruan-jian-guan-li-he-xi-tong-sheng-ji/di-6.3-jie-shi-yong-ports-yi-yuan-dai-ma-fang-shi-an-zhuang-ruan-jian.md)
...
> **注意**
>
> Ports 和 pkg 可以同时使用，多数用户也是如此。但需要注意 Ports 和 pkg 应使用同一分支：如果 Ports 使用 main 分支，则 pkg 应使用 latest 源；如果 Ports 使用 quarterly 分支，则 pkg 应使用 quarterly 源。分支不一致会导致依赖问题（比如 SSL）。latest 源也比 main 分支下的 Ports 发布更晚（其软件包由 main 构建而来），因此即使使用 latest 源，也可能会出现上述问题。遇到问题时卸载 pkg 安装的包，重新使用 Ports 编译即可。
...
portupgrade 是另一个常用的 Ports 更新工具。

```sh
# cd /usr/ports/ports-mgmt/portupgrade && make install clean
# portupgrade -ai # 自动升级所有软件，i 会逐个确认

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-6-zhang-ruan-jian-guan-li-he-xi-tong-sheng-ji/di-6.8-jie-freebsd-jing-xiang-zhan-xian-zhuang.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-6-zhang-ruan-jian-guan-li-he-xi-tong-sheng-ji/di-6.8-jie-freebsd-jing-xiang-zhan-xian-zhuang.md)

---

# 6.8 FreeBSD 镜像站现状

FreeBSD 官方 rsync 服务不向公众开放，且项目不接受官方二级镜像申请。本节梳理镜像站部署的政策约束、历史演变与社区实践方案。

## 镜像站现状与基本格局

### 官方未开放 rsync 服务

当前 FreeBSD 镜像站生态面临两个核心约束：

1. 官方 rsync 服务暂未对公众开放；
2. 项目未接受镜像站的官方二级镜像申请。
...
追溯历史记录，FreeBSD 基础设施集群曾于 2012 年发生安全入侵事件。项目在全面转向 pkg 分发机制后终止了外部镜像的授权，以保障软件供应链完整性与安全性。

参考文献：

- delphij. FreeBSD.org 这次的入侵事件[EB/OL]. (2012-12-17)[2026-03-25]. <https://blog.delphij.net/posts/2012/12/freebsdorg-2/>. 中文说明。
- FreeBSD Project. FreeBSD.org intrusion announced November 17th 2012[EB/OL]. (2012-11-17)[2026-03-25]. <https://www.freebsd.org/news/2012-compromise.html>. FreeBSD 项目官方说明。

#### 传输机制因素

当前集群的数据同步机制采用了基于 ZFS 文件系统的直接传输方式（通过 `zfs send` / `zfs receive` 命令实现），而非传统的 rsync 镜像站同步模式。这也限制了外部镜像的可行性。
...
>>
>> 就像今天发生的事情一样：我批评了一个据说用于测试并切换不同 FreeBSD 包镜像的测速工具，可安装它却需要额外拉上 45 个依赖。这就好比在设想皇帝该穿什么衣服、用哪种金锄头锄地——或者发明一种只能在白天亮灯的灯泡。
>>
...
>
>> 结合王阳明平定南赣，改革吏治，肃清朝野，破官、民“心中贼”的史实，请读者阐述：强迫某人使之自由，这本身是一种自由还是法西斯主义？

### 中国大陆目前没有 FreeBSD 官方镜像站

经多次沟通尝试（包括通过邮件列表联系约五次，其中三次获得回应，两次未获回应），均未取得有效进展。官方主要回复为“深表歉意，但台湾地区已有镜像”，未就镜像申请的具体流程提供进一步说明。此外，曾特别向中国科学技术大学 Linux 用户协会申请镜像，反馈 FreeBSD 官方也未获回应。
...
优先建议高校学生使用校内资源搭建，或者直接从 USTC 的 `rsync` 服务进行同步。建议同步前先咨询 USTCLUG，以免带来不必要的麻烦，联系方式：[lug@ustc.edu.cn](mailto:lug@ustc.edu.cn)。参考 [科大源同步方法与注意事项](https://mirrors.ustc.edu.cn/help/rsync-guide.html) 同步。

## 官方给出的镜像站基本要求

- 服务器的 root 权限在国内同样稀缺，开源镜像站通常不会给予；
- IPv6 及 BGP 网络——国内同样较为缺乏；其中 CN2 网络在中国大陆同样稀缺，但并非 FreeBSD 官方要求，而是国内网络环境的额外考量；
...
FreeBSD 在中国大陆境内为数不多且能正常同步的镜像站，均未使用 `rsync` 等方式同步，而是采取了一些特殊手段。

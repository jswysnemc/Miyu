> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-7-zhang-wang-luo-guan-li/di-7.7-jie-v2ray-pei-zhi.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-7-zhang-wang-luo-guan-li/di-7.7-jie-v2ray-pei-zhi.md)
...
配置完成后，应重新加载配置文件使更改立即生效。例如，sh 或 Bash 必须执行 `source ~/.profile`，csh 必须执行 `source ~/.cshrc`，也可以注销并重新登录。对于系统服务运行方式（如 rc.conf），因为系统服务通过 sysrc 注入环境变量，无需此配置。

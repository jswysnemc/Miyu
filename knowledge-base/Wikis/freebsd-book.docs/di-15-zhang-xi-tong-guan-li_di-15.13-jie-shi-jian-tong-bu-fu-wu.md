> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.13-jie-shi-jian-tong-bu-fu-wu.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.13-jie-shi-jian-tong-bu-fu-wu.md)
...
除了全局时区设置，每个用户也可单独设置时区，在用户的 Shell 配置文件中设置 `TZ` 变量即可。
...
创建 **/etc/wall_cmos_clock** 文件可兼容 Windows 时间设置，避免产生 8 小时时差：
...
关键字 `restrict` 控制哪些系统可访问服务器。可包含多个 `restrict` 条目，每个条目都会根据前面的声明细化访问限制。示例中的值授予本地系统完全的查询和控制访问权限，同时仅允许远程系统查询时间。

关键字 `server` 指定要查询的服务器。该文件可包含多个 `server` 关键字，每行列出一个服务器。
...
启动脚本 **/etc/rc.d/ntpd** 将首先检查 NTP 配置。如果条件允许，它会加载模块 mac_ntpd，然后以非特权用户 ntpd（用户 ID 123）启动 ntpd。

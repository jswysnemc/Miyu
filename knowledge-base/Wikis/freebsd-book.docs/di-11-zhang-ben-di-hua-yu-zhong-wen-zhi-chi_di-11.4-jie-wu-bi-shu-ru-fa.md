> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-11-zhang-ben-di-hua-yu-zhong-wen-zhi-chi/di-11.4-jie-wu-bi-shu-ru-fa.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-11-zhang-ben-di-hua-yu-zhong-wen-zhi-chi/di-11.4-jie-wu-bi-shu-ru-fa.md)
...
将 98 五笔码表（`free-bsd-98wubi-tables-master/wubi86.dict.yaml` 和 `free-bsd-98wubi-tables-master/wubi86.schema.yaml`；注意文件名虽为 `wubi86`，但实际内容为 98 五笔码表）复制到 **/usr/local/share/rime-data** 目录。98 五笔码表下载地址：[FreeBSD-98wubi-tables](https://github.com/FreeBSD-Ask/98-input)。该仓库提供了适用于 FreeBSD 的 98 五笔输入法码表文件。
...
修改 **/usr/local/share/rime-data/default.yaml** 文件，打开文件，找到 `schema_list`，在其第一行添加 `- schema: wubi98`（注意保持缩进），并删除其他输入方案，如下所示：
...
- 将 `98五笔/98/wbx.main.dict` 词库放置到 **/usr/local/share/libime/** 目录下。
...
上述操作涉及基本系统文件，建议仅修改用户级配置，直接修改系统文件会影响全局设置且系统更新时容易被覆盖。

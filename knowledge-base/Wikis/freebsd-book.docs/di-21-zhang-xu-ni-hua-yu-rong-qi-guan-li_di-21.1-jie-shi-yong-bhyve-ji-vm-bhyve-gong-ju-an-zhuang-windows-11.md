> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-21-zhang-xu-ni-hua-yu-rong-qi-guan-li/di-21.1-jie-shi-yong-bhyve-ji-vm-bhyve-gong-ju-an-zhuang-windows-11.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-21-zhang-xu-ni-hua-yu-rong-qi-guan-li/di-21.1-jie-shi-yong-bhyve-ji-vm-bhyve-gong-ju-an-zhuang-windows-11.md)

---

# 21.1 使用 bhyve 及 vm-bhyve 工具安装 Windows 11
...
Windows 11 IoT Enterprise LTSC，版本 24H2 (x64) - DVD (英文) 系统下载链接（来自 MSDN I Tell You：<https://next.itellyou.cn/>）：
...
> **技巧**
>
> 即使重启系统后也不需手动加载，vm-bhyve 会自动加载该模块。

```sh
# kldload vmm	# 加载 FreeBSD 虚拟机管理模块（vmm）
```

查看已加载的 vmm 相关内核模块。其中 `vmm.ko` 为 bhyve 虚拟机监控程序模块，`vmmemctl.ko` 为 VMware 内存气球驱动，与 bhyve 无关：
...
随后配置虚拟网络。`public` 是模板指定的虚拟交换机名称，`ue0` 为当前宿主机的物理网络接口名称，请根据实际网卡修改（可通过 `ifconfig` 命令查看）。虚拟交换机借助 bridge(4) 完成网络桥接，将虚拟机网络栈与宿主机物理网络接口相连，使虚拟机可直接访问物理网络。
...
# bhyve -H 对应 vm-bhyve cpu_hlt 指令：启用后在客户机执行 HLT 时让出宿主机 CPU，
# 可降低空闲虚拟机对宿主机的 CPU 占用
cpu_hlt="yes"
...
出现 `Press any key to boot from a CD/DVD` 提示后，请快速按几次回车键。
...
安装过程中 VNC 连接可能数次中断，重新连接即可恢复。

在 tigervnc-viewer 中输入 `localhost`，点击连接，然后按任意键继续安装过程。

> **技巧**
>
> 终止虚拟机：如果虚拟机无响应导致命令无效，请使用 `kill -9` 强制终止，以免影响宿主机关机；如果虚拟机阻碍宿主机关机，可在 tty 中按 `Ctrl`+`C` 跳过等待并强制关机。
>
> ```sh
> # vm list
...
安装过程中系统会执行更新，可以尝试跳过此步骤（经测试 `oobe\bypassnro` 无效），请根据实际情况决定。
...
本节列出 bhyve 虚拟机使用过程中可能遇到的问题及解决方法。
...
- Microsoft. 最低系统要求 - Windows IoT Enterprise[EB/OL]. [2026-04-17]. <https://learn.microsoft.com/zh-cn/windows/iot/iot-enterprise/hardware/system_requirements>. Microsoft Learn. Windows 11 IoT Enterprise LTSC 将 TPM 2.0 列为可选要求。
- FreeBSD Project. usbhid(4)[EB/OL]. [2026-04-17]. <https://man.freebsd.org/cgi/man.cgi?query=usbhid&sektion=4>. FreeBSD USB HID 子系统手册页。
- Microsoft. Firmware WEG: Frequently asked questions (FAQ)[EB/OL]. [2026-04-17]. <https://learn.microsoft.com/en-us/windows-hardware/drivers/bringup/frequently-asked-questions>. Microsoft Learn. Windows UEFI 启动要求说明，64 位 Windows 7 需要 CSM 或 64 位 UEFI 固件支持。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-1-zhang-freebsd-chu-jian/di-1.1-jie-shen-me-shi-unix.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-1-zhang-freebsd-chu-jian/di-1.1-jie-shen-me-shi-unix.md)
...
在当代语境中，UNIX 不仅意味着法律上的 **[商标](https://www.opengroup.org/openbrand/register/index2.html)**，更是一种 **哲学思想** 和一系列 **软件工程原则**。
...
1. 技术标准要求：符合 [单一 UNIX 规范](https://www.opengroup.org/openbrand/register/xym0.htm)（Single UNIX Specification，SUS），该规范定义了 UNIX 系统必须实现的接口、命令、实用程序和库函数，确保不同 UNIX 操作系统互相兼容。
...
- **避免使用不必要的二进制格式或复杂表示**：使用简单的文本格式，便于人工阅读和处理。
- **沉默是金**：程序在正常执行时不应产生冗余输出，仅在出错时提示，成功操作无输出，不显示进度等。
- **避免独占式用户界面**：应提供命令行接口，确保可通过脚本实现自动化操作。

这些原则在当时的软件设计中相辅相成，帮助开发者构建出简洁、高效、可维护的系统。FreeBSD 的开发实践深受 UNIX 哲学影响。其先进的网络功能、性能、安全与兼容性等目标，与 UNIX 哲学中“可移植性先于高效性”及“小即美”等原则一脉相承。
...
>
> 阅读上述文本和参考文献，如何理解 UNIX 哲学的局限性及其背后的时代背景。
...
Multics 是深刻影响 UNIX 的重要项目。1961 年，麻省理工学院（Massachusetts Institute of Technology，MIT）演示了 **兼容分时系统**（Compatible Time-Sharing System，CTSS）——当时最具创新性的操作系统之一。在此基础上，研究人员于 1964 年决定设计更为先进的版本，即 **多路复用** 信息和计算服务（Multiplexed Information and Computing Service，Multics）系统。

Multics 旨在创造功能强大的新软件，以及比肩 IBM 7094 且功能更丰富的新硬件。1965 年，通用电气公司（General Electric，GE）与贝尔实验室加入该项目，形成三方合作。其中，通用电气公司负责设计并生产具有全新硬件特性的计算机，以更好地支撑分时及多用户体系；贝尔实验室在计算机发展早期就开发了自己的操作系统，具备相关经验。

Multics 的开发最终陷入困境。该系统设计了大量程序及功能，且频繁引入相互矛盾的组件，导致系统过于复杂。1969 年，贝尔实验室认为，Multics 作为信息处理工具已无法为实验室提供计算服务，且设计成本过高。同年 4 月，贝尔实验室退出 Multics 项目，仅麻省理工学院和通用电气公司继续开发。
...
为验证该算法，需向磁盘写入数据，Thompson 于是编写了批量写入数据的程序。

他以每周一个程序的速度，先后完成了三项工作：用于编写代码的编辑器、将代码转换为 PDP-7 可执行机器语言的汇编器，以及“内核的外层，即操作系统”，最终初步构建了操作系统。

在 PDP-7 上开发新操作系统后不久，Thompson 与几位同事讨论时，系统尚未命名，最初称为“**UNICS**”（**非复用** 信息和计算服务，Uniplexed Information and Computing Service），后改为 UNIX，更便于记忆。

## 参考文献

- Raymond E S. UNIX 编程艺术[M]. 姜宏,何源,蔡晓骏,译. 北京: 电子工业出版社, 2012. ISBN: 978-7-121-17665-4. 阐述 UNIX 哲学与软件工程实践原则。
- Gancarz M. Linux/Unix 设计思想[M]. 漆犇,译. 北京: 人民邮电出版社, 2012. ISBN: 978-7-115-26692-7. （已绝版）提炼 UNIX 系统设计核心思想与实践方法。
...
- National Academy of Engineering. Dr. Fernando J. Corbato[EB/OL]. [2026-04-16]. <https://www.nae.edu/29551/Dr-Fernando-J-Corbato>. Corbato 主持开发了 CTSS，据该文记载，CTSS 于 1961 年 11 月在 IBM 709 上首次演示。
...
- Ritchie D M. The Evolution of the Unix Time-sharing System[EB/OL]. (1984)[2026-04-18]. <https://www.read.seas.harvard.edu/~kohler/class/aosref/ritchie84evolution.pdf>. Ritchie 详述 Unix 从 PDP-7 到 PDP-11 的演进历程，记载 Space Travel 最初在 Multics 上编写，后改写为 Fortran 在 GECOS/GE 635 上运行，最终移植至 PDP-7。
...
- SPENCER H. space news from Sept 28 AW&ST[EB/OL]. sci.space.shuttle, Google Groups, (1987-11-15)[2026-04-17]. <https://groups.google.com/g/sci.space.shuttle/c/L8-Upf8gZoY/m/NN6ngTI0K0QJ>. 亨利·斯宾塞言语出处。

## 课后习题

1. 在 FreeBSD 上安装 QEMU 并运行 FreeBSD 1.0 版本，记录启动过程中与当代 FreeBSD 的显著差异。
2. 在网页端体验 Thompson 最初的 Space Travel，总结其玩法。
3. 试列举 SUS 规范与 Windows 操作系统在设计理念与实现上的差异。

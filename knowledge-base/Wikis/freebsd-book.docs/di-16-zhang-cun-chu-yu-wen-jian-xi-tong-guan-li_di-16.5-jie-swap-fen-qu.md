> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-16-zhang-cun-chu-yu-wen-jian-xi-tong-guan-li/di-16.5-jie-swap-fen-qu.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-16-zhang-cun-chu-yu-wen-jian-xi-tong-guan-li/di-16.5-jie-swap-fen-qu.md)

---

# 16.5 swap 分区

有时，FreeBSD 系统需要更多的 swap 空间。在 FreeBSD 中，swap 可通过传统分区、交换文件或 ZFS 卷（ZVOL）等多种方式实现。本节介绍系统安装后添加 swap 空间的方法。

因为 UFS 及 ZFS 文件系统均不支持分区收缩操作，如果在系统安装阶段未配置 swap（交换分区），则只能通过 dd 命令创建交换文件或 ZFS 卷。
...
```sh
# dd if=/dev/zero of=/usr/swap0 bs=1M count=8192 status=progress  # bs=1M 表示使用 1 MB 块写入零；status=progress 用于显示写入进度
...
如果要立即启用，需先用 `mdconfig` 将交换文件配置为内存磁盘设备，再使用 `swapon` 激活交换空间。其中，`mdconfig` 用于将文件映射为内存磁盘，`swapon` 用于激活交换设备：
...
> **警告**
>
> 根据 OpenZFS Project. OpenZFS 文档[EB/OL]. [2026-03-26]. <https://openzfs.github.io/openzfs-docs/Getting%20Started/Ubuntu/Ubuntu%2022.04%20Root%20on%20ZFS.html> 所述，对内存压力极高的系统，无论 swap 空间剩余多少，使用 zvol 作为 swap 设备都可能导致系统锁死。参见 OpenZFS Project. Swap deadlock in 0.7.9[EB/OL]. [2026-03-26]. <https://github.com/openzfs/zfs/issues/7734>。而将 swap 放在其他分区又可能影响 ZFS 对 swap 的数据校验，因此上游文档建议弃用 swap。此外，swap 对系统休眠功能至关重要，如果需要该功能，需至少保证 swap 的容量不小于系统内存容量。
...
- `$(getconf PAGESIZE)`：返回系统页面大小（FreeBSD/amd64 下固定为 `4096`），使 swap 卷与系统页面对齐，以提高性能
...
写入配置后，可使用命令 `swapon -a` 检查（`-a` 表示激活 **/etc/fstab** 中所有 swap 条目），确保无错误输出。如需停用某个交换空间（如移除设备前），可执行 `swapoff /dev/zvol/zroot/swap`，该命令会将已换出的页面迁移至其余可用交换空间。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-26-zhang-jian-kong-xi-tong/di-26.3-jie-telegraf-influxdb-yu-grafana-jian-kong-ping-tai-jia-gou.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-26-zhang-jian-kong-xi-tong/di-26.3-jie-telegraf-influxdb-yu-grafana-jian-kong-ping-tai-jia-gou.md)

---

# 26.3 Telegraf、InfluxDB 与 Grafana 监控平台架构
...
InfluxDB 是一款存储和分析时间序列数据的开源数据库。FreeBSD Ports 中提供的版本为 1.8.x，该版本仍可正常使用，但 InfluxData 官方已推荐用户迁移至 InfluxDB 2.x 或 3.x。
...
安装完成后，需设置 InfluxDB 服务开机自启并启动服务。
...
> **技巧**
>
> 上述示例中的 `username`、`password` 为占位符，须替换为实际的值。
...
Grafana 服务启动后，可通过浏览器访问其 Web 界面进行配置。Grafana 的默认登录地址为 `http://localhost:3000`。
...
- InfluxData. InfluxDB 1.8 Release Notes[EB/OL]. [2026-04-17]. <https://docs.influxdata.com/influxdb/v1.8/about_the_project/releasenotes-changelog/>. 记录了 InfluxDB 1.8 最终版本 v1.8.10（2021 年发布），页面标注 v2 为当前稳定版，表明 1.8 已不再维护。

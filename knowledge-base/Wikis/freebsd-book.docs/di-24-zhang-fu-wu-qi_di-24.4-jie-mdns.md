> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-24-zhang-fu-wu-qi/di-24.4-jie-mdns.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-24-zhang-fu-wu-qi/di-24.4-jie-mdns.md)

---

# 24.4 零配置网络（mDNS/DNS-SD）

零配置网络（[Zero-configuration networking](https://en.wikipedia.org/wiki/Zero-configuration_networking)，也称 *Zeroconf*）是一组简化网络配置的技术，使网络设备无需管理员干预即可自动完成地址分配、域名解析与服务发现。

该功能在嵌入式设备上较为实用。在某些场景下，用户无法通过屏幕或串口获取树莓派的 IP 地址，但需通过 SSH 连接。树莓派 OS 默认启用 Avahi 服务且支持多播 DNS，此时用户可直接使用 `ping raspberrypi.local` 获取 IP 地址，或直接通过 SSH 连接 `raspberrypi.local`。
...
执行 ping 命令测试 FreeBSD 的主机名，将显示 IP 地址。
...
> **技巧**
>
> 上述示例中的 `ykla`、`ykla.local` 为占位符，须替换为实际的值。

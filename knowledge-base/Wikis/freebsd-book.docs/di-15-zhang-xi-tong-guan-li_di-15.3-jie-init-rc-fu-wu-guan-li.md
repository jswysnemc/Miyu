> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.3-jie-init-rc-fu-wu-guan-li.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.3-jie-init-rc-fu-wu-guan-li.md)

---

# 15.3 管理 FreeBSD 中的服务

## 概述

FreeBSD 使用传统的 BSD init（初始化系统）管理系统服务。与 systemd 等现代初始化系统不同，BSD init 采用基于脚本的服务管理方式。所有其他进程均由 init 直接或间接启动。rc 启动脚本系统用于系统初始化及服务管理。
...
`sysrc` 命令用于安全地修改 rc.conf(5) 中的系统配置值。它从 **/etc/defaults/rc.conf** 中读取 `rc_conf_files` 变量来确定要操作的文件列表，默认值为 `/etc/rc.conf /etc/rc.conf.local`，避免手动编辑导致语法错误。修改变量时，若变量已存在于这些文件中，则替换最后出现的那一个；若不存在，则追加到 `rc_conf_files` 列表中的第一个文件。
...
上述文件在 FreeBSD 源代码中主要位于 [libexec/rc](https://github.com/freebsd/freebsd-src/tree/main/libexec/rc) 路径下。

> **注意**
>
> 不应编辑 **/etc/defaults/rc.conf** 文件中包含的默认设置。所有系统特定的更改应写入 **/etc/rc.conf**。**/etc/rc.conf** 文件的优先级高于 **/etc/defaults/rc.conf** 文件。换言之，**/etc/rc.conf** 文件将覆盖 **/etc/defaults/rc.conf** 文件中的同名配置项。
...
系统配置的主要位置是 **/etc/rc.conf**。该文件包含广泛的配置信息，系统启动时读取，用于配置系统。它为 rc\* 文件提供配置信息。

**/etc/rc.conf** 中的条目将覆盖 **/etc/defaults/rc.conf** 中的默认设置。推荐将特定系统的配置放入 **/etc/rc.conf.local** 文件。

> **技巧**
>
> 系统更新不会覆盖 **/etc/rc.conf**，因此系统配置信息不会丢失。

集群应用程序中，可以采取多种策略，将全站配置与特定系统配置分开，以减少管理开销。

例如，**/etc/rc.conf** 文件中的这些条目适用于所有系统：
...
> ```
>
> 上述命令中 `"YES"` 的双引号可省略，系统会自动添加（FreeBSD 15.0-RELEASE 起如此）。`"NO"` 同理。
...
opensm_enable="NO"		# 启动 infiniband 设备的 Opensm，默认关闭
...
`reboot` 与 `halt` 在 FreeBSD 中属于同一程序，根据调用名称的不同而表现出不同行为。而在 Linux 中，这些命令可能是 systemd 的符号链接，行为也不同。FreeBSD 的设计更接近传统 UNIX 的行为。

关机：

- 使用 `shutdown now` 不会关机，而是切换到“单用户模式”，将提示：`Enter full pathname of shell or RETURN for /bin/sh :` 回车后进入单用户模式；
- 使用 `shutdown -h now` 不会彻底断电，只会停止系统的运行，提示：`The operating system has halted. Please press any key to reboot.` 此处按任意键可重启系统；
- 正确的关机并断电命令是 `poweroff`，等同于命令 `shutdown -p now`。它会通过 init(8) 运行 **/etc/rc.shutdown** 脚本（源代码位于 **libexec/rc/rc.shutdown**）有序关闭服务。

重启：

- 重启命令与 Linux 相同，均为 `reboot`，但参数不通用。
- FreeBSD 下 `reboot` 直接刷新文件系统缓存并向所有进程发送 SIGTERM/SIGKILL 后重启，不会执行 **/etc/rc.shutdown**。

> **注意**
>
> FreeBSD 下，关机与重启操作都只有 root 用户和 operator 组成员可以执行。

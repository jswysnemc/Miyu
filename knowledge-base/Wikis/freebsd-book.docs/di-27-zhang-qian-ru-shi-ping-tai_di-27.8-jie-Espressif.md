> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-27-zhang-qian-ru-shi-ping-tai/di-27.8-jie-Espressif.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-27-zhang-qian-ru-shi-ping-tai/di-27.8-jie-Espressif.md)
...
乐鑫科技 [Espressif](https://espressif.com/) 的核心开发项目为 ESP-IDF，这是该公司为 ESP8266、ESP32 等芯片系列提供的官方开发框架，以 Apache 2.0 许可证开源发布。ESP-IDF 框架包含完整的 Wi‑Fi/Bluetooth 协议栈、FreeRTOS 实时操作系统内核、各类外设驱动程序、功能组件库以及丰富的示例代码。

> **警告**
>
> ESP-IDF 官方尚未正式支持 FreeBSD 平台，其官方文档仅列出了 Windows、Linux、macOS 三大平台的支持说明。
>
...
在开始使用 ESP-IDF 之前，需先安装固件烧录工具和交叉编译工具链。
...
PyPI 和 Espressif 的 extra-index 中并不包含 FreeBSD-amd64 的预编译 wheel（官方只提供了 linux-amd64、win-amd64、macosx 等）。同时 `espidf.constraints.v5.5.txt` 文件中还强制指定了 `--only-binary cryptography` 和 `--only-binary tree-sitter-c`，导致 pip 拒绝从源代码编译。
...
这行命令旨在放开源代码编译限制，将文件中所有 `--only-binary` 都注释掉，允许 pip 从源代码编译 cryptography、tree-sitter-c 等包。
...
重新执行后，安装脚本即可顺利通过 Python 环境阶段。

成功编译后，即可激活 ESP-IDF 环境：
...
- 将 `--chip` 改为实际的芯片型号，如 `esp32s3`、`esp32c3`。

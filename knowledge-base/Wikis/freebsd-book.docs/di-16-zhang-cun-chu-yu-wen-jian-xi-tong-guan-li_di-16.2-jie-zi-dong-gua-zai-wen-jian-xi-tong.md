> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-16-zhang-cun-chu-yu-wen-jian-xi-tong-guan-li/di-16.2-jie-zi-dong-gua-zai-wen-jian-xi-tong.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-16-zhang-cun-chu-yu-wen-jian-xi-tong-guan-li/di-16.2-jie-zi-dong-gua-zai-wen-jian-xi-tong.md)
...
automount 已内置于 FreeBSD 基本系统，会自动监控设备插入事件并执行挂载操作，默认挂载点位于 **/media** 目录。
...
要自动挂载可移动媒体，需将文件 **/etc/auto_master** 中 **/media** 行首的注释符号移除，如下：
...
每个可自动挂载的文件系统都会在 **/media/** 下对应一个目录。该目录以文件系统标签命名。如果标签缺失，目录名称将以设备节点命名。查看挂载文件：
...
文件系统会在首次访问时自动挂载，并在闲置一段时间后自动卸载。也可以手动卸载自动挂载的设备：
...
在 **/etc/auto_master** 中 **/media** 行末尾追加 `-L=zh_CN.UTF-8`。注意 `-L=zh_CN.UTF-8` 是 mount_msdosfs(8) 的专有参数，而其他文件系统的挂载程序无法识别该参数，将导致其他设备无法挂载。
...
DSBMD（Desktop Scriptable Block Device Manager Daemon）是 FreeBSD 的介质和文件系统类型检测守护进程，采用客户端-服务器架构，允许客户端以受控方式挂载存储设备。默认配置即可自动检测并挂载可移动介质。
...
> **技术要点**
>
> DSBMD 系统分为守护进程和客户端两个组成部分。客户端向 DSBMD 发起挂载、卸载或弹出介质的请求，也可设置 CD/DVD 的读取速率；守护进程负责接收并执行这些请求。守护进程是系统特权进程，拥有执行所有操作的权限，而客户端作为普通用户进程受限于系统权限设置。权限不足的用户发出的挂载请求，由守护进程依据配置文件中的用户和组设定验证后决定是否允许。

守护进程配置文件路径为 **/usr/local/etc/dsbmd.conf**。

默认情况下，属于 `wheel` 和 `operator` 用户组的成员可以挂载设备。如果需要授予其他用户挂载权限，可修改配置文件中的相关配置项。
...
![系统托盘](../.gitbook/assets/dsbmd-dsbmc-systray.png)

打开主窗口，依次点击 `preferences`→`general settings`，勾选 `automatically mount devices` 以启用自动挂载功能。

![图形界面](../.gitbook/assets/dsbmd-dsbmc-gui.png)

![设置自动挂载](../.gitbook/assets/dsbmd-dsbmc-options.png)

插入 U 盘等可移动介质后，系统桌面将显示挂载提示信息：

![插入挂载提示](../.gitbook/assets/dsbmd-dsbmc-add-tip.png)

挂载点默认位于 **/media** 目录，且挂载点的属主为发起挂载请求的客户端用户。

![mount 信息](../.gitbook/assets/dsbmd-dsbmc-userperm.png)

##### Xfce 自动启动配置

在 Xfce 桌面环境，可以配置 `dsbmc` 自动启动。点击 `设置` → `会话和启动`，按如下方式配置：

![开机自启](../.gitbook/assets/dsbmd-dsbmc-7.png)
...
该配置将以后台方式启动 `dsbmc-cli` 并启用自动连接；`-a` 参数用于启用自动连接模式，使客户端持续监控设备事件并自动执行相应操作，此方式不提供图形界面提示。

### 守护进程配置详解

默认配置通常满足基本使用需求；如需更细致地控制挂载，则需修改守护进程的配置文件，其路径为 **/usr/local/etc/dsbmd.conf**。
...
配置文件默认启用了 `usermount` 选项，但要生效，还需在系统内核参数中启用 `vfs.usermount`，可在 **/etc/sysctl.conf** 文件中添加以下内容：
...
![改变挂载权限](../.gitbook/assets/dsbmd-customperm.png)
...
- FreeBSD Project. autofs -- filesystem automounter[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=autofs&sektion=4>. autofs 文件系统手册页，描述自动挂载机制。
- FreeBSD Project. auto_master -- auto_master database[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=auto_master&sektion=5>. 自动挂载主配置文件格式手册页。
- FreeBSD Project. automountd -- daemon handling autofs mount requests[EB/OL]. [2026-04-14]. <https://man.freebsd.org/cgi/man.cgi?query=automountd&sektion=8>. 自动挂载守护进程手册页。

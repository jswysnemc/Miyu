> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.1-jie-qi-dong-yin-dao-qi-ji-pei-zhi-wen-jian-loader-conf.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.1-jie-qi-dong-yin-dao-qi-ji-pei-zhi-wen-jian-loader-conf.md)

---

# 15.1 启动引导器

FreeBSD 的启动过程分为三个核心阶段：固件初始化（BIOS 或 UEFI）→ boot0/boot1 链式引导 → loader 引导，其中 loader 阶段通过 loader.conf 实现参数化配置。本节解析启动流程并说明 loader.conf 的关键配置项。

## FreeBSD 启动过程概述

FreeBSD 的启动过程是一个多阶段的有序流程，从硬件加电自检（POST）开始，经过固件初始化、引导加载程序执行、内核加载，最终进入用户空间初始化。loader 是启动过程的最终阶段。
...
4. **内核初始化阶段**：内核加载后，首先探测硬件并初始化设备，然后挂载根文件系统，启动 init(8) 进程。init(8) 始终是内核启动的第一个用户空间进程，其进程 ID（PID）始终为 1。
...
| ---- | ---- |
| SIGUSR1 | 停机 |
| SIGUSR2 | 停机并关闭电源（需硬件支持） |
| SIGTERM | 终止多用户操作，进入单用户模式 |
| SIGINT | 终止全部进程并重新启动系统 |
| SIGTSTP | 停止创建新进程，让系统缓慢闲置（reboot(8) 和 halt(8) 使用此信号） |
| SIGHUP | 重新读取 ttys(5) 文件 |
...
- 联想集团. 硬盘扇区标记相关说明[EB/OL]. [2026-04-28]. <https://iknow.lenovo.com.cn/detail/111710>.

### 阶段 1 和阶段 2

从概念上讲，阶段 1 和阶段 2 是同一程序在同一磁盘区域上的组成部分。受空间限制，它们分为两个部分，但始终一起安装。
...
最后，默认情况下，loader 等待用户按键 10 秒，若无中断则启动内核。若用户中断此过程，将看到支持命令集操作的提示符，用户可在其中调整变量、卸载所有模块、加载模块，然后启动或重启。
...
| `include filename` | 读取指定文件并逐行解释。错误会立即中断文件的引入。 |
...
使用 shutdown(8) 执行受控关机时，init(8) 将尝试运行脚本 **/etc/rc.shutdown**，然后向所有进程发送 TERM 信号，随后向任何未及时终止的进程发送 KILL 信号。
...
#bootlock_password=""		# 设置启动锁定密码，避免未授权启动（参见 loader.conf(5)）
...
引导选择界面的等待时间由 `autoboot_delay` 参数控制，该参数定义了系统自动启动默认内核前等待用户干预的时长。
...
可以自定义引导加载程序的 Logo 以定制系统启动界面。默认有几种 Logo 可选：

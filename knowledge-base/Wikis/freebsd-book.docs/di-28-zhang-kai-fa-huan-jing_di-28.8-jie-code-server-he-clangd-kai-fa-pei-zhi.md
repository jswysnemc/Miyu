> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-28-zhang-kai-fa-huan-jing/di-28.8-jie-code-server-he-clangd-kai-fa-pei-zhi.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-28-zhang-kai-fa-huan-jing/di-28.8-jie-code-server-he-clangd-kai-fa-pei-zhi.md)

---

# 28.8 code-server 和 clangd 开发环境

本节在 13.2-RELEASE 和 14.0-RELEASE 上已验证通过，其他版本请谨慎参考。

code-server 是 VS Code 的开源服务器版，可通过 Arch Linux 兼容层运行（因为上游 FreeBSD 版 code-server 存在问题）。
...
借助 FreeBSD 内核的路径劫持特性，虽然运行 code-server 使用了 Linux 兼容层，但 clangd 以及其他所有开发工具仍然完全由 FreeBSD 提供。

首先应当明确，Linux 二进制兼容模式并非 Linux 模拟器，也并非 Linux 虚拟机。运行 Linux 程序的主体仍然是 FreeBSD 内核本身。

由于主体仍是 FreeBSD 内核，这必然涉及 Linux 程序和 FreeBSD 程序的混合运行，而混合运行又会带来动态链接库调用的问题。对已编译的二进制程序，其所需的动态链接库路径是硬编码的。例如，如果某个 Linux 二进制程序依赖 **/lib/glibc.so**，那么它一定会在 **/lib/glibc.so** 查找这个文件，而非其他位置。但在 FreeBSD 下，Linux 的运行时环境位于 **/compat/linux** 目录下。

为了解决这个问题，FreeBSD 在内核层面劫持路径：一个 Linux 二进制程序尝试 open **/lib/glibc.so** 时，FreeBSD 内核会自动在路径前加上 **/compat/linux**，使其变为 **/compat/linux/lib/glibc.so**。这一过程对应用程序而言是透明的，应用程序自身无法感知最终获得的文件路径，但从 FreeBSD 内核的视角来看，它实际访问的是 **/compat/linux/lib/glibc.so**。

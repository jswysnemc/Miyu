> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-29-zhang-freebsd-nei-he-jia-gou/di-29.4-jie-ji-qi-wu-guan-de-nei-he-xuan-xiang-zhu-jie.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-29-zhang-freebsd-nei-he-jia-gou/di-29.4-jie-ji-qi-wu-guan-de-nei-he-xuan-xiang-zhu-jie.md)
...
>
>>以 `device`、`options`、`machine`、`ident`、`maxusers`、`makeoptions`、`hints` 等开头的行应放入将要运行 config(8) 的内核配置文件中。
>>
>>以 `envvar hint.` 开头的行应放入 hints 文件中。
>>
...
使用以上方法将值编译进内核，以便内核可以通过 `getenv()`（或者用户空间的 kenv(1)）访问。文件格式为 `variable=value`，详见 [kenv(1)](https://man.freebsd.org/cgi/man.cgi?query=kenv&sektion=1)。
...
2. 在 **/boot/loader.conf** 文件中设置可调节参数：`kern.maxswzone`、`kern.maxbcache`、`kern.maxtsiz`、`kern.dfldsiz`、`kern.maxdsiz`、`kern.dflssiz`、`kern.maxssiz` 和 `kern.sgrowsiz`。
...
#options 	SCHED_ULE # 默认的 ULE 调度程序
```

`SCHED_4BSD` 和 `SCHED_ULE` 必须二选一。这些选项用于选择需编译进内核的调度程序。

说明：

- `SCHED_4BSD` 使用全局运行队列且没有 CPU 亲和性，这在 SMP 系统中性能表现相对有限。但它具有非常好的交互性和优先级选择能力。
...
> **注意**
>
> 选项 `INVARIANT_SUPPORT`、`INVARIANTS`、`KTR`、`LOCK_PROFILING` 和 `WITNESS` 已隐式启用这一行为。
...
- `CALLOUT_PROFILING`：启用对 [callout(9)](https://man.freebsd.org/cgi/man.cgi?query=callout&sektion=9) 后端使用的 callwheel 数据结构的基础分析。
...
`LOCK_PROFILING` —— 锁分析。详情见 [LOCK_PROFILING(9)](https://man.freebsd.org/cgi/man.cgi?query=LOCK_PROFILING&sektion=9)。
...
为 [callout(9)](https://man.freebsd.org/cgi/man.cgi?query=callout&sektion=9) 后端进行分析。
...
针对 [epoch(9)](https://man.freebsd.org/cgi/man.cgi?query=epoch&sektion=9) 的误用进行调试跟踪。
...
`MALLOC_DEBUG_MAXZONES` 可为小于一页的 [malloc(9)](https://man.freebsd.org/cgi/man.cgi?query=malloc&sektion=9) 分配启用多个 uma 区域。其目的是将不同的 malloc 类型隔离到哈希类别中，这样所有缓冲区溢出和释放后使用（use-after-free，UAF）问题通常只会影响该哈希类别中的 malloc 类型的内存。这纯粹是一种调试工具；通过改变哈希函数并追踪遭到破坏的哈希类别，各实例哈希类别的交集可指向滥用的单个 malloc 类型。此时可以使用检查或 [memguard(9)](https://man.freebsd.org/cgi/man.cgi?query=memguard&sektion=9) 以捕获问题代码。
...
`DEBUG_REDZONE` 为 [malloc(9)](https://man.freebsd.org/cgi/man.cgi?query=malloc&sektion=9) 启用缓冲区下溢和缓冲区上溢的检测。

```ini
#options 	EARLY_PRINTF
```

`EARLY_PRINTF` 可在内核非常早期（在调用 `cn_init()` 之前）使用一种特殊的 `printf`（`eprintf`）。这通常仅用于启动早期调试。一般情况下，它未被定义。此处将其注释掉是因为该功能通常不可用，而且所需的 `eputc()` 也未定义。
...
`KTRACE` 启用系统调用跟踪功能 [ktrace(2)](https://man.freebsd.org/cgi/man.cgi?query=ktrace&sektion=2)。为更好地支持 SMP，KTRACE 使用一个工作线程来异步处理大多数跟踪事件，而非由生成事件的线程直接处理。这需要预先分配一个用于存储跟踪事件对象的池。选项 `KTRACE_REQUEST_POOL` 指定该池的初始大小。
...
- `KTR_BOOT_ENTRIES` 定义了早期启动期间的条目数量，在 [malloc(9)](https://man.freebsd.org/cgi/man.cgi?query=malloc&sektion=9) 可用之前使用。
...
选项 `INVARIANTS` 在多个源文件中用于启用对内部结构的额外完整性检查。默认禁用，因为检查这些条件会额外消耗时间，而这些问题通常仅由编程错误引起。

```ini
options 	INVARIANT_SUPPORT
```

选项 `INVARIANT_SUPPORT` 能在编译时加入对部分内部结构的验证支持。`INVARIANT_SUPPORT` 是启用 `INVARIANTS` 的前提，因为启用 `INVARIANTS` 会调用这些函数。其目的是，如果启用了 `INVARIANT_SUPPORT`，可以为单个源文件设置 `INVARIANTS`（通过修改源文件或在命令行指定）。如果希望构建带有 `INVARIANTS` 的内核模块，将 `INVARIANT_SUPPORT` 添加到内核中可以提供所有必要的基础设施，且不会增加额外开销。
...
STACK 将启用 [stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9) 功能，可捕获内核栈。如果内核编译中包含 [DDB(4)](https://man.freebsd.org/cgi/man.cgi?query=ddb&sektion=4)，则 [stack(9)](https://man.freebsd.org/cgi/man.cgi?query=stack&sektion=9) 也会自动编译进内核。
...
> **注意**
>
> 如果使用了选项 `INET`、`INET6` 或同时指定了两者：必须至少指定一个拥塞控制选项，否则将导致编译失败。`GENERIC` 定义了 `CC_CUBIC`。如果编译了多款拥塞控制，可能需要指定一个默认值。
...
这个选项可开启各种详细打印功能。它由一个位掩码控制（可通过 socket 选项和 sysctl 设置）。包含该选项本身不会立即产生日志，必须先设置相应的位。但打印信息可能非常冗长。因此，如果没有启用该选项，代码就不会做位检测和打印，这样可提高运行速度。如果并非调试用途，请不要使用。
...
使用这些功能需启用 [ktr(4)](https://man.freebsd.org/cgi/man.cgi?query=ktr&sektion=4)，然后通过 sysctl 设置打开或关闭各种日志位。使用 [ktrdump(8)](https://man.freebsd.org/cgi/man.cgi?query=ktrdump&sektion=8) 提取日志，再通过显示程序处理，就能生成图表等可视化内容。
...
[altq(9)](https://man.freebsd.org/cgi/man.cgi?query=altq&sektion=9)。通过选项 `ALTQ` 启用钩子的基础部分。各个调度策略必须编译进基本系统，不能在此阶段作为模块加载。ALTQ 需要稳定的 TSC，如果 TSC 不稳定或在 CPU 节流时发生变化，则必须同时启用选项 `ALTQ_NOPCC`。
...
- ①：[netgraph(4)](https://man.freebsd.org/cgi/man.cgi?query=netgraph&sektion=4) 系统，是一款网络工具包。使用选项 `NETGRAPH` 启用 netgraph 核心代码。

可通过下列对应的选项启用各个节点类型；不过这并非严格必要，因为如果节点类型尚未编译进内核，netgraph 会自动加载对应的 KLD 模块。下面的每种类型都有对应的手册页，例如 [ng_async(4)](https://man.freebsd.org/cgi/man.cgi?query=ng_async&apropos=0&sektion=4)。
...
```ini
options 	VIMAGE
options 	VNET_DEBUG	# VIMAGE 供调试使用
```
...
启用网络功能时，[loop(4)](https://man.freebsd.org/cgi/man.cgi?query=loop&apropos=0&sektion=4) 设备是 **必需的**。
...
[wlan(4)](https://man.freebsd.org/cgi/man.cgi?query=wlan&apropos=0&sektion=4) 设备提供了支持 802.11 驱动的通用代码，包括主机 AP 模式；wi 和 ath 驱动需要 `wlan`，并且最终将成为所有 802.11 驱动的必需项。
...
>
> 建议首次启用此功能时，在 **/etc/rc.conf** 文件中将 `firewall_type` 设置为 `open`，然后在确认新内核功能正常工作后，再在 **/etc/rc.firewall** 文件中完善防火墙规则。
...
`NETDUMP` 选项将在内核中启用对 [netdump(4)](https://man.freebsd.org/cgi/man.cgi?query=netdump&sektion=4) 客户端支持。它可在内核 panic 时将内核转储发送到远程主机。

```ini
options 	NETGDB
```

`NETGDB` 选项在内核中启用对 [netgdb(4)](https://man.freebsd.org/cgi/man.cgi?query=netgdb&sektion=4) 的支持。它能通过网络将 panic 的内核作为 GDB 远程调试目标。
...
[firmware(9)](https://man.freebsd.org/cgi/man.cgi?query=firmware&sektion=9) 支持，firmware 镜像的加载与管理。
...
`ISP_TARGET_MODE` 可启用 [isp(4)](https://man.freebsd.org/cgi/man.cgi?query=isp&sektion=4) target mode 操作。
...
以上驱动均为 [cam(4)](https://man.freebsd.org/cgi/man.cgi?query=cam&sektion=4) 子系统的一部分。

它们取代了功能较少的 [ata(4)](https://man.freebsd.org/cgi/man.cgi?query=ata&sektion=4) 子系统驱动，并支持相同的硬件。
...
还可通过 `atacore` 驱动，然后再按厂商判断各个总线和芯片组驱动以选择单独的驱动。例如，要构建一个仅支持 VIA 芯片组的系统，可以省略 `ata` 这行，仅包含驱动 `atacore`、`atapci` 和 `atavia`。
...
[iflib](https://man.freebsd.org/cgi/man.cgi?query=iflib&sektion=9) 网络接口驱动框架。
...
这两个选项分别用于调整 mbuf 集群大小和 mbuf 大小。在将它们的默认值更改为其他值时，必须格外谨慎处理网卡驱动模块，因为这可能导致内核假定的 mbuf 大小与模块假定的 mbuf 大小不匹配。目前，唯一能够检测这种不匹配的驱动是 [ti(4)](https://man.freebsd.org/cgi/man.cgi?query=ti&sektion=4)。
...
`SND_FEEDER_RATE_HP`（feeder_rate）将尽可能使用高精度 64 位运算（在默认情况下会尽量避免）。可能造成运算速度下降。
...
通过 [ioctl(2)](https://man.freebsd.org/cgi/man.cgi?query=ioctl&sektion=2) 在用户空间访问 I²C 从设备。
...
[miibus(4)](https://man.freebsd.org/cgi/man.cgi?query=miibus&sektion=4) MII 总线功能代理设备。
...
设置要分配的 `sf_buf` 数量。`sf_buf` 是 sendfile(2) 使用的虚拟缓冲区，用于映射文件虚拟存储器页，默认数量约为 16*MAXUSERS+512。通常每个并发文件传输需要约 4 个此类缓冲区。

```ini
options 	DEBUG_LOCKS
```

启用锁机制的额外调试代码。此功能会将获取锁的文件名和行号信息存储在锁结构中，并修改多个函数调用以传递相关数据。
...
device		wsp # Wellspring 触控板，Apple 笔记本用
```
...
使用基于 USB 的以太网通信的设备，特别是符合通信设备类（CDC）以太网规范。支持夏普 Zaurus 掌上电脑、部分 DOCSIS 有线调制解调器及移动电话等设备。
...
使 `mi_startup()` 执行的 SYSINIT 过程输出更为详细的信息。该功能在移植新架构时十分有用。如果同时启用了 DDB 调试器，将显示函数名称而非地址。如果定义值为 `0`，则详细输出代码会被编译进内核但默认禁用，可通过可调参数 `debug.verbose_sysinit=1` 启用。
...
设置系统在内核发生 panic 时自动重启前的等待时间（以秒为单位）。如果设置为（`-1`），系统将无限期等待，直至在控制台上按下任意键。

```ini
options 	DIRECTIO
```

文件设置了 `O_DIRECT` 标志时，尝试绕过缓冲区缓存，将数据直接读取到用户空间缓冲区。
...
> **注意**
>
> 除非正在接收超过 4 K 的巨型帧或通过 LRO 技术实现持续数据突发，否则 `m_next` 指针通常为 NULL。因此在常规情况下，该操作实际是在向熵池注入零值。即使该指针非空，它也只是指向某个经过 256 字节对齐的 mbuf（其数量范围在数千至数万之间）。因此，哪怕是在最理想情况下，这也是劣质的熵源。如果没有对熵收集进行实际运行时分析，可能会误导用户认为收集到的熵量远超实际值，这将引发另一类安全风险。在高数据包速率场景下，以太网熵收集的成本极其高昂，可能导致高达 50% 的数据包接收丢失率。本选项仅为保持向后兼容性而提供，但无论如何都不建议在任何环境中使用。

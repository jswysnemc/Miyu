> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.8-jie-OpenSSH.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-15-zhang-xi-tong-guan-li/di-15.8-jie-OpenSSH.md)
...
> **技巧**
>
> 上述示例中的 `192.168.179.128`、`192.168.1.32`、`ykla`、`1022`、`/home/ykla` 为占位符，须替换为实际的值。
...
SSH 隧道的工作原理是在 `localhost` 的 `localport` 端口创建侦听套接字。
...
可使用 OpenSSH 服务器配置文件中的 `AllowUsers` 关键字限制允许登录的用户及其来源地址。例如，要仅允许 `user` 从 **192.168.1.32** 登录，在 **/etc/ssh/sshd_config** 中添加以下行：
...
[/crypto/openssh/sshd_config](https://github.com/freebsd/freebsd-src/blob/main/crypto/openssh/sshd_config) 是默认的 sshd 配置文件，以下对其主要配置项进行简单注解。
...
以上配置基于 FreeBSD 源码树中的 [sshd_config](https://github.com/freebsd/freebsd-src/commit/7238317403b95a8e35cf0bc7cd66fbd78ecbe521)，具体配置可能随版本更新而变化。

> **原文链接：** [https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-27-zhang-qian-ru-shi-ping-tai/di-27.7-jie-STM32.md](https://github.com/FreeBSD-Ask/FreeBSD-Ask/blob/main/di-27-zhang-qian-ru-shi-ping-tai/di-27.7-jie-STM32.md)
...
STM32CubeMX 是意法半导体官方提供的图形化硬件配置与代码生成工具，用于快速完成 STM32 项目的硬件初始化。

在 FreeBSD 平台进行 STM32 嵌入式开发，推荐采用 STM32CubeMX 初始化配置。STM32CubeMX 官方仅支持 Windows、Linux、macOS 三大平台，在 FreeBSD 上运行该软件需借助 Linux 兼容层。

关于在 FreeBSD 上安装 Linux 兼容层的详细教程，请参考本书其他章节，推荐使用 FreeBSD 官方维护的 linux-rl9 兼容层。

在开始安装 STM32CubeMX 之前，请确保系统已安装 OpenJDK（版本要求不低于 17.0.6）作为运行时依赖，本例使用 Port **java/openjdk25**。

配置好兼容层后，从 [STM32CubeMX 官网](https://www.st.com/en/development-tools/stm32cubemx.html) 下载压缩包并解压。本节撰写时，下载无需注册登录，以访客身份下载即可。下载链接将通过邮件发送，请确保邮箱地址可正常收信。下载的文件为 **stm32cubemx-lin-v6-17-0.zip**。
...
填写安装路径，请记住该路径，本例路径为 **/home/ykla/STM32CubeMX**。
...
在 **~/Desktop**（或 **~/桌面**）目录下创建 `STM32CubeMX.desktop` 文件，然后写入：
...
请将上述路径 `Exec`、`Icon` 改为实际路径。然后授予可执行权限。
...
> **技巧**
>
> 更换 STM32 型号时，CMakeLists.txt 中有多处与具体 MCU 型号绑定的占位符，必须逐一调整。最简便的做法是：在 STM32CubeMX 中切换目标芯片后重新生成项目，再对照修改 CMakeLists.txt 中对应位置。
>

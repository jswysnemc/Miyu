...
`nmcli` is a command-line tool for controlling NetworkManager and reporting network status. It can be utilized as a replacement for `nm-applet` or other graphical clients. `nmcli` is used to create, display, edit, delete, activate, and deactivate network connections, as well as control and display network device status. See [`nmcli-examples(7)`](#nmcli-examples) for ready to run nmcli examples.

Typical uses include:

- Scripts: Utilize NetworkManager via `nmcli` instead of managing network connections manually. `nmcli` supports a terse output format which is better suited for script processing. Note that NetworkManager can also execute scripts, called "dispatcher scripts", in response to network events. See [[`NetworkManager(8)`](#NetworkManager)](#NetworkManager) for details about these dispatcher scripts.
...
The actual colors used are configured as described in `terminal-colors.d(5)`. Please refer to the [\#colors](#colors) section for a list of color names supported by `nmcli`.
...
The [exit status](#exit_status) will indicate success or return a code 65 to indicate the last argument is a file name.
...
Get and change NetworkManager logging level and domains. Without any argument current logging level and domains are shown. In order to change logging state, provide `level` and, or, `domain` parameters. See [[`NetworkManager.conf(5)`](#NetworkManager.conf)](#NetworkManager.conf) for available level and domain values.
...
`add` `save`yesno \<option\> \<value\> \[+\|-\]\<setting\>.\<property\> \<value\>
...
The most typical uses are described in the [\#examples](#examples) section.
...
See also `nm-settings-nmcli(5)` for all NetworkManager settings and property names, and their descriptions; and [`nmcli-examples(7)`](#nmcli-examples) for sample editor sessions.
...
The `managed` property accepts a `--permanent` option to persist the managed state to disk, and not only in runtime. With `--permanent-only` only the permanent managed state is set, and not the runtime managed state. The special values `up` and `down` can be used to set the administrative state of the device at the same time as the runtime managed state. The `reset` value clears the explicit managed setting, and with `--permanent` or `--permanent-only` it also removes the persisted managed setting.
...
If a list of interface names is specified, the checkpoint is taken only on the specified devices. Otherwise a checkpoint is taken for all devices.
...
This section presents various examples of `nmcli` usage. If you want even more, please refer to [`nmcli-examples(7)`](#nmcli-examples) manual page.
...
`nmcli con import type openvpn file $HOME/Downloads/frootvpn.ovpn`
...
[`nmcli-examples(7)`](#nmcli-examples), [`nm-settings-nmcli(5)`](#nm-settings-nmcli), [`nm-online(1)`](#nm-online), [`NetworkManager(8)`](#NetworkManager), [`NetworkManager.conf(5)`](#NetworkManager.conf), `nm-applet(1)`, `nm-connection-editor(1)`, `terminal-colors.d(5)`.

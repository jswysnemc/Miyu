...
Read, write and target should start with either ``$SNAP``, ``$SNAP_DATA`` or ``$SNAP_COMMON`` to refer to the designated directory. See [Environment variables](https://snapcraft.io/docs/reference/development/environment-variables/) for details on where these point to on the filesystem.
...
By default, the AppArmor sandbox allows _writes_ to `$SNAP_DATA` and _reads_ from `$SNAP` (see [Environment variables](https://snapcraft.io/docs/reference/development/environment-variables/) for details).

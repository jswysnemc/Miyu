...
- paths in `/dev`, such as `/dev/sda1`
  Access to `/dev` device nodes requires both AppArmor policy and device control group inclusion, but the _system-files_ interface does not have enough information to generate the necessary policy to enable these use cases. As such, purpose-specific interfaces should be used instead, such as  [block-devices](https://snapcraft.io/docs/reference/interfaces/block-devices-interface/) or [raw-volume](https://snapcraft.io/docs/reference/interfaces/raw-volume-interface/).

> ⓘ  Do not share data between snaps

While `system-files` can be used to share data with another snap, such as within a configuration file, this behaviour is not recommended. The [content interface](https://snapcraft.io/docs/reference/interfaces/content-interface/) should be used instead.

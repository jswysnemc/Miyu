...
By design, snaps using `strict` and `devmode` [confinement](https://snapcraft.io/docs/explanation/security/snap-confinement/) run with a different root filesystem than the host. Snaps that implement a backup function, however, need access to the host's snap-related files outside of this confinement, and this is where the `system-backup` interface can help.

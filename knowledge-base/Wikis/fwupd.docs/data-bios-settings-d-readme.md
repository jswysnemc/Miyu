...
```shell
# fwupdmgr get-bios-settings --json WindowsUEFIFirmwareUpdate > $HOME/foo.json
```

Now examine `$HOME/foo.json` and modify the `BiosSettingCurrentValue` key to your desired value.

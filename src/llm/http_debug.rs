...
//!
//! 环境变量:
//! - `MIYU_DEBUG_HTTP=1|true|yes|on`：开启
//! - `MIYU_DEBUG_HTTP_SESSION=<session_id>`：可选，仅记录指定会话
//!
...
impl HttpDebugConfig {
    ...
    /// 返回:
    /// - 开启时返回配置
    pub fn from_env(paths: &MiyuPaths) -> Option<Self> {
        let flag = std::env::var("MIYU_DEBUG_HTTP").ok()?;
        if !is_truthy(&flag) {
            return None;
        }
        let session_filter = std::env::var("MIYU_DEBUG_HTTP_SESSION")
            .ok()
...
impl HttpDebugRecorder {
    ...
    pub fn start(
        ...
        PATH_HINT_PRINTED.get_or_init(|| {
            eprintln!(
                "[miyu] HTTP debug enabled; writing under {}",
                config.root.display()
            );
        });
        // 3. 写入 meta / 请求头 / 请求体
...
mod tests {
    ...
    fn test_paths(temp: &TempDir) -> MiyuPaths {

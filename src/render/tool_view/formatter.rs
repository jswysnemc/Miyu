...
pub(crate) fn render(view: &ToolView, mode: ToolCallDisplayMode) -> String {
    ...
    if view.outcome.is_none() && matches!(view.name.as_str(), "run_command" | "background_command")
    {
    ...
    if mode == ToolCallDisplayMode::Summary {
        if view.outcome.as_ref().is_some_and(|outcome| outcome.ok)
            && matches!(view.name.as_str(), "run_command" | "edit_file")
        {
            return String::new();
        }

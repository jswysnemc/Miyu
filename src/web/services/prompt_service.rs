...
/// 返回:
/// - 按名称排序的文件摘要
pub(crate) fn list(paths: &MiyuPaths, kind: PromptKind) -> Result<Vec<PromptSummary>> {
...
/// 返回:
/// - 提示词文件内容
pub(crate) fn read(paths: &MiyuPaths, kind: PromptKind, name: &str) -> Result<PromptDocument> {
    let config = AppConfig::load_or_default(paths)?;
...
/// 返回:
/// - 文件存在并删除时返回 true
pub(crate) fn remove(paths: &MiyuPaths, kind: PromptKind, name: &str) -> Result<bool> {
    let config = AppConfig::load_or_default(paths)?;
    let file_name = sanitize_name(name)?;
    let path = prompt_path(paths, &config, kind, &file_name);
    if !path.exists() {
...
/// 返回指定类型的提示词目录。
fn prompt_directory(paths: &MiyuPaths, config: &AppConfig, kind: PromptKind) -> PathBuf {
    match kind {
...
/// 返回指定提示词文件路径。
fn prompt_path(paths: &MiyuPaths, config: &AppConfig, kind: PromptKind, name: &str) -> PathBuf {

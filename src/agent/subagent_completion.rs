...
impl Agent {
        ...
        F: FnMut(AgentEvent) -> Result<()>,
    {
        ...
        loop {
            // 1. 【Miyu】【主动消费】等待本轮后台子智能体结束,持久化未消费列表在请求失败时仍会保留结果
            while pending_finished_notices(&owner_key).is_empty() {
                ...
                {
                    // 2. 【Miyu】【主动消费】终态可能出现在两次读取之间,返回前再次确认未消费列表
            ...
            // 3. 【Miyu】【主动消费】把未消费结果作为系统提醒提交给同一主模型,本次请求禁止工具调用
...
fn append_result(target: &mut ChatResult, addition: ChatResult) {
    // 1. 【Miyu】【主动消费】正文和思考内容按模型实际输出顺序拼接

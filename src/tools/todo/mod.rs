...
mod tests {
    ...
    async fn tool_supports_crud_actions() {
        ...
        let completed = registry
            ...
            .unwrap();
        assert_eq!(
            serde_json::from_str::<Value>(&completed).unwrap()["items"][0]["status"],
            "completed"
        );
        registry
            .call("todo", &json!({"action":"remove","index":1}).to_string())
            .await
            .unwrap();

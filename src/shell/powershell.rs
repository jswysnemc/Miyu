...
pub fn hook() -> &'static str {
    r#"$script:MiyuCommandNotFoundActionInstalled = $script:MiyuCommandNotFoundActionInstalled -eq $true
if (-not $script:MiyuCommandNotFoundActionInstalled) {
    $script:MiyuPreviousCommandNotFoundAction = $ExecutionContext.InvokeCommand.CommandNotFoundAction
    $script:MiyuCommandNotFoundActionInstalled = $true
}

function Invoke-MiyuPreviousCommandNotFoundAction {
    param($Name, $EventArgs)

    if ($script:MiyuPreviousCommandNotFoundAction) {
        & $script:MiyuPreviousCommandNotFoundAction $Name $EventArgs
    }
}

function Get-MiyuCommandLineCandidate {
    param([string] $Name)
...
function Test-MiyuNaturalLanguageCommand {
    param([string] $Text)
...
$ExecutionContext.InvokeCommand.CommandNotFoundAction = {
    ...
    $text = Get-MiyuCommandLineCandidate -Name ([string]$Name)
    if (-not (Test-MiyuNaturalLanguageCommand -Text $text)) {
        Invoke-MiyuPreviousCommandNotFoundAction $Name $EventArgs
        return
    }

    $miyuMessage = $text
    $EventArgs.CommandScriptBlock = {
        miyu --shell-intercept --shell powershell -- $miyuMessage 2>$null
    }.GetNewClosure()
}
"#
}

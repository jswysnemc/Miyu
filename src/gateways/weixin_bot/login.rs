...
/// 返回:
/// - 登录是否成功
pub(crate) async fn run_weixin_login(paths: &MiyuPaths, config: WeixinLoginConfig) -> Result<()> {
    ...
    loop {
        ...
        match status.status.as_str() {
            ...
            "confirmed" => {
                ...
                println!(
                    "启动命令: miyu gateway weixin-server --account {}",
                    saved.account_id
                );
                return Ok(());
            }
...
/// 返回:
/// - 微信状态目录
fn weixin_state_dir(paths: &MiyuPaths) -> PathBuf {
...
/// 返回:
/// - 微信账号目录
fn weixin_accounts_dir(paths: &MiyuPaths) -> PathBuf {
...
/// 返回:
/// - 账号文件路径
fn account_file(paths: &MiyuPaths, account_id: &str) -> PathBuf {

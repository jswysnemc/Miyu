...
/// 返回:
/// - 渠道上下文映射
fn load_context_map(paths: &MiyuPaths) -> Result<BTreeMap<String, ChannelContext>> {
    let file = contexts_file(paths);
    if !file.exists() {
...
/// 返回:
/// - 状态文件路径
fn contexts_file(paths: &MiyuPaths) -> PathBuf {
    paths
        .state_dir
        .join("gateways")
...
mod tests {
    ...
    fn test_paths(root: PathBuf) -> MiyuPaths {
        MiyuPaths {
            ...
            fish_hook_file: root.join("fish").join("miyu.fish"),

import { Activity, Bot, PanelBottomClose, Plus, TerminalSquare, X } from "lucide-react";
...
export function TerminalDock({ onClose, manager }: { onClose: () => void; manager: TerminalManager }) {
  const [view, setView] = useState<"terminal" | "tasks" | "subagents">("terminal");
    ...
    <section className="terminal-dock">
      <aside className="terminal-sidebar">
        <header className="terminal-dock-nav">
          ...
          <button type="button" className={view === "subagents" ? "active" : ""} onClick={() => setView("subagents")}><Bot size={13} /><span>子智能体</span></button>
      ...
      <div className="terminal-main">
        {view === "tasks" ? <BackgroundTasksPanel /> : view === "subagents" ? <SubagentPanel /> : manager.activeId ? <TerminalPane terminalId={manager.activeId} /> : (
